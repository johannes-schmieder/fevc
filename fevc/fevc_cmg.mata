...
struct vckss_cmg__aggregation_result
{
    string scalar status
    string scalar message
...
struct vckss_cmg__hierarchy
{
    string scalar status
    string scalar message
    struct vckss_cmg__options scalar options
    real scalar n_level
    pointer(struct vckss_cmg__level scalar) rowvector level
...
struct vckss_cmg__apply_result
{
    string scalar status
    string scalar message
    real matrix value
...
struct vckss_cmg__preflight_result
{
    string scalar status
    string scalar message
    string scalar route
...
struct vckss_cmg__route_result
{
    string scalar status
    string scalar message
    string scalar route
...
struct vckss_cmg__cells scalar vckss_cmg__empty_cells()
{
    struct vckss_cmg__cells scalar out
...
struct vckss_cmg__graph scalar vckss_cmg__empty_graph()
{
    struct vckss_cmg__graph scalar out
...
struct vckss_cmg__options scalar vckss_cmg__options_default()
{
    struct vckss_cmg__options scalar out
...
struct vckss_cmg__options scalar vckss_cmg__options_resource(
    ...
    real scalar planned_rhs)
{
    ...
    out = vckss_cmg__options_default()
...
struct vckss_cmg__forest scalar vckss_cmg__empty_forest()
{
    struct vckss_cmg__forest scalar out
...
struct vckss_cmg__aggregation_result scalar vckss_cmg__empty_aggregation()
{
    struct vckss_cmg__aggregation_result scalar out
...
struct vckss_cmg__hierarchy scalar vckss_cmg__empty_hierarchy()
{
    struct vckss_cmg__hierarchy scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy"
    out.options = vckss_cmg__options_default()
    out.n_level = 0
...
struct vckss_cmg__level scalar vckss_cmg__empty_level()
{
    struct vckss_cmg__level scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy level"
    out.graph = vckss_cmg__empty_graph()
    out.component = J(0,1,.)
    out.n_component = 0
...
pointer(struct vckss_cmg__level scalar) scalar vckss_cmg__store_level(
    struct vckss_cmg__level scalar value)
{
    struct vckss_cmg__level scalar stored

    stored = value
    return(&stored)
}

pointer(real colvector) scalar vckss_cmg__store_colvector(
...
pointer(real matrix) scalar vckss_cmg__store_matrix(real matrix value)
{
    real matrix stored
...
struct vckss_cmg__apply_result scalar vckss_cmg__empty_apply_result()
{
    struct vckss_cmg__apply_result scalar out
...
struct vckss_cmg__preflight_result scalar vckss_cmg__empty_preflight()
{
    struct vckss_cmg__preflight_result scalar out
...
struct vckss_cmg__route_result scalar vckss_cmg__empty_route()
{
    struct vckss_cmg__route_result scalar out
...
struct vckss_cmg__workspace scalar vckss_cmg__empty_workspace()
{
    struct vckss_cmg__workspace scalar out
...
struct vckss_cmg__diag_result scalar vckss_cmg__empty_diagnostics()
{
    struct vckss_cmg__diag_result scalar out
...
real matrix vckss_cmg__lex_panel(
    real matrix values,
...
real scalar vckss_cmg__dense_ids(real colvector identifier)
{
...
struct vckss_cmg__graph scalar vckss_cmg__graph_finalize(
    struct vckss_cmg__graph scalar graph)
{
...
real scalar vckss_cmg__find_root(
...
real colvector vckss_cmg__components(
    struct vckss_cmg__graph scalar graph)
{
    ...
    for (edge=1; edge<=graph.n_edge; edge++) {
        u = graph.u[edge]
        v = graph.v[edge]
        root_u = vckss_cmg__find_root(parent,u)
        root_v = vckss_cmg__find_root(parent,v)
    ...
    for (vertex=1; vertex<=graph.n_vertex; vertex++) {
        ...
        root = vckss_cmg__find_root(parent,u)
...
real matrix vckss_cmg__graph_action(
    struct vckss_cmg__graph scalar graph,
...
real scalar vckss_cmg__pilot_cap(real scalar planned_rhs)
{
...
struct vckss_cmg__preflight_result scalar vckss_cmg__preflight(
    ...
    struct vckss_cmg__options scalar options)
{
    ...
    out = vckss_cmg__empty_preflight()
    ...
    out.pilot_cap = vckss_cmg__pilot_cap(max((planned_rhs,8)))
...
real scalar vckss_cmg__park_miller_step(real scalar state)
{
...
real matrix vckss_cmg__pilot_rhs(
    real colvector firm_key,
    real colvector component)
{
    ...
    for (position=1; position<=rows(firm_key); position++) {
        ...
        state[1] = vckss_cmg__park_miller_step(state[1])
        state[2] = vckss_cmg__park_miller_step(state[2])
...
struct vckss_cmg__route_result scalar vckss_cmg__route_decide(
    real scalar planned_rhs,
...
real colvector vckss_cmg__indexed_sum(
...
real colvector vckss_cmg__forest_depth(real colvector parent)
{
...
struct vckss_cmg__aggregation_result scalar vckss_cmg__aggregate_gpl(
    struct vckss_cmg__graph scalar graph,
    real colvector component,
    struct vckss_cmg__options scalar options)
{
    ...
    out = vckss_cmg__empty_aggregation()
    if (graph.status != "CONVERGED" |
    ...
    depth = vckss_cmg__forest_depth(parent)
    ...
    for (one_depth=max(depth); one_depth>=1; one_depth--) {
        ...
        if (rows(nodes) > 0) {
            contribution = vckss_cmg__indexed_sum(
    ...
    if (rows(active) > 0) {
        ...
        internal = vckss_cmg__indexed_sum(incidence_vertex,
...
struct vckss_cmg__forest scalar vckss_cmg__forest_select(
    struct vckss_cmg__graph scalar graph,
    real colvector component,
    struct vckss_cmg__options scalar options)
{
    ...
    if (rows(active) > 0) {
        forest_volume = vckss_cmg__indexed_sum(
...
real colvector vckss_cmg__cluster_members(
    real colvector head,
...
real colvector vckss_cmg__mask_side(
    real scalar mask,
...
real scalar vckss_cmg__mask_lex_less(
    ...
    real scalar cluster_size)
{
    ...
    candidate_side = vckss_cmg__mask_side(candidate,cluster_size)
    incumbent_side = vckss_cmg__mask_side(incumbent,cluster_size)
...
real scalar vckss_cmg__cluster_bad_mask(
    struct vckss_cmg__graph scalar graph,
    ...
    real scalar awd)
{
    ...
    for (candidate=2; candidate<=length(location); candidate++) {
        ...
        if (vckss_cmg__mask_lex_less(candidate_mask,best_mask,size)) {
...
struct vckss_cmg__aggregation_result scalar vckss_cmg__aggregate_vertices(
    struct vckss_cmg__graph scalar graph,
    real colvector component,
    struct vckss_cmg__forest scalar forest,
    struct vckss_cmg__options scalar options)
{
    struct vckss_cmg__aggregation_result scalar out
    ...
    for (seed_position=1; seed_position<=graph.n_vertex; seed_position++) {
        ...
        while (cluster_size[cluster_id] < options.target_size) {
            best_edge = 0
            best_rank = .
            members = vckss_cmg__cluster_members(
    ...
    while (screen_cursor <= cluster_count) {
        ...
        bad_mask = vckss_cmg__cluster_bad_mask(
        ...
        size = rows(members)
        side = vckss_cmg__mask_side(bad_mask,size)
        member_map[members] = (1::size)
...
struct vckss_cmg__aggregation_result scalar vckss_cmg__aggregate_fallback(
    struct vckss_cmg__graph scalar graph,
    real colvector component,
    struct vckss_cmg__options scalar options)
{
...
real scalar vckss_cmg__component_reduce(
    real scalar fine_vertices,
...
struct vckss_cmg__graph scalar vckss_cmg__contract_graph(
    struct vckss_cmg__graph scalar graph,
    real colvector aggregation,
    real scalar scratch_bytes)
{
    ...
    out.auxiliary_worker = J(0,1,.)
    out.status = "CONVERGED"
    out = vckss_cmg__graph_finalize(out)
...
struct vckss_cmg__level scalar vckss_cmg__level_prepare_cert(
    struct vckss_cmg__graph scalar graph,
    real colvector component)
{
    struct vckss_cmg__level scalar out
...
struct vckss_cmg__level scalar vckss_cmg__level_prepare(
    struct vckss_cmg__graph scalar graph)
{
    struct vckss_cmg__level scalar out
    real colvector component

    component = vckss_cmg__components(graph)
    ...
    return(vckss_cmg__level_prepare_cert(graph,component))
}

struct vckss_cmg__level scalar vckss_cmg__coarse_prepare(
    struct vckss_cmg__level scalar level,
    struct vckss_cmg__options scalar options)
{
    ...
    for (one_component=1; one_component<=level.n_component;
        ...
        level.coarse_keep[one_component] =
            vckss_cmg__store_colvector(keep)
        level.coarse_scale[one_component] =
            vckss_cmg__store_colvector(scale)
        level.coarse_factor[one_component] =
            vckss_cmg__store_matrix(factor)
...
real matrix vckss_cmg__component_project(
    struct vckss_cmg__level scalar level,
    real matrix argument)
{
...
real matrix vckss_cmg__coarse_apply(
    struct vckss_cmg__level scalar level,
    real matrix right_hand_side)
{
    ...
    compatible = vckss_cmg__component_project(level,right_hand_side)
    ...
    out = vckss_cmg__component_project(level,out)
...
real scalar vckss_cmg__options_valid(
    struct vckss_cmg__options scalar options)
{
...
struct vckss_cmg__hierarchy scalar vckss_cmg__attempt_begin(
...
struct vckss_cmg__hierarchy scalar vckss_cmg__attempt_update(
    struct vckss_cmg__hierarchy scalar hierarchy,
    real scalar components,
    real scalar coarse_vertices,
...
struct vckss_cmg__hierarchy scalar vckss_cmg__hierarchy_mode(
    struct vckss_cmg__graph scalar graph,
    struct vckss_cmg__options scalar options,
    string scalar algorithm)
{
    ...
    struct vckss_cmg__level scalar level
    struct vckss_cmg__forest scalar forest
    struct vckss_cmg__aggregation_result scalar aggregation_result
    struct vckss_cmg__graph scalar current, coarse
    ...
    out.options = options
    if (graph.status != "CONVERGED" | !vckss_cmg__options_valid(options) |
    ...
    while (1) {
        ...
        out = vckss_cmg__attempt_begin(out,current,
        ...
        if (missing(predicted_level_bytes) |
            ...
            out = vckss_cmg__attempt_update(out,.,.,.,0,"NONE",
                out.status,out.message)
            return(out)
        }
        if (out.n_level == 0) {
            level = vckss_cmg__level_prepare(current)
        }
        else {
            ...
            level = vckss_cmg__level_prepare_cert(
        ...
        if (out.n_level == 0) initial_components = level.n_component
            ...
            out = vckss_cmg__attempt_update(out,level.n_component,.,.,0,
        ...
        component_size = level.component_panel[.,2]-
            level.component_panel[.,1]:+1
        if (max(component_size) <= options.coarse_max) {
            level = vckss_cmg__coarse_prepare(level,options)
            if (level.status != "CONVERGED") {
                out.status = level.status
                out.message = level.message
                out = vckss_cmg__attempt_update(out,level.n_component,
                    current.n_vertex,0,1,"DENSE_TERMINAL",
                    out.status,out.message)
                return(out)
            }
            out.level = out.level,vckss_cmg__store_level(level)
        ...
        if (algorithm == "STEINER_MATA" &
            ...
            aggregation_result = vckss_cmg__aggregate_gpl(
            ...
            forest = vckss_cmg__forest_select(
                current,level.component,options)
            ...
            aggregation_result = vckss_cmg__aggregate_vertices(
        ...
        reduction = vckss_cmg__component_reduce(current.n_vertex,
            aggregation_result.n_coarse,level.n_component)
        if (missing(reduction) | reduction < options.min_reduction) {
            aggregation_result = vckss_cmg__aggregate_fallback(
                current,level.component,options)
            if (aggregation_result.status != "CONVERGED") {
                out.status = aggregation_result.status
                out.message = aggregation_result.message
        ...
        coarse = vckss_cmg__contract_graph(current,level.aggregation,
            options.construction_scratch_bytes)
...
struct vckss_cmg__hierarchy scalar vckss_cmg__hierarchy_build(
    struct vckss_cmg__graph scalar graph,
    struct vckss_cmg__options scalar options)
{
    return(vckss_cmg__hierarchy_mode(
        graph,options,"STEINER_MATA"))
}

struct vckss_cmg__hierarchy scalar vckss_cmg__hierarchy_v5(
    struct vckss_cmg__graph scalar graph,
    struct vckss_cmg__options scalar options)
{
    return(vckss_cmg__hierarchy_mode(
...
struct vckss_cmg__workspace scalar vckss_cmg__workspace_init(
    struct vckss_cmg__hierarchy scalar hierarchy,
    real scalar batch_capacity,
    real scalar memory_cap_bytes)
{
    ...
    out = vckss_cmg__empty_workspace()
    ...
    for (level_index=1; level_index<=hierarchy.n_level; level_index++) {
        level = *hierarchy.level[level_index]
        out.compatible[level_index] = vckss_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.iterate[level_index] = vckss_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.work[level_index] = vckss_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
    }
    out.status = "CONVERGED"
...
struct vckss_cmg__diag_result scalar vckss_cmg__diagnostics(
    struct vckss_cmg__hierarchy scalar hierarchy)
{
    struct vckss_cmg__diag_result scalar out
    struct vckss_cmg__level scalar level
    ...
    out = vckss_cmg__empty_diagnostics()
...
real matrix vckss_cmg__apply_level(
    struct vckss_cmg__hierarchy scalar hierarchy,
    real scalar level_index,
    real matrix right_hand_side)
{
    ...
    if (level_index == hierarchy.n_level) {
        return(vckss_cmg__coarse_apply(level,compatible))
    }
    out = vckss_cmg__smooth(level,compatible,hierarchy.options.omega)
    if (rows(out) != level.graph.n_vertex) return(J(0,0,.))
    action = vckss_cmg__graph_action(
    ...
    correction = vckss_cmg__apply_level(
...
struct vckss_cmg__apply_result scalar vckss_cmg__apply(
    struct vckss_cmg__hierarchy scalar hierarchy,
    real matrix right_hand_side)
{
    struct vckss_cmg__apply_result scalar out
    struct vckss_cmg__level scalar fine
    real colvector component_size
    real scalar one_component, vertex

    out = vckss_cmg__empty_apply_result()
    if (hierarchy.status != "CONVERGED" | hierarchy.n_level < 1 |
    ...
    out.value = vckss_cmg__apply_level(hierarchy,1,right_hand_side)
...
real scalar vckss_cmg__project_self(
    struct vckss_cmg__level scalar level,
...
real scalar vckss_cmg__smooth_into(
    struct vckss_cmg__level scalar level,
    real matrix argument,
    real scalar column_count,
    real scalar omega,
...
real scalar vckss_cmg__graph_into(
    struct vckss_cmg__graph scalar graph,
    real matrix argument,
    real scalar column_count,
    real scalar scratch_bytes,
...
real scalar vckss_cmg__coarse_into(
    struct vckss_cmg__level scalar level,
    real matrix right_hand_side,
    real scalar column_count,
    real matrix out)
{
    ...
    return(vckss_cmg__project_self(level,column_count,out))
}

real scalar vckss_cmg__apply_ws_level(
    struct vckss_cmg__hierarchy scalar hierarchy,
    struct vckss_cmg__workspace scalar workspace,
    real scalar level_index,
    real matrix right_hand_side)
{
    struct vckss_cmg__level scalar level
    ...
    if (level_index == hierarchy.n_level) {
        return(vckss_cmg__coarse_into(
            level,*compatible,column_count,*iterate))
    }

    if (!vckss_cmg__smooth_into(level,*compatible,column_count,
        hierarchy.options.omega,*iterate)) return(0)
    if (!vckss_cmg__graph_into(level.graph,*iterate,column_count,
    ...
    if (!vckss_cmg__graph_into(level.graph,*iterate,column_count,
        hierarchy.options.action_scratch_bytes,*work)) return(0)
    (*work)[.,used] = (*compatible)[.,used]-(*work)[.,used]
    if (!vckss_cmg__smooth_into(level,*work,column_count,
        hierarchy.options.omega,*compatible)) return(0)
    (*iterate)[.,used] = (*iterate)[.,used]+(*compatible)[.,used]
    return(vckss_cmg__project_self(level,column_count,*iterate))
}

struct vckss_cmg__apply_result scalar vckss_cmg__workspace_apply(
    struct vckss_cmg__hierarchy scalar hierarchy,
    struct vckss_cmg__workspace scalar workspace,
    real matrix right_hand_side)
{
...
real matrix vckss_cmg__project_labels(
...
struct vckss_cmg__apply_result scalar vckss_cmg__apply_kss(
    struct vckss_cmg__hierarchy scalar hierarchy,
    real matrix firm_rhs)
{
    struct vckss_cmg__apply_result scalar out, core
    struct vckss_cmg__level scalar fine
    ...
    compatible = vckss_cmg__project_labels(firm_component,firm_rhs)
    ...
    core = vckss_cmg__apply(hierarchy,injected)
    if (core.status != "CONVERGED") return(core)
    out = core
    out.value = vckss_cmg__project_labels(
...
struct vckss_cmg__cells scalar vckss_cmg__cells_prepare(
    real colvector worker,
    real colvector firm,
    real colvector weight,
    real colvector worker_key,
    real colvector firm_key)
{
    ...
    if (!vckss_cmg__dense_ids(worker) | !vckss_cmg__dense_ids(firm)) {
    ...
    pair_panel = vckss_cmg__lex_panel(pair_values,pair_order)
...
struct vckss_cmg__graph scalar vckss_cmg__hybrid_build(
    struct vckss_cmg__cells scalar cells)
{
    struct vckss_cmg__graph scalar out
...
real matrix vckss_cmg__dense_schur(struct vckss_cmg__cells scalar cells)
{
    real matrix out
...
real matrix vckss_cmg__dense_laplacian(struct vckss_cmg__graph scalar graph)
{
    real matrix out
...
real matrix vckss_cmg__dense_hybrid_schur(
    struct vckss_cmg__graph scalar graph)
{
    ...
    if (graph.status != "CONVERGED") return(J(0,0,.))
    lap = vckss_cmg__dense_laplacian(graph)
