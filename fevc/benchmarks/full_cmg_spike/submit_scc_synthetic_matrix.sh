...
temporary=$(mktemp -d /private/tmp/fevc-full-cmg-synthetic-scc.XXXXXX)
...
job_id=$(ssh scc qsub -terse -P welfgr -pe omp 14 \
  ...
  -N fevc-symx -j y -o "$remote/logs/job.txt" -m a \
  ...
  "$remote/source/candidate/fevc/benchmarks/full_cmg_spike/run_scc_synthetic_matrix.sge")
