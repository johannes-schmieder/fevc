...
temporary=$(mktemp -d /private/tmp/fevc-full-cmg-scc.XXXXXX)
...
job_id=$(ssh scc qsub -terse -P welfgr -pe omp 4 \
  ...
  -N fevc-fcmg4 -j y -o "$remote/logs" -m a \
  ...
  "$remote/source/candidate/fevc/benchmarks/full_cmg_spike/run_scc_smoke.sge")
