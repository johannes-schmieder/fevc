#!/usr/bin/env bash
# Deploy the exact-source fixed-CZ18 P200 alternating warm matrix.

set -euo pipefail

run_id=${1:?usage: submit_scc_cz18_matrix.sh RUN_ID}
probes=200
...
temporary=$(mktemp -d /private/tmp/fevc-full-cmg-cz18-scc.XXXXXX)
...
{
  printf 'schema=VCKSS_FULL_CMG_CZ18_MATRIX_TASK_V1\n'
...
job_id=$(ssh scc qsub -terse -P welfgr -pe omp 14 \
  ...
  -N "fevc-czmx" -j y -o "$remote/logs/job.txt" -m a \
  ...
  "$remote/source/candidate/fevc/benchmarks/full_cmg_spike/run_scc_cz18_matrix.sge")
printf '%s\n' "$job_id" | ssh scc "cat > '$remote/submissions/job_id'"
printf 'VCKSS_FULL_CMG_CZ18_MATRIX_SUBMITTED job_id=%s run_dir=%s source_commit=%s task_sha256=%s probes=%s fast_preparation=%s raw_match=%s\n' \
