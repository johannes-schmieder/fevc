    ...
    while process.poll() is None:
        if time.monotonic() - started > wall_seconds:
            ...
            try:
                ...
                process.wait(timeout=15)
            failure = {
                "schema": "FEVC-FULL-CMG-SPIKE-MACOS-PROCESS-TREE-V1",
                "status": "FAIL_TIMEOUT",
...
def validate_stata(row: dict[str, str], role: str, commit: str, task_sha: str,
                   input_sha: str) -> None:
    require(row["schema"] == "FEVC-FULL-CMG-SPIKE-STATA-V1", "Stata schema changed")
...
def main() -> int:
    ...
    for role, plugin in (
        ...
        shutil.copy2(plugin, roots[role] / "fevc/vckss_rust_macos_arm64.plugin")
    ...
    subprocess.run(
        [
            "python3", str(repo / "fevc/benchmarks/scc/verify_numopt2_matlab_source.py"),
    ...
    task_payload = {
        "schema": "FEVC-FULL-CMG-SPIKE-LOCAL-CASE-V2",
    ...
    summary = {
        "schema": "FEVC-FULL-CMG-SPIKE-LOCAL-V1",
        "status": "PASS",
