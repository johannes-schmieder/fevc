...
```bash
python3 fevc/benchmarks/full_cmg_spike/run_local.py \
  --output-dir /private/tmp/fevc-full-cmg-local \
  --baseline 4124b34f3ca216dcc3aae27e4b31bbac9e011f11 \
  --baseline-plugin /private/tmp/fevc-baseline-4124/plugin/vckss_rust_macos_arm64.plugin \
  --candidate HEAD \
  --candidate-plugin /private/tmp/fevc-full-cmg-candidate/candidate/vckss_rust_macos_arm64.plugin \
  --candidate-build-receipt /private/tmp/fevc-full-cmg-candidate/build-receipt.txt \
...
job under `/projectnb/welfgr/fevc/runs/`. It does not use an array or copy a
...
VCKSS_CMG_ROOT="$GIT_HOME/CMG" \
  fevc/benchmarks/full_cmg_spike/submit_scc_smoke.sh \
