...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fevc-prep-rhs1-") as tmp:
        temporary = Path(tmp)
        roots = {role: temporary / role for role in commits}
        for role, commit in commits.items():
            roots[role].mkdir()
            _archive(repo, commit, roots[role])
        driver = roots["candidate"] / "fevc/benchmarks/numopt2_local.do"
        baseline_driver = roots["baseline"] / "fevc/benchmarks/numopt2_local.do"
    ...
    summary = {
        "schema": "fevc-prep-rhs1-local-v1",
        "status": "PASS",
