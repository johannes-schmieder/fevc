...
args source_root input_csv output_csv source_commit task_sha input_sha firms_arg probes_arg seed_arg
...
if !ustrregexm("`source_commit'","^[0-9a-f]{40}$") | ///
   ...
   !inlist(`firms',64,256,1024) |                       ///
   !inlist(`probes',20,200) | missing(`seed') | `seed'<1 {
    di as error "invalid PREP-BND-1 Stata arguments"
    exit 198
}
...
quietly fevc y, worker(worker) firm(firm) deletion(match)       ///
    ...
    preconditioner(auto) memory_gib(16) wallseconds(7200)             ///
...
clear
set obs 1
generate str32 schema = "PREP-BND-1-STATA-AGGREGATE-V1"
...
generate int firms = `firms'
generate long workers = `workers'
generate long rows = `rows'
generate byte cells_per_worker = 3
...
di as result "PREP_BND1_MATLAB_STATA_PASS `source_commit' `task_sha'"
exit 0
