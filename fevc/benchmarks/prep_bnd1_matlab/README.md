...
./.venv/bin/python -m pytest -q \
  fevc/benchmarks/prep_bnd1_matlab/tests
./.venv/bin/python -m ruff check \
  fevc/benchmarks/prep_bnd1_matlab
```
...
/Applications/MATLAB_R2024b.app/bin/matlab -batch \
  "addpath('fevc/benchmarks/oracle'); vckss_dense_oracle('$oracle_dir','$candidate')"
/Applications/Stata/StataMP.app/Contents/MacOS/stata-mp -q do \
  fevc/benchmarks/oracle/stata_oracle.do "$oracle_dir" "$candidate"
./.venv/bin/python \
...
bundle=$(./.venv/bin/python \
  fevc/benchmarks/prep_bnd1_matlab/build_bundle.py \
...
./.venv/bin/python \
  fevc/benchmarks/prep_bnd1_matlab/build_manifest.py \
...
```text
/projectnb/welfgr/fevc/bundles/<bundle>.tar.gz
/projectnb/welfgr/fevc/bundles/<bundle>.files.sha256
/projectnb/welfgr/fevc/bundles/<bundle>/source/
/projectnb/welfgr/fevc/prep-bnd1-matlab/runs/<run-id>/input/tasks/
```
...
```text
PBM_ROOT=/projectnb/welfgr/fevc/prep-bnd1-matlab
PBM_RUN_ID=<unique-run-id>
PBM_SOURCE_DIR=/projectnb/welfgr/fevc/bundles/<bundle>/source
PBM_SOURCE_COMMIT=<candidate-full-sha>
PBM_BUNDLE_SHA256=<bundle>
PBM_BUNDLE_MANIFEST=/projectnb/welfgr/fevc/bundles/<bundle>.files.sha256
...
```bash
./.venv/bin/python \
  fevc/benchmarks/prep_bnd1_matlab/validate_scc_job.py \
...
```bash
./.venv/bin/python fevc/benchmarks/prep_bnd1_matlab/analyze.py \
...
pbm_root=/projectnb/welfgr/fevc/prep-bnd1-matlab
source_dir=/projectnb/welfgr/fevc/bundles/$bundle/source
bundle_manifest=/projectnb/welfgr/fevc/bundles/$bundle.files.sha256
run_root=$pbm_root/runs/$run_id

test -d "$source_dir"
test "$(tr -d '[:space:]' < "$source_dir/SOURCE_COMMIT.txt")" = "$candidate"
test "$(sha256sum "/projectnb/welfgr/fevc/bundles/$bundle.tar.gz" | \
  awk '{print $1}')" = "$bundle"
...
job_id=$(qsub -terse -P welfgr -pe omp 4 \
  ...
  "$source_dir/fevc/benchmarks/prep_bnd1_matlab/run_dense_oracle.sge")
...
python3 \
  "$source_dir/fevc/benchmarks/prep_bnd1_matlab/validate_dense_oracle_scc.py" \
