...
def monitor_matlab(
    ...
    receipt = {
        "schema": "FEVC-FULL-CMG-PRODUCTION-MATLAB-PROCESS-V1",
        "status": "PASS",
...
def validate_vckss(row: dict[str, str], source_commit: str, task_sha: str) -> None:
    require(row["schema"] == "FEVC-FULL-CMG-PRODUCTION-STATA-V1", "Stata schema changed")
...
def main() -> int:
    ...
    source_contract = repo / "fevc/benchmarks/matlab_scale/source_contract.json"
    subprocess.run(
        [
            os.sys.executable,
            str(repo / "fevc/benchmarks/scc/verify_numopt2_matlab_source.py"),
            "--matlab-root",
            str(matlab_runtime),
            "--contract",
            str(source_contract),
    ...
    task = {
        "schema": "FEVC-FULL-CMG-PRODUCTION-LOCAL-CASE-V1",
        "source_commit": source_commit,
    ...
    rows: dict[str, list[dict[str, object]]] = {"fevc": [], "matlab": []}
    evidence: list[dict[str, object]] = []
    driver_dir = Path(__file__).resolve().parent
    matlab_driver = repo / "fevc/benchmarks/paper_matlab_scaling"
    ...
    production_over_matlab = medians["fevc"] / medians["matlab"]
    production_over_private = medians["fevc"] / PRIVATE_WINNER_SECONDS
    promotion_gates = {
    ...
    summary = {
        "schema": "FEVC-FULL-CMG-PRODUCTION-LOCAL-V1",
