...
HARNESS = ROOT / "fevc" / "benchmarks" / "full_cmg_production"
...
def test_runner_is_source_bound_and_position_balanced() -> None:
    ...
    for token in (
        ...
        assert token in source
    assert source.count('(\"fevc\", \"matlab\")') == 3
    assert source.count('(\"matlab\", \"fevc\")') == 3
...
def test_scc_production_scripts_are_normal_build_and_r2024b_bound() -> None:
    ...
    common = (ROOT / "fevc" / "benchmarks" / "matlab_scale" / "common.py").read_text(
        encoding="utf-8"
    )
    ast.parse(validator)
    ...
    assert "/projectnb/welfgr/fevc/runs/" in submit
