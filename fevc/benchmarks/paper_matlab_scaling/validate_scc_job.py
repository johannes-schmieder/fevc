#!/usr/bin/env python3
"""Validate one same-host paper Stata/MATLAB scaling job."""
...
def validate(
    ...
    require(input_receipt["schema"] == "PAPER-MATLAB-SCALING-INPUT-V1" and
            input_receipt["structure"] == task["structure"] and
            input_receipt["connectivity"] == task["connectivity"] and
    ...
    require(stata["schema"] == "PAPER-MATLAB-SCALING-STATA-V1" and
    ...
    for field in ("structure", "connectivity"):
        require(stata[field] == task[field], f"Stata task changed: {field}")
    ...
    require(finite(stata["resource_peak_bytes"], "Stata direct peak") <= 48 * GIB,
    ...
    require(matlab.get("schema") == "PAPER-MATLAB-SCALING-MATLAB-V1" and
    ...
    for field in ("structure", "connectivity"):
        require(matlab[field] == task[field], f"MATLAB task changed: {field}")
    for field in ("rows", "workers", "firms", "cells_per_worker", "probes", "seed"):
    ...
    require(matlab["same_literal_match_rows"] is True and
    ...
    accounting = parse_qacct(qacct_path)
    require(node["schema"] == "PAPER-MATLAB-SCALING-NODE-V1" and
    ...
    require((job_dir / "pair.pass").read_text().strip() ==
            f"PAPER_MATLAB_SCALING_PAIR_PASS {task['experiment_id']} {task_sha}" and
    ...
    return {
        ...
        "cells_per_worker": integer(task["cells_per_worker"], "degree"),
        ...
        "stata_over_matlab_command_ratio": (
            finite(stata["command_seconds"], "Stata command") /
...
def main() -> int:
    ...
    args.output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n",
                           encoding="utf-8")
    print(f"PAPER_MATLAB_SCALING_VALIDATION_PASS "
