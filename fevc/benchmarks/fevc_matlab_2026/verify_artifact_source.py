...
ARTIFACT_SOURCE_SCHEMA = "FEVC-MATLAB-2026-ARTIFACT-SOURCE-V1"
PREPARATION_SCHEMA = "FEVC-MATLAB-2026-PREPARATION-V1"
...
def safe_replacement_path(path: str) -> bool:
    ...
    protected_build_inputs = {
        "fevc/benchmarks/fevc_matlab_2026/build_benchmark_ado.py",
        "fevc/benchmarks/fevc_matlab_2026/prepare_artifacts.sge",
        "fevc/benchmarks/fevc_matlab_2026/prepare_matlab_mex.m",
    }
    return (
        path not in protected_build_inputs
        and (
            path == "SOURCE_COMMIT.txt"
            or path == "fevc/PLAN.md"
            or path ==
            "fevc/benchmarks/cmg_candidate_qualification/README.md"
            or path.startswith(
                "fevc/benchmarks/cmg_candidate_qualification/evidence/")
            or path.startswith("fevc/benchmarks/fevc_matlab_2026/")
        )
    )
...
def main() -> int:
    ...
    args.output.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n",
                           encoding="utf-8")
    print("FEVC_MATLAB_2026_ARTIFACT_SOURCE_PASS "
