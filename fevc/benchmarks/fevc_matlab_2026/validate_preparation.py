...
PREPARATION_QACCT_SCHEMA = "FEVC-MATLAB-2026-PREPARATION-QACCT-V1"
...
def validate(run_dir: Path) -> dict[str, Any]:
    ...
    require(preparation.get("schema") ==
            "FEVC-MATLAB-2026-PREPARATION-V1" and
            ...
            identity.get("source_manifest_sha256") and
            preparation.get("matlab_module") == "matlab/2026a",
    ...
    adapter_source = (run_dir / "source" / "fevc" / "benchmarks" /
                      "fevc_matlab_2026" / "build_benchmark_ado.py")
    adapter = load_json(adapter_path)
    require(required_rust == 28 and
            preparation.get("required_rust_threads") == "28" and
            preparation.get("benchmark_thread_contract") ==
            "FEVC-BENCHMARK-THREADS-V1" and
            adapter.get("schema") == "FEVC-BENCHMARK-ADO-ADAPTER-V1" and
            adapter.get("status") == "PASS" and
            adapter.get("thread_contract") == "FEVC-BENCHMARK-THREADS-V1" and
            adapter.get("maximum_stata_processors") == 4 and
            adapter.get("allowed_native_threads") == [1, 2, 4, 8, 14, 28] and
    ...
    require((receipt_dir / "wrapper.pass").read_text(encoding="utf-8").strip() ==
            f"FEVC_MATLAB_2026_PREPARE_PASS {identity['source_commit']} "
    ...
    require(effective.get("schema") == "FEVC-SGE-EFFECTIVE-SUBMISSION-V1" and
...
def main() -> int:
    ...
    args.output.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n",
                           encoding="utf-8")
    print(f"FEVC_MATLAB_2026_PREPARATION_QACCT_PASS {value['job_id']}")
    return 0
