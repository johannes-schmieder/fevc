...
ssh scc "set -euo pipefail; cd '$remote/input'; sha256sum -c source.tar.gz.sha256; sha256sum -c stata-spi.sha256; cd '$remote'; test ! -e source; tar -xzf input/source.tar.gz; printf '%s\\n' '$source_commit' > source/SOURCE_COMMIT.txt; cd source; sha256sum -c '$remote/input/source.files.sha256' > '$remote/receipts/deployment_source_manifest.check'; printf 'FEVC_MATLAB_2026_DEPLOY_PASS %s %s\\n' '$source_commit' '$bundle_sha' > '$remote/receipts/deployment.pass'"
printf 'FEVC MATLAB 2026 SCC DEPLOY PASS: %s\n' "$remote"
