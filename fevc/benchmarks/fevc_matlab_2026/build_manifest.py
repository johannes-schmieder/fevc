#!/usr/bin/env python3
"""Build the frozen 240-cell FEVC--MATLAB R2026a main manifest."""
...
def build_rows(
    ...
    *,
    ...
    task_id = 0
    registered_cells = [*( (rows, 28) for rows in ROW_GRID ),
                        *( (491_520, cores) for cores in CORE_GRID if cores != 28 )]
    require(len(registered_cells) == 10 and len(set(registered_cells)) == 10,
            "main row/core slices changed")
    for structure, (connectivity, degree, _) in STRUCTURES.items():
        for replicate, seed, order in REPLICATES:
            for rows, cores in registered_cells:
                workers = rows // degree
                ...
                result.append({
                        ...
                        "sample_contract": "same_literal_match_rows_v3",
                        "target_contract": "uniform_stored_rows_v1",
                        "comparison_contract": (
                            "paired_same_host_rust_matlab_time_absolute_memory_v1"
                        ),
                    })
    require(task_id == 240, "main matrix must contain 240 cells")
    return result
...
def main() -> int:
    ...
    print(
        "FEVC_MATLAB_2026_MAIN_MANIFEST_PASS "
        f"cells=240 sha256={sha256(args.output)}"
    )
    return 0
