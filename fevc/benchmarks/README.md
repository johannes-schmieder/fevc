...
The original KB6 drivers qualify the internal `fevc` point-estimation
...
```text
/projectnb/welfgr/fevc/runs/<run-id>/
```
...
```bash
bash fevc/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/fevc/runs/<run-id> "$PWD" <commit> portability

bash fevc/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/fevc/runs/<run-id> "$PWD" <commit> oracle

bash fevc/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/fevc/runs/<run-id> "$PWD" <commit> \
...
```bash
bash fevc/benchmarks/scc/collect_qacct.sh \
  /projectnb/welfgr/fevc/runs/<run-id> smoke
```
...
```bash
./.venv/bin/python fevc/benchmarks/validate_scc.py \
...
`/projectnb/welfgr/fevc/bundles/<sha256>/`. Every job in a run verifies that
...
```bash
./.venv/bin/python fevc/benchmarks/build_prod_bundle.py \
  --root "$PWD" \
  --allowlist fevc/benchmarks/prod_bundle_allowlist.txt \
  --source-commit "$(git rev-parse HEAD)" \
  --check-only
./.venv/bin/python fevc/benchmarks/validate_prod_scc.py --static
bash -n fevc/benchmarks/scc/*.sh fevc/benchmarks/scc/*.sge
```
...
```bash
cp fevc/benchmarks/prod_data_manifest.example.tsv \
...
rsync -av /private/tmp/kss-prod-data-manifest.tsv \
  scc:/projectnb/welfgr/fevc/manifests/<run-id>.tsv
bash fevc/benchmarks/scc/deploy_prod_bundle.sh "$PWD" <run-id>

ssh scc bash /projectnb/welfgr/fevc/bundles/<bundle-sha>/source/fevc/benchmarks/scc/submit_prod_dag.sh \
  /projectnb/welfgr/fevc/runs/<run-id> \
  /projectnb/welfgr/fevc/bundles/<bundle-sha> <bundle-sha> \
  /projectnb/welfgr/fevc/manifests/<run-id>.tsv preflight
```
...
python3 <bundle>/source/fevc/benchmarks/validate_prod_scc.py \
  --run-dir /projectnb/welfgr/fevc/runs/<run-id> \
  --bundle-sha <bundle-sha256> --source-commit <commit> \
  --data-manifest /projectnb/welfgr/fevc/runs/<run-id>/input/data_manifest.tsv \
...
```bash
bash <bundle>/source/fevc/benchmarks/scc/submit_matlab_phase_profile.sh \
...
bash <bundle>/source/fevc/benchmarks/scc/collect_matlab_phase_profile.sh \
  <run-dir> <label>

python3 <bundle>/source/fevc/benchmarks/validate_matlab_phase_profile.py \
