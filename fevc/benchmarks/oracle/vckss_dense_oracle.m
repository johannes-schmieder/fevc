...
if nargin ~= 2 || strlength(string(source_commit)) < 7
    error('fevc:invalidInput','output_dir and source_commit are required');
end
...
if max(abs(plugin-expected_plugin)) > 2e-10 || ...
    ...
    error('fevc:oracleMismatch','dense MATLAB oracle missed registered values');
end
fprintf('FEVC MATLAB ORACLE PASS: %s\n',source_commit);
end
