...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fevc-prep-bnd1-") as temporary_name:
        temporary = Path(temporary_name)
        roots = {role: temporary / role for role in ROLES}
        for role in ROLES:
            roots[role].mkdir()
            archive(repo, commits[role], roots[role])
        driver = repo / "fevc/benchmarks/prep_bnd1/local_driver.do"
        require(driver.is_file(), "tracked benchmark driver is missing")
        git(
            repo,
            "ls-files",
            "--error-unmatch",
            "fevc/benchmarks/prep_bnd1/local_driver.do",
        )
    ...
    summary = {
        "schema": "fevc-prep-bnd1-local-v1",
        "status": "PASS",
