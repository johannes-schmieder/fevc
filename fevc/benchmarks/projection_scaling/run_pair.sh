...
harness=$source_dir/fevc/benchmarks/projection_scaling
...
scratch=$TMPDIR/fevc-projection-$JOB_ID-${SGE_TASK_ID:-gate}-$rows-$replicate
...
run_stata() {
  ...
  grep -F "FEVC PROJECTION SCALING STATA PASS $role rows=$rows" \
...
run_matlab() {
  ...
  python3 -c 'import sys; print(float(sys.argv[2])-float(sys.argv[1]))' \
    "$started" "$finished" > "$role_dir/whole_wall_seconds.txt"
  test "$app_rc" = 0 && test "$monitor_rc" = 0
  grep -F "FEVC PROJECTION SCALING MATLAB PASS rows=$rows" \
    "$role_dir/application.txt"
...
grep -F 'FEVC PROJECTION SCALING STATA BASELINE PASS' \
...
printf 'FEVC PROJECTION SCALING PAIR PASS rows=%s replicate=%s order=%s\n' \
