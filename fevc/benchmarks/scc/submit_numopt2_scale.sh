...
test -f "$source_dir/fevc/benchmarks/scc/run_numopt2_scale.sge"
test -f "$source_dir/fevc/benchmarks/scc/numopt2_generate.do"
...
qsub_args=(-P welfgr -pe omp "$slots" -l "mem_per_core=${mem_per_core}G" \
  ...
  -v "$environment" "$source_dir/fevc/benchmarks/scc/run_numopt2_scale.sge")
