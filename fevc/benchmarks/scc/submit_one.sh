...
case "$run_dir" in
  ...
  *) printf '%s\n' "RUN_DIR must be under /projectnb/welfgr/fevc/runs/" >&2; exit 198 ;;
esac
case "$source_dir" in
  "$run_dir"/source) ;;
...
test -f "$source_dir/fevc/fevc.ado"
...
case "$job" in
  ...
  smoke|medium|large)
    ...
    script="$source_dir/fevc/benchmarks/scc/run_scale.sge"
    case "$job" in
