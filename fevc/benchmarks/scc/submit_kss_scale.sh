...
case "$source_dir" in
  /projectnb/welfgr/fevc/*) ;;
  *) printf '%s\n' "invalid source directory" >&2; exit 198 ;;
esac
case "$bundle_archive" in
  /projectnb/welfgr/fevc/bundles/*.tar.gz) ;;
...
test -f "$source_dir/fevc/benchmarks/scc/kss_scale_driver.do"
test -f "$source_dir/fevc/benchmarks/scc/run_kss_scale.sge"
...
qsub_args=(
  ...
  -v "$environment"
  "$source_dir/fevc/benchmarks/scc/run_kss_scale.sge"
)
