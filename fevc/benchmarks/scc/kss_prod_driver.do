...
if inlist("`stage'", "install_auto", "install_cmg") {
    ...
    net install fevc, from(`"`c(pwd)'/fevc"') replace
    discard
    mata: mata clear
...
if "`temperature'" == "warm" {
    capture noisily fevc y_minus_xb, worker(worker) firm(firm) `command_options'
    if _rc {
...
forvalues repetition = 1/`repetitions' {
    // Mata's profiling service owns Stata's timer registry while fevc is
