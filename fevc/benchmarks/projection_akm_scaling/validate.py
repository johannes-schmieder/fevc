...
def validate_rust_result(
    ...
    if rust.get("schema") != "FEVC-PROJECTION-AKM-STATA-V2" or rust.get("status") != "PASS":
...
def validate_matlab_nonconvergence(
    ...
    if (
        failure.get("schema") != "FEVC-PROJECTION-AKM-MATLAB-FAILURE-V1"
        or failure.get("status") != "FAIL"
        or failure.get("identifier") != "fevc:projectionAkm:Fit"
...
def validate_role(
    ...
    if status.get("schema") != "FEVC-PROJECTION-AKM-ROLE-STATUS-V2":
...
def validate_pair(
    ...
    *,
    ...
    if receipt.get("schema") != "FEVC-PROJECTION-AKM-PAIR-V2" or receipt.get("status") != "CAPTURED":
    ...
    if outcomes["rust"] != "PASS":
        ...
        pair_status = "FAIL"
    result: dict[str, Any] = {
        "schema": "FEVC-PROJECTION-AKM-VALIDATION-V2",
        "status": pair_status,
