...
def load_captured_task(
    ...
    for public_role, directory_role in (("fevc", "rust"), ("matlab", "matlab")):
        ...
        cell.update({
            ...
            "stata_baseline_rss_kib": baseline if public_role == "fevc" else "",
            "incremental_rss_kib": int(tree["whole_peak_rss_kib"]) - baseline if public_role == "fevc" else "",
        ...
        if status["outcome"] == "PASS" and public_role == "fevc":
            ...
            for role in ROLES:
                ...
                if len(passed) == 3:
                    value.update({
                        ...
                        "median_incremental_rss_kib": statistics.median(item["incremental_rss_kib"] for item in passed) if role == "fevc" else None,
                    })
                values.append(value)
            fevc = values[-2]
            matlab = values[-1]
            if fevc["rankable"] and matlab["rankable"]:
                values.append({
                    "rows": rows,
                    "cores": cores,
                    ...
                    "rankable": True,
                    "median_command_seconds": matlab["median_command_seconds"] / fevc["median_command_seconds"],
                    "median_whole_wall_seconds": matlab["median_whole_wall_seconds"] / fevc["median_whole_wall_seconds"],
                    "median_peak_rss_kib": None,
                    "median_incremental_rss_kib": None,
                })
    return values
...
def main() -> None:
    ...
    result = {
        "schema": "FEVC-PROJECTION-AKM-SUMMARY-V2",
        "status": "COMPLETE",
        "source_commit": source,
