...
try
    ...
    identity = struct('schema','FEVC-PROJECTION-AKM-MATLAB-PROCESS-V1', ...
    ...
    record = struct('schema','FEVC-PROJECTION-AKM-MATLAB-V1', ...
    ...
    write_marker(fullfile(output_dir,'application.pass'), ...
        sprintf('FEVC PROJECTION AKM MATLAB PASS rows=%d cores=%d', ...
        rows_expected,active_cores));
    fprintf('FEVC PROJECTION AKM MATLAB PASS rows=%d cores=%d\n', ...
        rows_expected,active_cores);
catch exception
    ...
    failure = struct('schema','FEVC-PROJECTION-AKM-MATLAB-FAILURE-V1', ...
...
if ~condition
    error(['fevc:projectionAkm:' code],'%s',message);
end
end
