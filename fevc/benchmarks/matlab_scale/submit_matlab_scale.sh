...
case "$source_dir" in
  "/projectnb/welfgr/fevc/bundles/$bundle_sha/source") ;;
  *) exit 198 ;;
esac
case "$bundle_archive" in
  "/projectnb/welfgr/fevc/bundles/$bundle_sha/$bundle_sha.tar.gz") ;;
  *) exit 198 ;;
esac
case "$source_manifest" in
  "/projectnb/welfgr/fevc/bundles/$bundle_sha/$bundle_sha.files.sha256") ;;
  *) exit 198 ;;
esac
...
if [[ "$stage" == prepare ]]; then
  ...
  mem_per_core_gib=4
  stata_processors=4
  script="$source_dir/fevc/benchmarks/matlab_scale/run_prepare_fixed_sample.sge"
  ...
  contract_path=\
"$source_dir/fevc/benchmarks/matlab_scale/source_contract.json"
  ...
  test ! -e "$case_gate"
  "$python_bin" \
    "$source_dir/fevc/benchmarks/matlab_scale/verify_submission.py" \
  ...
  script="$source_dir/fevc/benchmarks/matlab_scale/run_matlab_scale.sge"
