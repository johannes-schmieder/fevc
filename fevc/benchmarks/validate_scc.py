...
def validate_portability(root: Path, commit: str) -> None:
    ...
    require("FEVC SCC PORTABILITY PASS" in wrapper_log, "portability wrapper marker absent")
    full_log = text(root / "portability" / "full_suite.application.txt")
    install_log = text(root / "portability" / "install.application.txt")
    require("FEVC TEST SUITE PASS: full" in full_log, "full Stata suite marker absent")
    require("FEVC INSTALL TEST PASS" in install_log, "install marker absent")
...
def validate_oracle(root: Path, commit: str) -> None:
    ...
    require("FEVC SCC ORACLE PASS" in wrapper_log, "oracle wrapper marker absent")
    require("FEVC MATLAB ORACLE PASS" in matlab_log, "MATLAB oracle marker absent")
    require("FEVC STATA/MATLAB ORACLE PASS" in stata_log, "paired oracle marker absent")
    require(
...
def validate_scale(root: Path, commit: str, scenario: str) -> None:
    ...
    require(f"FEVC SCC SCALE PASS: {scenario}" in wrapper_log, f"{scenario} wrapper marker absent")
    require(f"FEVC BENCHMARK PASS: {scenario}" in app_log, f"{scenario} application marker absent")
    require(
