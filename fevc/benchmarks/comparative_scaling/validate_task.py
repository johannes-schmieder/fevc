...
def validate_status(role_dir: Path, role: str, active_cores: int) -> dict[str, str]:
    ...
    require(value.get("schema") == "FEVC-COMPARATIVE-SCALING-ROLE-STATUS-V2",
...
def role_result(
    ...
    require(tree.get("schema") ==
            "FEVC-COMPARATIVE-SCALING-PROCESS-TREE-V1" and
    ...
    if role in {"mata", "rust"}:
        ...
        require(value.get("schema") == "FEVC-COMPARATIVE-SCALING-STATA-V3" and
        ...
        require(value.get("schema") == "FEVC-COMPARATIVE-SCALING-MATLAB-V1" and
...
def validate(job_dir: Path, qacct_path: Path) -> dict[str, Any]:
    ...
    require(input_receipt.get("schema") ==
            "FEVC-COMPARATIVE-SCALING-INPUT-V7", "input schema changed")
