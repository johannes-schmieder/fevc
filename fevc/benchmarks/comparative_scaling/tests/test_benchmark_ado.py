...
from fevc.benchmarks.comparative_scaling.build_benchmark_ado import (
    THREAD_CONTRACT,
    build,
)
...
def test_adapter_transforms_candidate_and_checkpoint_once(
    ...
    source = (REPO / "fevc" / "fevc.ado" if source_kind == "candidate"
...
def test_adapter_rejects_already_adapted_source(tmp_path: Path) -> None:
    ...
    build(REPO / "fevc" / "fevc.ado", first, tmp_path / "first.json")
...
def test_adapter_decouples_native_threads_in_stata(tmp_path: Path) -> None:
    ...
    assert "FEVC BENCHMARK ADAPTER STATA PASS" in result.stdout
