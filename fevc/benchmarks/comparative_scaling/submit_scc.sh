...
test -d "$matlab_root"
harness=$source_dir/fevc/benchmarks/comparative_scaling
attempt_id=$mode
task_ids=NONE
...
case "$mode" in
  ...
  production)
    ...
    "$python_bin" "$harness/verify_pilots.py" \
      ...
      --small "/projectnb/welfgr/fevc/runs/$pilot_small/receipts/pilot.pass.json" \
      --worst "/projectnb/welfgr/fevc/runs/$pilot_worst/receipts/pilot.pass.json" \
  ...
  replacement)
    ...
    "$python_bin" "$harness/authorize_replacement.py" \
      --new-run "$run_dir" \
      --old-run "/projectnb/welfgr/fevc/runs/$replaces_run_id" \
...
printf 'FEVC COMPARATIVE SCALING SUBMITTED: mode=%s job=%s range=%s\n' \
  "$mode" "$job_id" "$range"
