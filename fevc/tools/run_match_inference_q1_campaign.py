#!/usr/bin/env python3
"""Run the source-bound fixed-offset collapsed-match q=1 development pipeline."""
...
MANIFEST_SCHEMA = "fevc-match-inference-q1-campaign-manifest-v1"
PREFLIGHT_SCHEMA = "fevc-match-inference-q1-design-preflight-v1"
PREFLIGHT_RECEIPT_SCHEMA = "fevc-match-inference-q1-preflight-receipt-v1"
ROW_SCHEMA = "fevc-match-inference-q1-development-row-v1"
TASK_RECEIPT_SCHEMA = "fevc-match-inference-q1-task-receipt-v1"
BUILD_RECEIPT_SCHEMA = "fevc-match-inference-q1-scc-build-v1"
SUMMARY_SCHEMA = "fevc-match-inference-q1-summary-v1"
REGISTRATION_SCHEMA = "FEVC_MATCH_INFERENCE_Q1_CAMPAIGN_V1"
REGISTRATION_PATH = Path("fevc/docs/match_inference_q1_campaign_v1.json")
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("schema") != MANIFEST_SCHEMA or manifest.get("status") != "FROZEN_DEVELOPMENT_TASKS" or profile not in PROFILE_DEFAULTS:
...
def _validate_row(row: dict[str, Any], task: dict[str, Any], spec: dict[str, Any], critical_simulations: int) -> tuple[str, int, int, str]:
    ...
    if status == "success":
        ...
        if (
            ...
            or row["confidence_lower"] > row["confidence_upper"]
        ...
        if spec["reference_distribution"] == "q1":
            if not row["q1_diagnostics_applicable"] or row["critical_simulations"] != critical_simulations:
                raise CampaignError(f"{key}: q1 accounting mismatch")
            for field in Q1_NUMERIC_FIELDS:
                _finite(row.get(field), f"{key} {field}")
            determinant = row["leading_variance"] * row["remainder_variance"] - row["leading_remainder_covariance"] ** 2
            if row["leading_variance"] <= 0.0 or row["remainder_variance"] <= 0.0 or determinant <= 0.0 or row["critical_value"] <= 0.0:
...
def run_task(manifest_path: Path, task_id: int, output_dir: Path, binary: Path) -> dict[str, Any]:
    ...
    command = [
        str(binary), "q1-matrix-v1", task["cell"], str(task["k"]),
...
def _scientific_failures(summaries: Sequence[dict[str, Any]], profile: str) -> tuple[list[str], dict[str, Any]]:
    ...
    for row in summaries:
        ...
        elif row["gate"] in {"correct", "solver_diagnostic", "multi_mode_diagnostic", "conditioning_diagnostic"} and row["success_rate"] < THRESHOLDS["correct_success_rate"]:
...
def aggregate(manifest_path: Path, task_dir: Path, output_dir: Path, build_receipt_path: Path | None = None) -> dict[str, Any]:
    ...
    receipt = {
        ...
        "status": status,
        "decision": "grouped_match_q1_development_pass" if status == "PASS" else "grouped_match_q1_development_failed" if status == "FAIL" else "pipeline_complete_non_evidentiary",
