#!/usr/bin/env python3
"""Run the source-bound V5 structured observation-inference confirmation."""
...
MANIFEST_SCHEMA = "fevc-structured-inference-confirmation-manifest-v5"
ROW_SCHEMA = "fevc-structured-inference-confirmation-v5"
TASK_RECEIPT_SCHEMA = "fevc-structured-inference-confirmation-task-v5"
SUMMARY_SCHEMA = "fevc-structured-inference-confirmation-summary-v5"
REGISTRATION_SCHEMA = "FEVC_STRUCTURED_INFERENCE_CONFIRMATION_V5"
REGISTRATION_PATH = Path("fevc/docs/structured_inference_confirmation_v5.json")
...
V5_CONFIRMATION_SEED = 0x7A93_D4C1_52E8_6B0F
...
PROFILE_DEFAULTS = {
    ...
    "smoke": {
        "cells": tuple(cell for cell in ALL_CELLS if (cell["cell"], cell["k"]) in SMOKE_KEYS),
        "replications": 4,
        "shard_size": 4,
    },
    "confirmation": {"cells": ALL_CELLS, "replications": 2_500, "shard_size": 250},
}
...
def create_manifest(root: Path, profile: str, output: Path, shard_size: int | None) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "scientific_contract": {
            ...
            "outcome_seed_hex": "0x7a93d4c152e86b0f",
...
def _validate_row(row: Any, task: dict[str, Any], spec: dict[str, Any]) -> tuple[str, int, int, str]:
    ...
    if row.get("semantic_seed") != _semantic_seed(task["cell"], task["k"], key[2]):
...
def aggregate(
    ...
    if build_receipt_path is not None:
        ...
        if build.get("status") != "success" or build.get("binary_sha256") != binary_hash:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "create-manifest":
            result = create_manifest(arguments.root.resolve(), arguments.profile, arguments.output, arguments.shard_size)
