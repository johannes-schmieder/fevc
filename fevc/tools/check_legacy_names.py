...
SELF_REL = "fevc/tools/check_legacy_names.py"
...
EXCEPTIONS: dict[tuple[str, str, str], int] = {
    ...
    **{(SELF_REL, token, "content"): 1 for token in TOKENS[1:]},
    ...
    ("fevc/cmg/tools/assemble.py", TOKENS[5], "content"): 1,
    ("fevc/cmg/tools/assemble.py", TOKENS[10], "content"): 1,
    ("fevc/cmg/tests/test_assembly.py", TOKENS[5], "content"): 1,
    ("fevc/cmg/tests/test_assembly.py", TOKENS[10], "content"): 1,
    ("fevc/tests/python/test_package_layout.py", TOKENS[4], "content"): 1,
    ("fevc/tests/stata/test_rust_public_install.do", TOKENS[0], "content"): 1,
    ...
    ("fevc/CHANGELOG.md", TOKENS[0], "content"): 4,
...
SECOND_RENAME_EVIDENCE_PREFIXES = (
    "rust/qualification/evidence/",
    "vckss/qualification/",
)


def _is_second_rename_evidence(path: str) -> bool:
    """Return true for immutable evidence retained under its VCKSS identity."""

    return any(path.startswith(prefix) for prefix in SECOND_RENAME_EVIDENCE_PREFIXES) or (
        path.startswith("vckss/benchmarks/") and "/evidence/" in path
    )
...
def main() -> int:
    ...
    for relative in candidates:
        if relative in frozen or _is_second_rename_evidence(relative):
            continue
