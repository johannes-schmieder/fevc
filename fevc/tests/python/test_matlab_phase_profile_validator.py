...
VALIDATOR_PATH = ROOT / "fevc" / "benchmarks" / "validate_matlab_phase_profile.py"
...
def evidence(tmp_path: Path) -> tuple[Path, dict[str, object]]:
    ...
    (job_dir / "application.txt").write_text(
        f"FEVC MATLAB PHASE PROFILE PASS: {LABEL}\n", encoding="utf-8"
    )
...
def test_matlab_profiler_has_bounded_top_level_real_time_contract() -> None:
    source = (ROOT / "fevc/benchmarks/separations_matlab_phase_profile.m").read_text(
        encoding="utf-8"
    )
...
def test_scc_wrapper_binds_and_rechecks_immutable_inputs() -> None:
    source = (ROOT / "fevc/benchmarks/scc/run_matlab_phase_profile.sge").read_text(
        encoding="utf-8"
    )
    ...
    submit = (ROOT / "fevc/benchmarks/scc/submit_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
    collect = (ROOT / "fevc/benchmarks/scc/collect_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
...
def test_new_shell_entrypoints_pass_bash_syntax() -> None:
    scripts = [
        ROOT / "fevc/benchmarks/scc/run_matlab_phase_profile.sge",
        ROOT / "fevc/benchmarks/scc/submit_matlab_phase_profile.sh",
        ROOT / "fevc/benchmarks/scc/collect_matlab_phase_profile.sh",
    ]
