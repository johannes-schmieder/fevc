...
POLICY_PATH = REPO_ROOT / "fevc/docs/development_acceptance_v1.json"
...
def test_active_instructions_reference_the_registered_policy() -> None:
    required = (
        REPO_ROOT / "AGENTS.md",
        REPO_ROOT / "fevc/AGENTS.md",
        REPO_ROOT / "fevc/PLAN.md",
        REPO_ROOT / "fevc/docs/DECISIONS.md",
        REPO_ROOT / "fevc/TESTING.md",
...
def test_active_alpha_harness_uses_policy_not_exact_repeatability() -> None:
    runner = (REPO_ROOT / "fevc/benchmarks/alpha/run_local.py").read_text(
        encoding="utf-8"
    )
    analyzer = (REPO_ROOT / "fevc/benchmarks/alpha/analyze.py").read_text(
        encoding="utf-8"
    )
