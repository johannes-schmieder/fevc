...
HARNESS = REPO_ROOT / "fevc/benchmarks/full_cmg_spike"
...
def test_cz18_scc_smoke_keeps_restricted_rows_remote_and_pinned() -> None:
    ...
    matlab_driver = (
        REPO_ROOT / "fevc/benchmarks/scc/cmg_cz18_matlab_run.m"
...
def test_production_full_cmg_reconciles_phase_specific_tolerances_explicitly() -> None:
    reconciler = (
        REPO_ROOT / "fevc/fevc__rust_reconcile_comp_v7.ado"
    ).read_text(encoding="utf-8")
...
def test_normal_runtime_has_no_private_full_cmg_activation_or_logging() -> None:
    active_paths = (
        ...
        "fevc/fevc.ado",
        "fevc/fevc_rust.ado",
        "fevc/fevc__rust_post_comp_v7.ado",
        "fevc/fevc__rust_reconcile_comp_v7.ado",
    )
    active = "\n".join(
...
def test_cz18_private_diagnostics_are_not_suppressed_by_quietly() -> None:
    ...
    assert "`vckss_prefix' fevc y" in harness
