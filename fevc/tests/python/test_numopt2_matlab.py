...
from fevc.benchmarks.matlab_scale.common import sha256_inventory
from fevc.benchmarks.scc.verify_numopt2_matlab_source import inventory_hash
from fevc.benchmarks.summarize_numopt2_matlab import (
    ADMISSION_HEADROOM,
    HARD_MEMORY_BYTES,
    matlab_pcg_status,
    parse_memory,
...
def test_matlab_comparator_files_are_in_scale_bundle() -> None:
    allowlist = set(
        (ROOT / "fevc/benchmarks/scale_bundle_allowlist.txt")
        .read_text(encoding="utf-8")
        .splitlines()
    )
    required = {
        "fevc/benchmarks/matlab_scale/common.py",
        "fevc/benchmarks/matlab_scale/monitor_process_tree.py",
        "fevc/benchmarks/matlab_scale/source_contract.json",
        "fevc/benchmarks/scc/numopt2_matlab_run.m",
        "fevc/benchmarks/scc/run_numopt2_matlab.sge",
        "fevc/benchmarks/scc/submit_numopt2_matlab.sh",
        "fevc/benchmarks/scc/validate_numopt2_matlab.py",
        "fevc/benchmarks/scc/verify_numopt2_matlab_source.py",
    }
...
def test_matlab_summary_has_no_corrected_estimate_equality_gate() -> None:
    source = (
        ROOT / "fevc/benchmarks/summarize_numopt2_matlab.py"
    ).read_text(encoding="utf-8")
