...
fevc y c1 c2, worker(worker_s) firm(firm_s) deletion(match) ///
...
fevc y c1 c2, worker(worker_r) firm(firm_r) deletion(match) ///
...
fevc y z1 z2, worker(worker) firm(firm) deletion(match) ///
    deletionid(match) algorithm(jla) probes(400) batch(11) ///
    seed(20261001) tolerance(1e-12) nodisplay
...
foreach nuisance_mode in joint fixedoffset {
    ...
    fevc y control_t1 control_t2, worker(worker) firm(firm) ///
...
fevc y, worker(worker_r) firm(firm_r) deletion(match) ///
...
fevc y, worker(worker_swap) firm(firm_swap23) deletion(observation) ///
    algorithm(jla) probes(2) batch(1) seed(1) tolerance(1e-4) ///
    backend(mata) rng(stata) nodisplay
...
assert e(solver_max_residual) <= e(residual_acceptance_tolerance)
fevc y, worker(worker) firm(firm_swap13) deletion(observation) ///
...
sort obsid
fevc y i.time, worker(worker) firm(firm) deletion(match) ///
...
fevc y time1 time2 time3, worker(worker) firm(firm) deletion(match) ///
    deletionid(match) algorithm(exact) nodisplay
...
fevc y if obsid != 2, worker(worker) firm(firm) ///
...
fevc y_missing, worker(worker) firm(firm) ///
    deletion(observation) algorithm(exact) nodisplay
