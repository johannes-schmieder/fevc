...
capture noisily fevc y constant, worker(worker) firm(firm) ///
...
foreach deletion_mode in observation match {
    ...
    foreach nuisance_mode in joint fixedoffset {
        capture noisily fevc y explicit_zero, worker(worker) firm(firm) ///
...
capture noisily fevc y negative_zero, worker(worker) firm(firm) ///
...
capture noisily fevc y explicit_zero [fw=frequency_two], ///
...
capture noisily fevc y basis_one transformed_zero, worker(worker) ///
...
capture noisily fevc y [fw=frequency], worker(worker_relabel) ///
...
quietly fevc y, worker(worker) firm(firm) ///
...
generate double y = z
capture noisily fevc y z, worker(worker) firm(firm) ///
...
capture noisily fevc y block_only_control, worker(worker) firm(firm) ///
...
capture noisily fevc y z1 z2, worker(worker) firm(firm) ///
    deletion(match) deletionid(match) algorithm(exact) nodisplay
assert _rc == 498
