...
if strpos("`c(machine_type)'", "Mac") == 1 {
    local rust_plugin fevc__rust_macos
}
else if "`c(os)'" == "Windows" {
    local rust_plugin fevc__rust_windows
}
else if "`c(os)'" == "Unix" {
    local rust_plugin fevc__rust_linux
}
else {
...
quietly fevc__rust_plugin_call `rust_plugin', result `cmg_handle'
