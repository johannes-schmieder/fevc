...
fevc_rust requestcapability, algorithm(exact)                 ///
...
fevc_rust requestcapability, algorithm(exact) deletion(match) ///
...
foreach nuisance in joint fixedoffset {
    fevc_rust prepare worker firm deletion_id outcome frequency  ///
    ...
    fevc_rust solve `handle', algorithm(exact) deletion(match)   ///
        nuisance(`nuisance') exactlimit(100) blocksizelimit(100)
    fevc_rust result `handle'
...
assert r(deletion_mode_code) == 2
fevc_rust solve `handle', algorithm(exact) deletion(observation) ///
...
di as result "FEVC RUST EXACT CONTROLS PASS"
exit 0
