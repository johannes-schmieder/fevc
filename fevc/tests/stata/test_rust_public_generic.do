...
quietly fevc_rust solve `private_handle', algorithm(jla) deletion(match) ///
...
assert `"`e(route_api)'"' == "FEVC-NATIVE-GENERIC-PLANNED-V4-V7"
...
quietly fevc outcome ib0.category [fw=frequency],            ///
...
quietly fevc outcome category_1 category_2 category_3 category_4 ///
...
quietly fevc outcome c.control#ib0.replicate [fw=frequency], ///
...
capture quietly fevc outcome_q `controls32', worker(worker_q) firm(firm_q) ///
    deletion(observation) nuisance(joint) algorithm(jla)            ///
    backend(rust) rng(counter_v1) engine(generic) preconditioner(diagonal) ///
...
di as result "FEVC RUST PUBLIC GENERIC PASS"
exit 0
