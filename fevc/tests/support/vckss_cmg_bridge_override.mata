*! Test-only replacement for vckss__stata_jla(); load only after mata drop
...
{
    struct vckss_cmg_estimator_result scalar routed
    ...
    memory_gib = strtoreal(st_global("VCKSS_CMG_MEMORY_GIB"))
    if (missing(memory_gib)) memory_gib = 4
    routed = vckss__jla_cmg(
    ...
    st_matrix("VCKSS_CMG_ROUTE_DIAGNOSTICS",routed.diagnostics)
    st_global("VCKSS_CMG_STATUS",routed.cmg_status)
    st_global("VCKSS_CMG_MESSAGE",routed.cmg_message)
