...
`U=(Pq)_i^2` and `V=(Mq)_i^2`. The literal-frequency pooled observation
statistic is defined below. With `R` independent directions,
...
The stored-row implementation draws $S_r$ directly as
$2\operatorname{Binomial}(f_r,1/2)-f_r$. The variables $S_r$ are
independent across stored rows. This changes the random-number consumption,
but it reproduces the joint distribution of the literal physical-copy sums.
...
Consequently, define the per-probe pooled statistics for an observation row as

\[
U_r=p_r^2,
\qquad
V_r=f_r^{-1}\sum_a(q_{ra}-p_r)^2
    =1+p_r^2-2p_rS_r/f_r.
\]

All identical copies have the same true projection and residual leverage, so
the pooled means estimate that common per-copy pair. The raw moments required
by the ratio delta method are available without expansion:

\[
\begin{aligned}
U_r^2 &=p_r^4,\\
V_r^2 &=\{1+p_r^2-2p_rS_r/f_r\}^2,\\
U_rV_r&=p_r^2\{1+p_r^2-2p_rS_r/f_r\}.
\end{aligned}
\]

In particular, (V_r^2) is the square of the pooled copy-residual second
moment. It is generally not the average of the copywise fourth powers when
(f_r>1). Using that average would mix two different random variables in the
same ratio expansion. The runtime and exhaustive test use the pooled square.

The explicit-expanded reference for this collapsed calculation is precise:
draw one independent sign for every physical copy, compute the expanded
projection and residual coordinates, pool the squared coordinates of the
exchangeable copies within each stored row for that probe, and only then form
the constrained ratio and its finite-projection correction. The collapsed
runtime reproduces those per-probe pooled statistics draw for draw as a
function of (S_r). It does not reproduce the different finite-(R) algorithm
that forms a separate nonlinear ratio for every physical copy and averages
those ratios afterward. That coordinatewise algorithm retains copy identities
across probes and cannot in general be recovered from the collapsed sums.
Both algorithms converge to the same common per-copy leverage as (R) grows;
the package contract selects the symmetric pooled algorithm and labels its
finite-projection diagnostic accordingly.
...
expanded design and checks the pooled-observation, match, and target
aggregation identities direction by direction.
