...
def test_frequency_two_pooled_ratio_uses_pooled_second_moment() -> None:
    ...
    m = np.mean(pooled_residual_square)
    p_second = np.mean(projected_square**2)
    pooled_second = np.mean(pooled_residual_square**2)
    wrong_copywise_fourth = np.mean(physical_residual**4)
    mixed = np.mean(projected_square * pooled_residual_square)
    _, correct_bias, _ = jla_bias_terms(
        p, m, p_second, pooled_second, mixed, 1
    )
    _, wrong_bias, _ = jla_bias_terms(
        p, m, p_second, wrong_copywise_fourth, mixed, 1
    )
    assert np.isclose(correct_bias, -0.00105, atol=2e-14)
    assert np.isclose(wrong_bias, -0.04155, atol=2e-14)
    ...
    m_hat = counts @ pooled_residual_square / probes
    scaled_bias = np.mean(probes * (m_hat / (p_hat+m_hat) - m))
    assert abs(scaled_bias-correct_bias) < 0.012
    assert abs(scaled_bias-correct_bias) < 0.25*abs(scaled_bias-wrong_bias)
