...
def test_observation_probe_moments_equal_literal_copy_aggregation() -> None:
    ...
    for stored_row, group in enumerate(groups):
        ...
        collapsed_m2_second = collapsed_m2**2
        ...
        if frequency[stored_row] > 1:
            copywise_fourth = np.mean(physical_m**4, axis=1)
            assert np.max(np.abs(copywise_fourth-collapsed_m2_second)) > 0.1

    # For the first frequency-two row the old average-copy fourth moment is
    # not the second raw moment of the pooled residual-square statistic.
    first_p = projected[:, 0]
    first_residual = (directions @ residual_maker.T)[:, groups[0]]
    ...
    assert np.isclose(np.mean(first_p**2), 0.45, atol=2e-15)


def test_pooled_ratio_is_distinct_from_coordinatewise_finite_probe_ratio() -> None:
    frequency = np.array([2, 1, 2], dtype=int)
    stored_design = np.array([[1.0, 0.0], [1.0, 1.0], [1.0, 2.0]])
    expanded_design = np.repeat(stored_design, frequency, axis=0)
    projection = expanded_design @ np.linalg.inv(
