...
program define kss_bc, eclass sortpreserve
    ...
    local expected_mata_build "kss-bc-api11-quotient-canonical-controls-explicit-rank"
    capture mata: kssbc__api_level()
    local mata_runtime_loaded = (_rc == 0)
    capture mata: assert(kssbc__api_level() == 11 &                 ///
    ...
    if "`selected_algorithm'" == "jla" {
        ...
        if "`deletion'" == "observation" & r(sum) > `physical_limit' {
            quietly _kss_bc_post_failure "PHYSICAL_COPY_LIMIT"
            di as error "observation JLA physical copies exceed physical_limit()"
            exit 498
        }
        ...
        // index the stream: each can change under a contract-preserving
        // relabeling, split, or invertible control reparameterization.  Rows
        // tied on the primary key are exchangeable only when their controls,
        // model coordinate, and match block agree.  Otherwise withhold.
        tempvar probe_target probe_worker_min probe_worker_max
        tempvar probe_firm_min probe_firm_max
        tempvar probe_ambiguous
        quietly generate double `probe_target' = `target'/`frequency' if `touse'
        local probe_key `depvar' `probe_target'
        sort `probe_key'
        quietly generate byte `probe_ambiguous' = 0
        foreach control of local controlvars {
            tempvar probe_control_min probe_control_max
            quietly by `probe_key': egen double `probe_control_min' = ///
                min(`control') if `touse'
            quietly by `probe_key': egen double `probe_control_max' = ///
                max(`control') if `touse'
            quietly replace `probe_ambiguous' = 1 if `touse' & ///
                `probe_control_min' != `probe_control_max'
        }
        quietly by `probe_key': egen long `probe_worker_min' = ///
            min(`id_worker') if `touse'
        quietly by `probe_key': egen long `probe_worker_max' = ///
            max(`id_worker') if `touse'
        quietly by `probe_key': egen long `probe_firm_min' = ///
            min(`id_firm') if `touse'
        quietly by `probe_key': egen long `probe_firm_max' = ///
            max(`id_firm') if `touse'
        quietly replace `probe_ambiguous' = 1 if `touse' & ///
            (`probe_worker_min' != `probe_worker_max' | ///
             `probe_firm_min' != `probe_firm_max')
        if "`deletion'" == "match" {
            tempvar probe_delete_min probe_delete_max
            quietly by `probe_key': egen long `probe_delete_min' = ///
                min(`deletion_id') if `touse'
            quietly by `probe_key': egen long `probe_delete_max' = ///
                max(`deletion_id') if `touse'
            quietly replace `probe_ambiguous' = 1 if `touse' & ///
                `probe_delete_min' != `probe_delete_max'
        }
        quietly count if `touse' & `probe_ambiguous'
        if r(N) {
        ...
        if "`deletion'" == "match" {
            sort `probe_key' `id_worker' `id_firm' `deletion_id'
        }
        else sort `probe_key' `id_worker' `id_firm'
    }
