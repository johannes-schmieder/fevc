...
Before matrix-free inverse actions, the implementation replaces `Z` by an
ID-free canonical basis for the same weighted column span. It first whitens
the span, selects anchor rows using invariant row inner products in the
conceptual-copy order, and maps those rows to the identity. Thus an invertible
input change `Z -> ZT` generates the same iterative right-hand sides up to the
registered whitening, anchor, solve, and roundoff gates. Failure to construct
that basis at the requested rank margin causes typed conservative withholding.
