...
The full suite includes the source-bound API10 counterexamples: explicit and
negative-zero controls across every backend/nuisance/deletion route, automatic
dispatch, the six-row `K(2,3)` firm-relabeling attack, the determinant-four
control-basis map at `probes(2)`, maximum allowed tolerance, physical-copy
allocation withholding, frequency regrouping, and stale-runtime rejection.
