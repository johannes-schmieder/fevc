...
{
    return("kss-bc-api12-certified-anchor-finite-corrected")
}
    ...
    real scalar rank_tolerance)
{
    ...
    numerical_error = max((whitening_error,
        gram_inverse.relres/max((gram_inverse.rcond,rank_tolerance))))
    uncertainty = max((1e-12,100*numerical_error))
    ...
    // the orthogonal rotation above.  The ado layer has already put accepted
    // JLA rows in its ID-free conceptual-copy order; equal-key rows that may
    // permute are control-identical.  Hence the selected anchor and the RREF-
    // style basis below are independent of the user's control coordinates.
    ...
    for (pivot=1; pivot<=control_count; pivot++) {
        ...
        numerical_error = max((numerical_error,
            anchor_inverse.relres/
            max((anchor_inverse.rcond,rank_tolerance))))
        uncertainty = max((1e-12,100*numerical_error))
        if (uncertainty >= margin/4) {
            out.status = "AMBIGUOUS_CONTROL_BASIS"
...
{
    real colvector fitted, worker_mean, residual, firm_sum
