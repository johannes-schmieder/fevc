...
canonical basis for the same weighted column span. It first whitens the span,
selects anchor rows using invariant row inner products in the conceptual-copy
order, and maps those rows to the identity. Each pivot uses a forward-error
envelope built from the measured whitening and inverse residuals divided by
their reciprocal-conditioning margins. A row score within that envelope of
the eligibility cutoff, or an envelope too large relative to the anchor
margin, causes `AMBIGUOUS_CONTROL_BASIS`. On accepted inputs, an invertible
change `Z -> ZT` therefore generates the same right-hand sides up to the
registered whitening, anchor, solve, and roundoff gates.
