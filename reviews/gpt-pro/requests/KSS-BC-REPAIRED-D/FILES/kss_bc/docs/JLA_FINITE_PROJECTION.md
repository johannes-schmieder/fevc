...
Consequently, the physical-copy averages needed for observation deletion are
available without expansion:

\[
\begin{aligned}
f_r^{-1}\sum_a(q_{ra}-p_r)^2
 &=1+p_r^2-2p_rS_r/f_r,\\
f_r^{-1}\sum_a(q_{ra}-p_r)^4
 &=1+6p_r^2+p_r^4-4p_r(1+p_r^2)S_r/f_r,\\
f_r^{-1}\sum_ap_r^2(q_{ra}-p_r)^2
 &=p_r^2\{1+p_r^2-2p_rS_r/f_r\}.
\end{aligned}
\]
...
expanded design and checks the observation, match, and target aggregation
identities direction by direction.
