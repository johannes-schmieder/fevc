    ...
    real scalar rank_tolerance)
{
    ...
    out.relres = kssbc__norm2(residual) / sqrt(dimension)
    ...
    real scalar blocksize_limit)
{
    ...
    if (n <= full_parameters) {
        return(kssbc__failure("INSUFFICIENT_OBSERVATIONS", "stored observations do not exceed coefficient dimension"))
    }
    ...
    if (deletion == "observation") {
        ...
        if (min(leverage_diagonal) < -100 * rank_tolerance |
            max(leverage_diagonal) >= 1 - block_tolerance) {
    ...
    real scalar rank_tolerance)
{
    ...
    out.preparation_relres = kssbc__norm2(schur-checked_schur) /
        (1+kssbc__norm2(checked_schur))
    ...
    real scalar maxiter)
{
    ...
    out.relres = kssbc__norm2(residual) /
        (1+kssbc__norm2(right_hand_side))
    if (hasmissing(out.coefficient) | hasmissing(out.relres) |
        out.relres > max((1e-11,10*tolerance))) {
        out.status = "SOLVER_RESIDUAL_FAILED"
...
{
    real colvector out, values, index
    real scalar group, trials

    out = J(rows(frequency),1,.)
    values = uniqrows(sort(frequency,1))
    for (group=1; group<=rows(values); group++) {
        trials = values[group]
        index = selectindex(frequency :== trials)
        out[index] = 2 :* rbinomial(rows(index),1,trials,0.5) :- trials
    }
    return(out)
}
    ...
    real scalar rank_tolerance)
{
    ...
    struct kssbc_inverse_result scalar within_inverse
    real scalar n, control_count, row, cell, group, groups
    real scalar remaining_frequency, new_trace, loss, threshold
    real colvector cell_order, cell_sorted_code, cell_of_row
    real colvector cell_frequency, cell_square, cell_scatter_trace
    real colvector deletion_order, block_frequency, block_square, block_cell
    real matrix cell_panel, deletion_panel, weighted_controls, cell_sum
    real matrix within, whitener, transformed, weighted_transformed
    real matrix checked_within, block_sum
    ...
    within = controls' * weighted_controls -
        cell_sum' * (cell_sum :/
    ...
    transformed = controls * whitener
    weighted_transformed = frequency :* transformed
    cell_sum = panelsum(weighted_transformed[cell_order,.],cell_panel)
    cell_square = panelsum(
        (frequency:*rowsum(transformed:^2))[cell_order],cell_panel)
    cell_scatter_trace = cell_square -
        rowsum(cell_sum:^2) :/ cell_frequency
    checked_within = transformed' * weighted_transformed -
        cell_sum' * (cell_sum :/
        (cell_frequency*J(1,control_count,1)))
    if (hasmissing(checked_within) |
        kssbc__norm2(checked_within-I(control_count)) >
        max((1e-9,1000*rank_tolerance))) {
    ...
    if (deletion == "match") {
        ...
        block_square = panelsum(
            (frequency:*rowsum(transformed:^2))[deletion_order],
            deletion_panel)
        ...
        for (group=1; group<=groups; group++) {
            ...
            if (remaining_frequency > 0) {
                new_trace = cell_square[cell]-block_square[group] -
                    sum((cell_sum[cell,.]-block_sum[group,.]):^2) /
                    remaining_frequency
            }
            else new_trace = 0
            loss = cell_scatter_trace[cell]-new_trace
            if (loss < -max((1e-9,1000*rank_tolerance))) {
        ...
        for (row=1; row<=n; row++) {
            ...
            if (remaining_frequency > 0) {
                new_trace = cell_square[cell]-sum(transformed[row,.]:^2) -
                    sum((cell_sum[cell,.]-transformed[row,.]):^2) /
                    remaining_frequency
            }
            else new_trace = 0
            loss = cell_scatter_trace[cell]-new_trace
    ...
    out.gap = 1-out.max_loss
    threshold = max((1e-10,100*rank_tolerance))
    if (hasmissing(out.gap) | out.gap <= threshold) {
    ...
    real scalar blocksize_limit)
{
    ...
    real scalar deletion_rank_gap
    real matrix deletion_panel, sorted_delete, rhs, target_rhs, target_draws
    ...
    real colvector rademacher_sum, projected, p_first, m_first, p_second
    ...
    for (batch_start=1; batch_start<=probes; batch_start=batch_start+batch) {
        ...
        for (batch_column=1; batch_column<=batch_columns; batch_column++) {
            ...
            if (deletion == "observation") {
                p_first = p_first + projected:^2
                m_first = m_first :+ 1 :+ projected:^2 :-
                    2 :* projected :* rademacher_sum :/ frequency
                p_second = p_second + projected:^4
                m_second = m_second :+ 1 :+ 6:*projected:^2 :+ projected:^4 :-
                    4:*projected:*(1:+projected:^2):*
                    rademacher_sum:/frequency
                mixed_second = mixed_second + projected:^2 :*
                    (1:+projected:^2:-2:*projected:*
                    rademacher_sum:/frequency)
            }
            else {
    ...
    maximum_leverage = 0
    if (deletion == "observation") {
        total_residual_leverage = m_constrained - control_leverage
        ...
        inverse_weight = 1:/total_residual_leverage +
            finite_bias:/total_residual_leverage:^2 -
            finite_variance:/total_residual_leverage:^3
        if (hasmissing(inverse_weight) | min(inverse_weight) <= 0) {
        ...
        maximum_leverage = max(p_constrained+control_leverage)
    }
    else {
