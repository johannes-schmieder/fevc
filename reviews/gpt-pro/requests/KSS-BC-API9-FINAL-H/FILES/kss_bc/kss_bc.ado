...
program define kss_bc, eclass sortpreserve
    ...
    local expected_mata_build "kss-bc-api9-coordinatewise-copy-tie-safe"
    capture mata: kssbc__api_level()
    local mata_runtime_loaded = (_rc == 0)
    capture mata: assert(kssbc__api_level() == 9 &                  ///
    ...
    if "`selected_algorithm'" == "exact" {
        ...
        ereturn local deletion_rank_certificate "within-cell trace and direct factor gates"
    }
    else {
