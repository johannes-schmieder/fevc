...
An ID-free canonical control-span basis makes iterative inverse actions
independent of an invertible user control reparameterization. Every anchor
decision carries a measured numerical uncertainty envelope; a score near the
selection boundary is withheld as `AMBIGUOUS_CONTROL_BASIS`. Observation JLA
also has a typed `physical_limit()` allocation boundary. Both backends check
the final plug-in-minus-correction row separately before posting it.
