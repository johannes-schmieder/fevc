...
the selected anchor rows become the identity. The implementation propagates a
conservative forward-error envelope from the whitening and anchor-inverse
residuals, scaled by their reciprocal-conditioning margins. It rejects an
envelope at least one quarter of the anchor margin and rejects any pivot score
whose eligibility is within the envelope of the cutoff. Those cases return
`AMBIGUOUS_CONTROL_BASIS` instead of selecting the first row in an uncertified
tolerance band. If `Z` is replaced by `ZT` for invertible `T`, every accepted
calculation therefore supplies the same certified anchor sequence and
canonical controls up to the registered roundoff gates. Adaptive per-column
