...
rule makes accepted JLA executions pathwise invariant to control-basis
reparameterization, ID relabeling, harmless row sorting, and regrouping the
same physical copies. Sequential direction generation also makes the logical
stream invariant to `batch()`.
