...
def test_observation_frequency_uses_pooled_residual_second_moment() -> None:
    runtime = re.sub(
        r"\s+", "", (ROOT / "kss_bc.mata").read_text(encoding="utf-8")
    )
    assert "m_second=m_second+observation_residual_square:^2" in runtime
    assert "m_second=m_second:+1:+6:*projected:^2" not in runtime
