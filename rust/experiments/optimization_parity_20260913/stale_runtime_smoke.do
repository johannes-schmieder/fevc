...
assert mod(floor(r(core_ready_flags)/256),2)==1
assert mod(floor(r(core_ready_flags)/1024),2)==0
capture noisily fevc y, worker(worker) firm(firm) algorithm(jla) ///
    backend(rust) rng(counter_v1) engine(generic) preconditioner(cmg) ///
    batch(auto) probes(9) nodisplay
assert _rc==498
...
display "STALE_POINT_RUNTIME_REJECTION_PASS"
