...
local private_input : environment PF_PRIVATE_INPUT
local output_directory : environment PF_OUTPUT_DIRECTORY
...
if "`private_input'"=="1" {
    confirm variable year y_adjusted
    isid worker year
    rename y_adjusted y
    egen double match = group(worker firm)
}
else {
    isid worker firm
    isid match
...
tempname marker
file open `marker' using `"`output_directory'/primary.started"', write text
file write `marker' "START" _n
file close `marker'
timer on 80
adopath ++ `"`package'"'
...
timer off 80
file open `marker' using `"`output_directory'/primary.ended"', write text
file write `marker' "END" _n
file close `marker'
if "`diagnostic'"=="1" {
    profiler off
    profiler report
...
assert e(N_stored)==real("`expected_rows'")
assert e(sample)
