...
- **Pass:** executed at `f2fa740` with the stated result.
...
| P1-CORE-LOCKED | Portable declared-toolchain core gate | All compiled `vckss-core` unit fixtures and root lockfile | 53 tests and doctests pass without changing `Cargo.lock` | Rust 1.81 and stable; all CI OSes | `cargo +1.81.0 test --manifest-path rust/Cargo.toml -p vckss-core --locked` | **Pending:** portable command was not the local evidence command | Planned: `rust/qualification/evidence/P1-CORE-LOCKED.txt` |
...
| P1-WORKSPACE | Full workspace compile/tests | Core, plugin library, all unit and integration targets | Exit 0 for every target | Rust 1.81 and stable; Windows, Linux, macOS | `cargo +1.81.0 test --manifest-path rust/Cargo.toml --workspace --all-targets --locked` | **Red:** missing `vckss_core::engine` and nonexistent `ErrorCode::InvalidId` | Not yet recorded; blockers are visible in `session_engine.rs`, `engine_result_surface.rs`, and `ffi_session.rs` |
...
| Test identifier | Component | Fixture | Expected result | Platforms | Command | Status | Evidence artifact |
| --- | --- | --- | --- | --- | --- | --- | --- |
| P3-ENGINE-FINITE | `run_jla_no_controls` engine | Small explicit finite two-way problem | Preparation through plugin, correction, corrected components, MCSEs, accounting, and complete residuals pass | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/Cargo.toml -p vckss-core engine::tests -- --nocapture` | **Blocked:** `engine.rs` and `pub mod engine` are absent | Planned: `rust/qualification/evidence/P3-ENGINE-FINITE.json` |
| P3-SESSION-E2E | Staged numerical session | Existing dense no-control fixture | Prepare, solve, inspect result, and release succeed | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/Cargo.toml -p vckss-plugin --test session_engine -- --nocapture` | **Red:** missing core engine module | Planned: `rust/qualification/evidence/P3-SESSION-E2E.txt` |
| P3-RESULT-SURFACE | Numerical result contract | Engine result type | Plugin, correction, corrected, MCSE, probe, route, residual, timing, and resource receipts are present and validated | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/Cargo.toml -p vckss-plugin --test engine_result_surface -- --nocapture` | **Red:** missing core engine; existing test freezes only plugin and corrected components | Planned: `rust/qualification/evidence/P3-RESULT-SURFACE.txt` |

## P4: PCG and CMG

| Test identifier | Component | Fixture | Expected result | Platforms | Command | Status | Evidence artifact |
| --- | --- | --- | --- | --- | --- | --- | --- |
| P4-BATCH-PCG | Scalar and batched PCG | Existing mixed RHS fixtures, including zero RHS | Batched actions match scalar actions; every RHS has an independent receipt and complete residual | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/crates/vckss-core/Cargo.toml --lib batch::tests -- --nocapture` | **Pass within the 53-test core runs** | Not yet recorded; local terminal output only |
| P4-ROUTER | Exact/diagonal/CMG routing | Small, boundary, setup-failure, and relabeling fixtures | `F-1` admission is exact; fallback occurs only before RNG on setup failure; iterative failure does not reroute | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/crates/vckss-core/Cargo.toml --lib solver::tests -- --nocapture` | **Pass within the 53-test core runs** | Not yet recorded; local terminal output only |
| P4-CMG-RUST | Hybrid graph, hierarchy, V-cycle, CMG-PCG | Degree 2/3/4+, density-four, tie-rich and structural-twin fixtures | Exact hybrid algebra, bounded hierarchy, symmetric positive application, converged certified solve, and relabeling equivariance | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/crates/vckss-core/Cargo.toml --lib cmg:: -- --nocapture` | **Pass within the 53-test core runs** | Not yet recorded; local terminal output only |
...
| Test identifier | Component | Fixture | Expected result | Platforms | Command | Status | Evidence artifact |
| --- | --- | --- | --- | --- | --- | --- | --- |
| P5-COUNTER-VECTORS | `VCKSS-COUNTER-V1` | Frozen Philox block/word/sum/domain/batch vectors | Exact bitwise match and typed invalid-count failure | Rust 1.81 and stable; all CI OSes | `cargo test --manifest-path rust/crates/vckss-core/Cargo.toml --lib rng::tests -- --nocapture` | **Pass within the 53-test core runs on macOS arm64** | Not yet recorded; local terminal output only |
| P5-END-TO-END-INVARIANCE | Complete randomized estimator | Row permutations; batch widths 1/2/4/8/16; threads 1/2/4/8/16/32 | Same logical atoms and registered deterministic outputs for fixed counter contract | Available thread counts; all target OSes | `cargo test --manifest-path rust/Cargo.toml -p vckss-core --test rng_invariance -- --nocapture` | **Blocked:** engine and test target are absent | Planned: `rust/qualification/evidence/P5-END-TO-END-INVARIANCE/` |
...
| P10-SCALE | End-to-end preparation and solve | Medium production-like and synthetic cases approaching 30M workers, 1M firms, 20 years; probes 20/100/200; threads 1/4/8/16/32 | Phase timings, RSS, modeled memory, route, residuals, accounting, and thread invariance recorded; no tiny-graph extrapolation substitutes for run evidence | Qualified local/SCC hosts | `cargo bench --manifest-path rust/Cargo.toml --bench rust_backend_scale -- --save-baseline f2fa740` | **Blocked:** benchmark target and scale fixtures do not exist | Planned: `rust/qualification/evidence/P10-SCALE/` |
