...
//! Source-bound development generator for fixed-offset collapsed-match q=0 inference.
//!
//! This executable exercises the real internal generic-JLA attachment.  It is
//! intentionally not part of the plugin ABI or Stata command surface.
...
use vckss_core::component_inference::{
    ...
    ComponentInferenceOptions, ComponentVarianceSource,
};
...
const ROW_SCHEMA: &str = "fevc-match-inference-q0-development-row-v1";
const PREFLIGHT_SCHEMA: &str = "fevc-match-inference-q0-design-preflight-v1";
const MASTER_SEED: u64 = 0x98d3_407a_f651_2cbe;
...
impl Cell {
    ...
    fn dependence_name(self) -> &'static str {
    ...
    fn variance_dgp_name(self) -> &'static str {
    ...
    fn target_shape_name(self) -> &'static str {
    ...
    fn route_name(self) -> &'static str {
        match self.route {
            ...
            ModelSolverRoute::Auto => unreachable!("campaign never uses automatic routing"),
        }
    }
}
...
fn main() {
    ...
    match arguments.first().map(String::as_str) {
        Some("q0-matrix-v1") => run_matrix(&arguments[1..]),
        Some("design-preflight-v1") => run_preflight(&arguments[1..]),
        _ => panic!("expected q0-matrix-v1 or design-preflight-v1"),
    }
}
...
fn parse_request(
    arguments: &[String],
    with_range: bool,
) -> (Cell, usize, usize, usize, NumericalSettings) {
    let expected = if with_range { 8 } else { 6 };
    assert_eq!(
        arguments.len(),
        expected,
        "invalid grouped q0 argument count"
    );
    let cell = Cell::named(&arguments[0]).expect("unknown grouped q0 cell");
    ...
    assert!(
        settings.estimator_probes >= 2
            && settings.covariance_probes >= 2
            && settings.spectrum_probes >= 2
            && settings.spectrum_iterations >= 2,
        "all probe and iteration counts must be at least two"
    );
    (cell, dimension, start, replications, settings)
}

fn run_matrix(arguments: &[String]) {
    let (cell, dimension, start, replications, settings) = parse_request(arguments, true);
    for replication in start..start + replications {
        let seed = semantic_seed(MASTER_SEED, cell.name, dimension, replication);
        let generated = make_problem(cell, dimension, seed);
        match run_attached(cell, &generated, settings, seed) {
            Ok((result, baseline)) => {
                ...
                emit_success(cell, dimension, replication, seed, &generated, &result);
            }
...
fn run_preflight(arguments: &[String]) {
    let (cell, dimension, _, _, settings) = parse_request(arguments, false);
    let seed = semantic_seed(MASTER_SEED, cell.name, dimension, 0);
    // Preflight certifies outcome-free geometry.  Weak/null signal is a later
    // execution diagnostic, so do not let its expected covariance withholding
    // obscure the design's leverage and spectrum here.
    ...
    let estimator = estimator_options(cell, settings, seed);
    let prepared = prepare_grouped_oracle_component_inference(
        &generated.problem,
        &generated.aggregate_variance,
        inference_options(settings, seed),
    )
    .expect("registered preflight oracle attachment must prepare");
    ...
    .expect("registered design preflight must execute");
    let inference = result
        .component_inference
        .expect("preflight inference result");
    for target in 0..4 {
        let spectrum = inference.spectrum[target];
        println!(
            concat!(
                "{{\"schema\":\"{}\",\"cell\":\"{}\",\"k\":{},\"target\":\"{}\",",
                "\"target_shape\":\"{}\",\"independent_matches\":{},",
...
> {
    ...
    let prepared = prepare_grouped_structured_component_inference(
        ...
        StructuredVarianceOptions {
            seed: seed ^ 0x63e1_8b7f_a504_2d99,
...
fn make_problem(cell: Cell, dimension: usize, seed: u64) -> GeneratedProblem {
    ...
    for worker_index in 0..dimension {
        for firm_index in 0..dimension {
            ...
            .clamp(0.0, 1.0);
            // The registered correct model is exactly linear in the same
            // normalized match-mass midrank included by structured-common.
...
fn match_errors(
    ...
    match dependence {
        ...
        WithinMatch::EqualAggregateShapes => unreachable!("mixed shape resolves before drawing"),
    }
}
...
fn emit_failure(
    ...
    let (code, phase) = error.map_or(("NONE", "none"), |value| (value.code.as_str(), value.phase));
    for target in 0..4 {
        println!(
            concat!(
                "{{\"schema\":\"{}\",\"cell\":\"{}\",\"gate\":\"{}\",\"k\":{},",
                ...
                "\"variance_model\":\"{}\",\"variance_dgp\":\"{}\",",
                "\"within_match_dependence\":\"{}\",\"match_mass\":\"{}\",",
                "\"target_shape\":\"{}\",\"signal\":\"{}\",\"controls\":{},",
                "\"solver\":\"{}\",\"coverage_eligible\":{}}}"
            ),
            ROW_SCHEMA,
            cell.name,
            cell.gate,
...
fn emit_success(
    ...
    for target in 0..4 {
        ...
        println!(
            concat!(
                "{{\"schema\":\"{}\",\"cell\":\"{}\",\"gate\":\"{}\",\"k\":{},",
                "\"replication\":{},\"target\":\"{}\",\"semantic_seed\":{},\"status\":\"success\",",
                "\"variance_model\":\"{}\",\"variance_dgp\":\"{}\",",
                "\"within_match_dependence\":\"{}\",\"match_mass\":\"{}\",",
                "\"target_shape\":\"{}\",\"signal\":\"{}\",\"controls\":{},",
                "\"solver\":\"{}\",\"coverage_eligible\":{},",
                ...
                "\"leading_share\":{:.17e},\"remainder_share\":{:.17e},",
                "\"maximum_mode_weight\":{:.17e},\"leading_residual\":{:.17e},",
                "\"minimum_active_terms\":{},\"minimum_training_matches\":{},",
                "\"variance_floor_count\":{},\"variance_floor_share\":{:.17e},",
                "\"boundary_share\":{:.17e},\"maximum_boundary_excess\":{:.17e},",
                "\"maximum_prediction_leverage\":{:.17e},\"minimum_fitted_rcond\":{:.17e},",
                "\"sensitivity_median_log_ratio\":{:.17e},\"sensitivity_p90_log_ratio\":{:.17e},",
                "\"sensitivity_maximum_log_ratio\":{:.17e},\"sensitivity_log_variance_correlation\":{:.17e},",
                "\"maximum_complete_residual\":{:.17e},\"full_residual_tolerance\":{:.17e},",
                "\"maximum_trace_mcse\":{:.17e},\"psd_cleanup\":{:.17e},",
                "\"smallest_covariance_eigenvalue\":{:.17e},\"largest_covariance_eigenvalue\":{:.17e},",
                "\"point_correction_identity_error\":{:.17e},\"aggregate_variance_mean\":{:.17e},",
                "\"nuisance_uncertainty_conditioned_away\":{}}}"
            ),
            ROW_SCHEMA,
            cell.name,
            ...
            estimated_variance,
            standard_error,
            point_error.abs() <= NORMAL_CRITICAL * standard_error,
            point_error < -NORMAL_CRITICAL * standard_error,
            point_error > NORMAL_CRITICAL * standard_error,
            inference.independent_units,
            inference.effective_match_count,
            inference.largest_match_mass_share,
...
mod tests {
    ...
    fn semantic_seed_is_frozen() {
        assert_eq!(
            semantic_seed(MASTER_SEED, "diffuse_equal_independent", 14, 0),
            14_199_910_461_300_949_147
        );
    }

    #[test]
    fn every_registered_cell_constructs_valid_match_aggregates() {
        for name in [
            "diffuse_equal_independent",
            "diffuse_equal_independent_cmg",
            "diffuse_unequal_independent",
            "diffuse_unequal_common_shock",
            "diffuse_unequal_serial",
            "diffuse_equal_aggregate_shapes",
            "diffuse_leverage_sensitivity",
            "diffuse_mild_omitted",
            "diffuse_severe_omitted",
            "one_mode_diagnostic",
            "multi_mode_diagnostic",
            "weak_signal",
            "null_signal",
            "controls_varying_fixedoffset",
        ] {
            let cell = Cell::named(name).expect("registered name");
            let generated = make_problem(cell, 8, semantic_seed(MASTER_SEED, name, 8, 0));
            assert_eq!(generated.problem.deletion_units(), 64);
            assert_eq!(generated.aggregate_variance.len(), 64);
            assert!(generated
                .aggregate_variance
                .iter()
                .all(|value| *value > 0.0));
            assert_eq!(
                generated.problem.controls.len(),
                usize::from(cell.controls) * 2
            );
        }
    }

    #[test]
    fn correct_aggregate_variance_is_exactly_in_match_mass_midrank_basis() {
        for mass in [MatchMass::Equal, MatchMass::Unequal] {
            let name = match mass {
                MatchMass::Equal => "diffuse_equal_independent",
                MatchMass::Unequal => "diffuse_unequal_independent",
            };
            let cell = Cell::named(name).expect("registered name");
            let dimension = 8;
            let generated = make_problem(
                cell,
                dimension,
                semantic_seed(MASTER_SEED, name, dimension, 0),
            );
            let masses = (0..dimension * dimension)
                .map(|group| {
                    frequencies_for_match(mass, dimension, group)
                        .iter()
                        .sum::<u64>() as f64
                })
                .collect::<Vec<_>>();
            for (group, (&observed, &value)) in
                generated.aggregate_variance.iter().zip(&masses).enumerate()
            {
                let less = masses.iter().filter(|&&other| other < value).count();
                let equal = masses.iter().filter(|&&other| other == value).count();
                let midrank = less as f64 + 0.5 * (equal + 1) as f64;
                let rank = 2.0 * ((midrank - 0.5) / masses.len() as f64) - 1.0;
                let expected = 0.26 + 0.08 * rank;
                assert!(
                    (observed - expected).abs() < 1.0e-15,
                    "group {group}: observed={observed} expected={expected}"
                );
            }
        }
    }
