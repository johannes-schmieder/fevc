...
//! Source-bound development generator for fixed-offset collapsed-match q=1 inference.
//!
//! The registered q=0 generator remains immutable. This executable retains its
//! DGP algebra in a separately frozen source, then changes the registered cells,
//! Counter domain, reference distribution, and result schema.
...
mod campaign {
    ...
    const Q1_ROW_SCHEMA: &str = "fevc-match-inference-q1-development-row-v1";
    const Q1_PREFLIGHT_SCHEMA: &str = "fevc-match-inference-q1-design-preflight-v1";
    const Q1_MASTER_SEED: u64 = 0x4d61_7463_6851_31a1;
    ...
    fn q1_cell(name: &str) -> Option<(Cell, Reference)> {
        let (base, registered_name, gate, shape, reference) = match name {
            "one_mode_equal_independent" => (
            ...
            "one_mode_equal_independent_cmg" => (
            ...
            "one_mode_equal_aggregate_shapes" => (
        ...
        let mut cell = Cell::named(base).expect("q1 base cell is registered by q0");
        cell.name = registered_name;
        cell.gate = gate;
        cell.target_shape = shape;
        Some((cell, reference))
    }
    ...
    fn coverage_eligible(cell: Cell, reference: Reference, target: usize) -> bool {
        match reference {
            Reference::Q0 => !cell.controls && cell.signal == Signal::Regular,
            Reference::Q1 => {
                target != 2
                    && !cell.controls
    ...
    fn run_q1_matrix(arguments: &[String]) {
        ...
        for replication in start..start + replications {
            let seed = semantic_seed(Q1_MASTER_SEED, cell.name, dimension, replication);
            let generated = make_problem(cell, dimension, seed);
            match run_q1_attached(cell, reference, &generated, settings, seed) {
                Ok((result, baseline)) => {
    ...
    > {
        ...
        let prepared = prepare_grouped_structured_component_inference(
            ...
            StructuredVarianceOptions {
                seed: seed ^ 0xa51e_79d3_6c42_f807,
    ...
    fn common_success_json(
        ...
        format!(
            concat!(
                ...
                "\"confidence_lower\":{:.17e},\"confidence_upper\":{:.17e},",
                ...
                "\"second_residual\":{:.17e},\"minimum_active_terms\":{},",
                ...
                for target in 0..4 {
                    ...
                    println!(
                        concat!(
                            ...
                            "\"remainder_trace_mcse\":{:.17e},\"curvature\":{:.17e},",
                            "\"critical_value\":{:.17e},\"leading_f_statistic\":{:.17e},",
                            "\"remainder_influence_concentration\":{:.17e},",
                            "\"q1_point_identity_error\":{:.17e}}}"
                        ),
                        common,
                        inference.critical_simulations,
                        value.leading_score,
