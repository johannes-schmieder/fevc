...
const V5_CONFIRMATION_SEED: u64 = 0xc247_f908_3ae1_6d5b;
const RC_PIPELINE_SEED: u64 = 0x9816_ac5d_72b0_f3e4;
...
fn main() {
    ...
    if arguments.first().map(String::as_str) == Some("confirmation-matrix-v5") {
        run_confirmation_matrix(&arguments[1..], V5_CONFIRMATION_SEED);
        return;
    }
    if arguments.first().map(String::as_str) == Some("pipeline-matrix-rc-v1") {
        run_confirmation_matrix(&arguments[1..], RC_PIPELINE_SEED);
        return;
    }
    if arguments.first().map(String::as_str) == Some("design-preflight-rc-v1") {
        run_rc_preflight();
        return;
    }
...
fn run_rc_preflight() {
    for cell in cells(Profile::Confirmation) {
        let design = make_design(cell.k, cell.controls, cell.dominant, cell.beta_zero);
        let variance = make_variance(&design, cell.variance);
        let maker_minimum = design
            .maker_inverse
            .iter()
            .map(|x| 1.0 / x)
            .fold(f64::INFINITY, f64::min);
        let variance_minimum = variance.iter().copied().fold(f64::INFINITY, f64::min);
        for target in 0..4 {
            println!(
                "{{\"schema\":\"fevc-rc-observation-preflight-v1\",\"cell\":\"{}\",\"k\":{},\"target\":\"{}\",\"observations\":{},\"parameters\":{},\"maker_minimum\":{:.17e},\"variance_minimum\":{:.17e},\"mean_leading_share\":{:.17e},\"mean_remainder_share\":{:.17e},\"variance_fold_seed\":{}}}",
                cell.name, cell.k, target_name(target), design.rows, design.parameters,
                maker_minimum, variance_minimum, design.leading_share[target],
                design.remainder_share[target], INTERVAL_SEED,
            );
        }
    }
}

fn run_confirmation_matrix(arguments: &[String], master_seed: u64) {
    assert_eq!(
        arguments.len(),
        4,
    ...
    run_confirmation_matrix_cell(cell, start, replications, master_seed);
}
...
fn emit_confirmation_failure(
    ...
    println!(
        "{{\"schema\":\"fevc-rc-observation-inference-row-v1\",\"cell\":\"{}\",\"k\":{},\"replication\":{},\"target\":\"{}\",\"semantic_seed\":{},\"status\":\"{}\"}}",
        cell.name,
        cell.k,
        replication,
        target_name(target),
...
fn emit_confirmation_success(
    ...
    curvature: f64,
    outer_fold_fingerprint: u64,
) {
    println!(
        "{{\"schema\":\"fevc-rc-observation-inference-row-v1\",\"cell\":\"{}\",\"gate\":\"{}\",\"k\":{},\"controls\":{},\"dominant\":{},\"variance_model\":\"{}\",\"variance_dgp\":\"{:?}\",\"error_dgp\":\"{:?}\",\"reference\":\"{:?}\",\"beta\":\"{}\",\"replication\":{},\"target\":\"{}\",\"semantic_seed\":{semantic_seed},\"status\":\"success\",\"point_error\":{point_error:.17e},\"estimated_sd\":{estimated_sd:.17e},\"covered\":{},\"lower_miss\":{},\"upper_miss\":{},\"interval_width\":{:.17e},\"leading_variance\":{leading_variance:.17e},\"remainder_variance\":{remainder_variance:.17e},\"leading_remainder_covariance\":{leading_remainder_covariance:.17e},\"remainder_identity_error\":{remainder_identity_error:.17e},\"critical\":{critical:.17e},\"leading_share\":{:.17e},\"remainder_share\":{:.17e},\"floor_share\":{floor_share:.17e},\"boundary_share\":{boundary_share:.17e},\"maximum_boundary_excess\":{maximum_boundary_excess:.17e},\"maximum_prediction_leverage\":{maximum_prediction_leverage:.17e},\"minimum_fitted_rcond\":{minimum_fitted_rcond:.17e},\"leading_eigenvalue\":{leading_eigenvalue:.17e},\"curvature\":{curvature:.17e},\"outer_fold_fingerprint\":{outer_fold_fingerprint}}}",
        cell.name,
        cell.gate,
        cell.k,
        cell.controls,
...
fn run_confirmation_matrix_cell(cell: Cell, start: usize, replications: usize, master_seed: u64) {
    let design = make_design(cell.k, cell.controls, cell.dominant, cell.beta_zero);
    let variance = make_variance(&design, cell.variance);
    let mean = matrix_vector(&design.x, design.rows, design.parameters, &design.beta);
    for replication in start..start + replications {
        let replication_seed = semantic_seed(master_seed, cell.name, cell.k, replication);
        ...
        let outer_fold_fingerprint = fitted
            .outer_fold
            .iter()
            .fold(0xcbf2_9ce4_8422_2325_u64, |hash, &fold| {
                (hash ^ u64::from(fold)).wrapping_mul(0x100_0000_01b3)
            });
        let mut entity_folds = std::collections::BTreeMap::new();
        for (&entity, &fold) in design.fold_entity.iter().zip(&fitted.outer_fold) {
            if let Some(previous) = entity_folds.insert(entity, fold) {
                assert_eq!(
                    previous, fold,
                    "design-equivalent rows must stay in one fold"
                );
            }
        }
