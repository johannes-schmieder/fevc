...
if (( $# != 3 )); then
  printf 'usage: deploy_rc_match_q0.sh SOURCE_ROOT RUN_ID MANIFEST\n' >&2
  exit 198
fi
...
printf 'FEVC_RC_MATCH_Q0_CAMPAIGN_DEPLOYED run_dir=%s source_commit=%s bundle_sha256=%s manifest_sha256=%s\n' \
