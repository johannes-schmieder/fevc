...
if (( $# != 1 )); then
  printf 'usage: submit_rc_observation_inference.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
build_script=${run_dir}/source/rust/stata_backend/scc/run_rc_observation_inference_build.sge
task_script=${run_dir}/source/rust/stata_backend/scc/run_rc_observation_inference_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_rc_observation_inference_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
...
printf 'FEVC_RC_OBSERVATION_INFERENCE_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
