...
if (( $# != 3 )); then
  printf 'usage: deploy_structured_inference_campaign.sh SOURCE_ROOT RUN_ID MANIFEST\n' >&2
  exit 198
fi
...
temporary=$(mktemp -d "${TMPDIR:-/tmp}/fevc-structured-bundle.XXXXXX")
...
printf 'FEVC_STRUCTURED_CAMPAIGN_DEPLOYED run_dir=%s source_commit=%s bundle_sha256=%s manifest_sha256=%s\n' \
