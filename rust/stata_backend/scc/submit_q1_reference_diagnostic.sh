...
if (( $# != 1 )); then
  printf 'usage: submit_q1_reference_diagnostic.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
assert value["schema"] == "fevc-q1-reference-campaign-v4"
...
build_script=${run_dir}/source/rust/stata_backend/scc/run_q1_reference_build.sge
task_script=${run_dir}/source/rust/stata_backend/scc/run_q1_reference_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_q1_reference_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
...
printf 'FEVC_Q1_V4_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
