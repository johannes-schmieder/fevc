...
if (( $# != 1 )); then
  printf 'usage: submit_match_inference_q0_campaign.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
build_script=${run_dir}/source/rust/stata_backend/scc/run_match_inference_q0_build.sge
task_script=${run_dir}/source/rust/stata_backend/scc/run_match_inference_q0_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_match_inference_q0_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
...
printf 'FEVC_MATCH_Q0_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
