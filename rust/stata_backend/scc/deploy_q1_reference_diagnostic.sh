...
if (( $# != 3 )); then
  printf 'usage: deploy_q1_reference_diagnostic.sh SOURCE_ROOT RUN_ID MANIFEST\n' >&2
  exit 198
fi
...
[[ -z $(git -C "${source_root}" status --porcelain --untracked-files=all) ]] || {
  printf 'SCC V4 diagnostic deployment requires a clean committed checkout\n' >&2
  exit 198
}
test -f "${manifest}"
...
temporary=$(mktemp -d "${TMPDIR:-/tmp}/fevc-q1-v4-bundle.XXXXXX")
...
mkdir -p "${run_dir}/input" "${run_dir}/logs/tasks" \
  "${run_dir}/output/tasks" "${run_dir}/receipts" \
...
printf 'FEVC_Q1_V4_DEPLOYED run_dir=%s source_commit=%s bundle_sha256=%s manifest_sha256=%s\n' \
