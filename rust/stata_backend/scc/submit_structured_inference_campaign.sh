...
if (( $# != 1 )); then
  printf 'usage: submit_structured_inference_campaign.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
task_script=${run_dir}/source/rust/stata_backend/scc/run_structured_inference_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_structured_inference_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
...
printf 'FEVC_STRUCTURED_CAMPAIGN_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
