...
if (( $# != 3 )); then
  printf 'usage: deploy_rc_observation_inference.sh SOURCE_ROOT RUN_ID MANIFEST\n' >&2
  exit 198
fi
...
printf 'FEVC_RC_OBSERVATION_INFERENCE_CAMPAIGN_DEPLOYED run_dir=%s source_commit=%s bundle_sha256=%s manifest_sha256=%s\n' \
