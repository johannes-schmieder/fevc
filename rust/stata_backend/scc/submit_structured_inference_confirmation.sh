...
if (( $# != 1 )); then
  printf 'usage: submit_structured_inference_confirmation.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
build_script=${run_dir}/source/rust/stata_backend/scc/run_structured_inference_build.sge
task_script=${run_dir}/source/rust/stata_backend/scc/run_structured_inference_confirmation_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_structured_inference_confirmation_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
test -x "${aggregate_script}"

build_id=$(qsub -terse -P welfgr -pe omp 1 -l h_rt=01:00:00 \
...
tasks_id=$(qsub -terse -P welfgr -pe omp 1 -l h_rt=01:00:00 \
...
aggregate_id=$(qsub -terse -P welfgr -pe omp 1 -l h_rt=00:30:00 \
...

{
  ...
  printf 'build\t%s\t\tscalar\t1\t01:00:00\t4G\n' "${build_id}"
  printf 'tasks\t%s\t%s\t1-%s\t1\t01:00:00\t4G\n' \
    "${tasks_id}" "${build_id}" "${task_count}"
  printf 'aggregate\t%s\t%s\tscalar\t1\t00:30:00\t4G\n' \
    "${aggregate_id}" "${tasks_id}"
} > "${run_dir}/submissions/ledger.tsv"
printf 'FEVC_STRUCTURED_CONFIRMATION_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
