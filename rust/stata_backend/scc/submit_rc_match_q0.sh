...
if (( $# != 1 )); then
  printf 'usage: submit_rc_match_q0.sh RUN_DIR\n' >&2
  exit 198
fi
run_dir=$1
...
build_script=${run_dir}/source/rust/stata_backend/scc/run_rc_match_q0_build.sge
task_script=${run_dir}/source/rust/stata_backend/scc/run_rc_match_q0_task.sge
aggregate_script=${run_dir}/source/rust/stata_backend/scc/run_rc_match_q0_aggregate.sge
test -x "${build_script}"
test -x "${task_script}"
...
printf 'FEVC_RC_MATCH_Q0_SUBMITTED build=%s tasks=%s aggregate=%s task_count=%s run_dir=%s\n' \
