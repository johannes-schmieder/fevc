*! kss_bc experimental compressed estimator engine 0.2.0 16aug2026
...
It consumes a prepared kssbc_scale_design.  The retained Stata rows are not
...
string scalar kssbc_scale_engine__version()
{
...
struct kssbc_scale_atom_provider
{
    string scalar status
    string scalar message
...
struct kssbc_scale_engine_matrix_atoms
{
    real matrix leverage_unit
    real matrix target_stratum
}

struct kssbc_scale_rng_context
{
    string scalar status
    string scalar message
    ...
    struct kssbc_rng__cursor scalar leverage_cursor
    struct kssbc_rng__cursor scalar target_cursor
}

struct kssbc_scale_unit_adjust
{
    string scalar status
    string scalar message
...
struct kssbc_scale_engine_result
{
    string scalar status
    string scalar message
...
struct kssbc_scale_route_context
{
    string scalar status
    string scalar message
    ...
    struct kssbc_scale_engine_result scalar last
}

struct kssbc_scale_engine_atom_batch scalar kssbc_scale_eng__empty_atoms()
{
    struct kssbc_scale_engine_atom_batch scalar out
...
struct kssbc_scale_engine_result scalar kssbc_scale_eng__empty_result()
{
    struct kssbc_scale_engine_result scalar out
...
struct kssbc_scale_engine_atom_batch scalar kssbc_scale_eng__mat_leverage(
    pointer scalar context,
    real scalar first_probe,
    real scalar last_probe)
{
    struct kssbc_scale_engine_atom_batch scalar out
    pointer(struct kssbc_scale_engine_matrix_atoms scalar) scalar source

    out = kssbc_scale_eng__empty_atoms()
    source = context
...
struct kssbc_scale_engine_atom_batch scalar kssbc_scale_eng__mat_target(
    pointer scalar context,
    real scalar first_probe,
    real scalar last_probe)
{
...
struct kssbc_scale_atom_provider scalar kssbc_scale_eng__mat_provider(
    pointer(struct kssbc_scale_engine_matrix_atoms scalar) scalar source)
{
    struct kssbc_scale_atom_provider scalar out
    ...
    out.context = source
    out.leverage = &kssbc_scale_eng__mat_leverage()
    out.target = &kssbc_scale_eng__mat_target()
    if (source != NULL) {
        out.status = "CONVERGED"
...
struct kssbc_scale_engine_atom_batch scalar kssbc_scale_eng__rng_lev(
    pointer scalar context,
    real scalar first_probe,
    real scalar last_probe)
{
    ...
    pointer(struct kssbc_scale_rng_context scalar) scalar source
    ...
    generated = kssbc_rng__cursor_next(
...
struct kssbc_scale_engine_atom_batch scalar kssbc_scale_eng__rng_target(
    pointer scalar context,
    real scalar first_probe,
    real scalar last_probe)
{
...
struct kssbc_scale_atom_provider scalar kssbc_scale_eng__rng_provider(
    pointer(struct kssbc_scale_rng_context scalar) scalar source)
{
    struct kssbc_scale_atom_provider scalar out
    ...
    out.target_trials = J(0,1,.)
    out.context = source
    out.leverage = &kssbc_scale_eng__rng_lev()
    out.target = &kssbc_scale_eng__rng_target()
...
/* Stable scatter-add in semantic input order.  The group vector is dense and
   one based. */
real matrix kssbc_scale_engine__scatter_sum(
    real matrix values,
...
real matrix kssbc_scale_eng__center_target(
    real matrix first,
    real colvector mass_share)
{
    ...
    total = kssbc_scale_engine__column_sum(first)
...
real matrix kssbc_scale_eng__balance_score(
    real matrix score,
    real rowvector reference_scale)
{
    ...
    if (rows(out) > 1) preceding =
        kssbc_scale_engine__column_sum(out[1..(rows(out)-1),.])
...
real scalar kssbc_scale_eng__roundoff_gate()
{
    return(4096*2.2204460492503131e-16)
}

real scalar kssbc_scale_eng__aggregate_ok(
...
void kssbc_scale_eng__merge_col(
    pointer(real matrix) scalar subtotal,
    pointer(real matrix) scalar compensation,
    real scalar column,
...
real scalar kssbc_scale_eng__moment_width()
{
...
void kssbc_scale_eng__moment_add(
    pointer(real matrix) scalar subtotal,
    pointer(real matrix) scalar compensation,
    ...
    real matrix residual)
{
    ...
    width = kssbc_scale_eng__moment_width()
...
real matrix kssbc_scale_eng__moment_reduce(
    real matrix projection,
    real matrix residual)
{
...
real matrix kssbc_scale_eng__target_contract(
    ...
    real matrix prediction)
{
    ...
    width = kssbc_scale_eng__target_rows()
...
real scalar kssbc_scale_engine__dot(
    real colvector left,
...
real scalar kssbc_scale_eng__valid_atoms(
    real matrix atoms,
    real colvector trials)
{
    ...
    width = kssbc_scale_eng__atom_rows()
...
real rowvector kssbc_scale_engine__plugin(
    struct kssbc_scale_design scalar design,
    ...
    real colvector coefficient)
{
    ...
    worker_variance = kssbc_scale_engine__dot(
        design.cell_target_mass,(worker_effect:-worker_mean):^2)/mass
    firm_variance = kssbc_scale_engine__dot(
        design.cell_target_mass,(firm_effect:-firm_mean):^2)/mass
    covariance = kssbc_scale_engine__dot(
...
real rowvector kssbc_scale_engine__mcse(real matrix draws)
{
    ...
    mean_draw = kssbc_scale_engine__column_sum(draws):/rows(draws)
    out = J(1,cols(draws),.)
...
real scalar kssbc_scale_eng__identity_resid(
    real matrix values)
{
...
struct kssbc_scale_unit_adjust scalar kssbc_scale_eng__unit_adjust(
    real scalar projection_share,
    real scalar residual_share,
    real scalar finite_bias,
    real scalar finite_variance,
    ...
    real scalar block_tolerance)
{
    struct kssbc_scale_unit_adjust scalar out
...
struct kssbc_scale_engine_result scalar kssbc_scale_eng__record(
    struct kssbc_scale_engine_result scalar out,
    ...
    real scalar batch_id)
{
    ...
    out.solver_rhs_diagnostics = out.solver_rhs_diagnostics \
        kssbc__solver_trace_rows(
            stage,batch_id,solved.rhs_iterations,solved.rhs_relres)
    return(out)
}

struct kssbc_scale_engine_result scalar kssbc_scale_eng__run_prepared(
    struct kssbc_scale_design scalar design,
    struct kssbc_fe_design scalar base,
    ...
    real scalar block_tolerance)
{
    ...
    out = kssbc_scale_eng__empty_result()
    ...
    cell_frequency_from_units = kssbc_scale_engine__scatter_sum(
        design.unit_frequency,design.unit_cell,design.coefficient_cells)
    cell_outcome_from_units = kssbc_scale_engine__scatter_sum(
        design.unit_outcome_sum,design.unit_cell,design.coefficient_cells)
    cell_outcome_abs_from_units = kssbc_scale_engine__scatter_sum(
        abs(design.unit_outcome_sum),design.unit_cell,
        design.coefficient_cells)
    cell_frequency_from_strata = kssbc_scale_engine__scatter_sum(
    ...
    solved = kssbc__fe_solve_matrix_backend(
        base,kssbc__fe_transpose_full(base,design.cell_outcome_sum),
    ...
    out = kssbc_scale_eng__record(out,solved,2,0)
    out.coefficient = solved.coefficient
    out.fitted_cell = solved.prediction
    plugin = kssbc_scale_engine__plugin(design,base,out.coefficient)
    out.weighted_rss = kssbc_scale_engine__column_sum(
    ...
    for (batch_start=1; batch_start<=probes;
        ...
        cell_atoms = kssbc_scale_engine__scatter_sum(
            unit_atoms,design.unit_cell,design.coefficient_cells)
        leverage_rhs = kssbc__fe_transpose_full(base,cell_atoms)
        solved = kssbc__fe_solve_matrix_backend(
        ...
        out = kssbc_scale_eng__record(out,solved,4,batch_start)
    ...
    for (cell=1; cell<=groups; cell++) {
        unit_adjustment = kssbc_scale_eng__unit_adjust(
    ...
    out.cell_correction_weight = kssbc_scale_engine__scatter_sum(
    ...
    for (batch_start=1; batch_start<=probes;
        ...
        target_first = sqrt(design.strata.per_copy_mass:/target_mass):*
            target_atoms
        target_first = kssbc_scale_engine__scatter_sum(
            target_first,design.strata.cell,design.coefficient_cells)
        target_reference_scale =
            kssbc_scale_engine__column_sum(abs(target_first)) +
            abs(kssbc_scale_engine__column_sum(target_first))
        direction_cell = kssbc_scale_eng__center_target(
        ...
        full_target_score = kssbc__fe_transpose_full(base,direction_cell)
        full_target_score[1..base.worker_levels,.] =
            kssbc_scale_eng__balance_score(
                full_target_score[1..base.worker_levels,.],
                target_reference_scale)
        ...
        out = kssbc_scale_eng__record(out,solved,5,batch_start)
        timer_on(99)
        target_prediction = solved.prediction
        target_contractions = kssbc_scale_eng__target_contract(
    ...
    timer_on(99)
    correction = kssbc_scale_engine__column_sum(out.target_draws):/probes
    corrected = plugin-correction
    out.plugin = plugin
    out.correction = correction
    out.corrected = corrected
    out.numerical_mcse = kssbc_scale_engine__mcse(out.target_draws)
    identity_residual = (
        kssbc_scale_eng__identity_resid(plugin),
        kssbc_scale_eng__identity_resid(correction),
        kssbc_scale_eng__identity_resid(corrected),
        kssbc_scale_eng__identity_resid(out.target_draws))
    ...
    out.rng_seconds = kssbc__timer_seconds(94)
    out.correction_seconds = kssbc__timer_seconds(99)
    out.total_seconds =
...
struct kssbc_scale_engine_result scalar kssbc_scale_engine__run(
    struct kssbc_scale_design scalar design,
    struct kssbc_solver_backend scalar backend,
    struct kssbc_scale_atom_provider scalar provider,
    real scalar probes,
    ...
    real scalar block_tolerance)
{
    ...
    return(kssbc_scale_eng__run_prepared(
...
struct kssbc_result scalar kssbc_scale_eng__as_result(
    struct kssbc_scale_engine_result scalar source,
    real scalar setup_seconds)
{
    struct kssbc_result scalar out

    if (source.status != "CONVERGED") {
        return(kssbc__failure(source.status,source.message))
    }
...
struct kssbc_scale_route_context scalar kssbc_scale_eng__route_context(
    ...
    real scalar block_tolerance)
{
    struct kssbc_scale_route_context scalar out
...
struct kssbc_result scalar kssbc_scale_eng__route_callback(
    pointer scalar context,
    struct kssbc_fe_design scalar base,
    struct kssbc_solver_backend scalar backend,
    real scalar setup_seconds)
{
    pointer(struct kssbc_scale_route_context scalar) scalar source
    ...
    return(kssbc_scale_eng__as_result((*source).last,setup_seconds))
}

/* Pass this pointer and &kssbc_scale_route_context to the optional callback
   arguments of kssbc_solver__jla_routed().  The router must itself be called
...
pointer scalar kssbc_scale_eng__callback_ptr()
{
    return(&kssbc_scale_eng__route_callback())
}

struct kssbc_scale_engine_result scalar kssbc_scale_eng__run_atoms(
    struct kssbc_scale_design scalar design,
    struct kssbc_solver_backend scalar backend,
    ...
    real scalar block_tolerance)
{
    struct kssbc_scale_engine_matrix_atoms scalar source
    struct kssbc_scale_atom_provider scalar provider

    source.leverage_unit = leverage_unit_atoms
    source.target_stratum = target_stratum_atoms
    provider = kssbc_scale_eng__mat_provider(&source)
    if (cols(leverage_unit_atoms) != cols(target_stratum_atoms)) {
        return(kssbc_scale_eng__empty_result())
    }
    return(kssbc_scale_engine__run(
