...
if `converged' {
    file write `marker' ///
        "KSS_BC_NUMOPT_CONVERGED `route' `scenario' `source_commit'" _n
}
else {
    file write `marker' ///
        "KSS_BC_NUMOPT_TYPED_FAILURE `route' `scenario' `route_status' `source_commit'" _n
}
file close `marker'
...
di as result "KSS_BC NUMOPT BENCHMARK PASS: `route' `scenario'"
exit 0
