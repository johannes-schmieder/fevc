function kss_bc_dense_oracle(output_dir, source_commit)
...
if nargin ~= 2 || strlength(string(source_commit)) < 7
    error('kss_bc:invalidInput','output_dir and source_commit are required');
end
...
if max(abs(plugin-expected_plugin)) > 2e-10 || ...
    ...
    error('kss_bc:oracleMismatch','dense MATLAB oracle missed registered values');
end
fprintf('KSS_BC MATLAB ORACLE PASS: %s\n',source_commit);
end
