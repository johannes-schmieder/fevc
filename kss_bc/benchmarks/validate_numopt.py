...
def validate_success(
    ...
    require(f"KSS_BC_NUMOPT_CONVERGED {route} {scenario} {commit}" in marker, "bad marker")
    rss: int | None = None
    if require_scc:
...
def validate_easy_cmg_failure(run_dir: Path, commit: str, require_scc: bool) -> None:
    ...
    require("KSS_BC_NUMOPT_TYPED_FAILURE cmg easy" in marker, "bad easy/CMG marker")
    if require_scc:
...
def main() -> int:
    ...
    print("KSS_BC NUMOPT EVIDENCE PASS; automatic routing remains disabled")
    return 0
