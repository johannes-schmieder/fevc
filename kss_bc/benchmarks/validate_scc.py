...
def validate_portability(root: Path, commit: str) -> None:
    ...
    require("KSS_BC SCC PORTABILITY PASS" in wrapper_log, "portability wrapper marker absent")
    full_log = text(root / "portability" / "full_suite.application.txt")
    install_log = text(root / "portability" / "install.application.txt")
    require("KSS_BC TEST SUITE PASS: full" in full_log, "full Stata suite marker absent")
    require("KSS_BC INSTALL TEST PASS" in install_log, "install marker absent")
...
def validate_oracle(root: Path, commit: str) -> None:
    ...
    require("KSS_BC SCC ORACLE PASS" in wrapper_log, "oracle wrapper marker absent")
    require("KSS_BC MATLAB ORACLE PASS" in matlab_log, "MATLAB oracle marker absent")
    require("KSS_BC STATA/MATLAB ORACLE PASS" in stata_log, "paired oracle marker absent")
    require(
        text(root / "oracle" / "oracle.wrapper.pass").strip()
        == f"KSS_BC_ORACLE_PASS {commit}",
...
def validate_scale(root: Path, commit: str, scenario: str) -> None:
    ...
    require(f"KSS_BC SCC SCALE PASS: {scenario}" in wrapper_log, f"{scenario} wrapper marker absent")
    require(f"KSS_BC BENCHMARK PASS: {scenario}" in app_log, f"{scenario} application marker absent")
    require(
        text(job_dir / f"{scenario}.wrapper.pass").strip()
        == f"KSS_BC_SCALE_PASS {scenario} {commit}",
...
def main() -> int:
    ...
    for job in args.jobs:
        ...
        print(f"PASS {job}")
    print("KSS_BC SCC EVIDENCE PASS")
    return 0
