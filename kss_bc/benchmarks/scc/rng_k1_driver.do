...
confirm file "`c(pwd)'/kss_bc/kss_bc_rng.mata"
quietly do "`c(pwd)'/kss_bc/kss_bc_rng.mata"
...
string colvector rngk1__capture_streams(real colvector streams)
{
    ...
    restore_rc = kssbc_rng__restore(saved)
...
real scalar rngk1__full_equal(
    struct kssbc_rng__full_snapshot scalar left,
    struct kssbc_rng__full_snapshot scalar right)
{
...
{
    struct rngk1_evidence scalar out
    struct kssbc_rng__result scalar plev, ptgt, dlev, dtgt
    struct kssbc_rng__result scalar first, second, shuffled
    struct kssbc_rng__result scalar pone_lev, pone_tgt, pfour_lev, pfour_tgt
    struct kssbc_rng__result scalar done_lev, done_tgt, dfour_lev, dfour_tgt
    struct kssbc_rng__result scalar large
    struct kssbc_rng__result scalar prod_p_lev, prod_p_tgt
    struct kssbc_rng__result scalar prod_d_lev, prod_d_tgt
    struct kssbc_rng__cursor scalar cursor_all, cursor_split
    struct kssbc_rng__result scalar cursor_whole, cursor_first, cursor_second
    struct kssbc_rng__call_shape_result scalar shape, boundary_shape
    struct kssbc_rng__benchmark_result scalar lev_timing, tgt_timing
    struct kssbc_rng__snapshot scalar saved
    struct kssbc_rng__snapshot scalar caller9_before, caller9_after
    ...
    out = rngk1__empty()
    pone_lev = kssbc_rng__empty_result()
    pone_tgt = kssbc_rng__empty_result()
    pfour_lev = kssbc_rng__empty_result()
    pfour_tgt = kssbc_rng__empty_result()
    done_lev = kssbc_rng__empty_result()
    done_tgt = kssbc_rng__empty_result()
    dfour_lev = kssbc_rng__empty_result()
    dfour_tgt = kssbc_rng__empty_result()
    prod_p_lev = kssbc_rng__empty_result()
    ...
    out.module_contract_ok =
        kssbc_rng__api_level() == 2 &
        kssbc_rng__build_id() ==
            "kss-bc-rng-k1-mt64s-complete-guard-v2" &
        kssbc_rng__invariant_version() == "KSS-RNG-K1-INVARIANT-V1" &
        kssbc_rng__k1_recommendation() ==
            "per_domain_stream_cursor" &
        kssbc_rng__max_binomial_trials() == 100000000000 &
        kssbc_rng__maximum_exact_integer() == 2^53-1
    ...
    if (!rc1 & st_numscalar("c(processors)") == 1) {
        pone_lev = kssbc_rng__generate(
            "per_probe_stream",8675309,"leverage",1,3,keys,trials)
        pone_tgt = kssbc_rng__generate(
            "per_probe_stream",8675309,"target",1,3,keys,trials)
    }
    rc4 = _stata("set processors 4",1,1)
    if (!rc4 & st_numscalar("c(processors)") == 4) {
        pfour_lev = kssbc_rng__generate(
            "per_probe_stream",8675309,"leverage",1,3,keys,trials)
        pfour_tgt = kssbc_rng__generate(
            "per_probe_stream",8675309,"target",1,3,keys,trials)
    }
    ...
    lev_timing = kssbc_rng__benchmark(
        8675309,"leverage",3,keys,trials,3)
    tgt_timing = kssbc_rng__benchmark(
    ...
    if (!selected9_rc) {
        caller9_before = kssbc_rng__capture()
        lev78_before = kssbc_rng__capture_streams((7 \ 8))
        tgt78_before = kssbc_rng__capture_streams((16390 \ 16391))
        first = kssbc_rng__generate(
            "per_probe_stream",8675309,"leverage",7,2,keys,trials)
        second = kssbc_rng__generate(
            "per_probe_stream",8675309,"target",7,2,keys,trials)
        lev78_after = kssbc_rng__capture_streams((7 \ 8))
        tgt78_after = kssbc_rng__capture_streams((16390 \ 16391))
        caller9_after = kssbc_rng__capture()
    ...
    if (cursor_all.status == "OK" & cursor_split.status == "OK") {
        cursor_whole = kssbc_rng__cursor_next(&cursor_all,3)
    ...
    rc1 = _stata("set processors 1",1,1)
    if (!rc1 & st_numscalar("c(processors)") == 1) {
        done_lev = kssbc_rng__generate(
            "per_domain_stream",8675309,"leverage",1,3,keys,trials)
        done_tgt = kssbc_rng__generate(
            "per_domain_stream",8675309,"target",1,3,keys,trials)
    }
    rc4 = _stata("set processors 4",1,1)
    if (!rc4 & st_numscalar("c(processors)") == 4) {
        dfour_lev = kssbc_rng__generate(
            "per_domain_stream",8675309,"leverage",1,3,keys,trials)
        dfour_tgt = kssbc_rng__generate(
            "per_domain_stream",8675309,"target",1,3,keys,trials)
    }
    ...
    prod_after = kssbc_rng__capture_streams(prod_streams)
    ...
    saved = kssbc_rng__capture()
    if (kssbc_rng__set_stream_seed(30001,8675309) == 0) {
        timer_clear(81)
        timer_on(81)
        scalar_draw = kssbc_rng__draw_probe_scalar(production_trials)
        timer_off(81)
        ...
        if (kssbc_rng__set_stream_seed(30001,8675309) == 0) {
            timer_clear(82)
            timer_on(82)
            production_trials = kssbc_rng__draw_probe_vector(
                production_trials)
            timer_off(82)
    ...
    out.exact_integer_gate_pass =
        kssbc_rng__trials_ok((2^53-1)) == 1 &
        kssbc_rng__trials_ok((2^53-1 \ 1)) == 0 &
        rows(kssbc_rng__draw_probe_vector(
            (kssbc_rng__max_binomial_trials()+1))) == 0
    saved = kssbc_rng__capture()
    if (kssbc_rng__set_stream_seed(30002,8675309) == 0) {
        chunked = kssbc_rng__binomial_atom_scalar(
    ...
    if (large.status == "OK") {
        out.large_generation_pass = large.chunk_calls == 2 &
            abs(large.atoms[1,1]) <= kssbc_rng__max_binomial_trials()+7 &
            mod(large.atoms[1,1]+
                kssbc_rng__max_binomial_trials()+7,2) == 0
    }
...
void rngk1__write_snapshot(
    string scalar path,
    struct kssbc_rng__full_snapshot scalar snapshot,
    real colvector streams,
    string colvector states)
{
    ...
    struct rngk1_evidence scalar evidence)
{
    ...
    rngk1__put(file,"rng_api_level",
        rngk1__number(kssbc_rng__api_level()))
    rngk1__put(file,"rng_build_id",kssbc_rng__build_id())
    rngk1__put(file,"rng_invariant_version",
        kssbc_rng__invariant_version())
...
RNGK1_BEFORE = kssbc_rng__capture_full()
RNGK1_STREAMS_BEFORE = rngk1__capture_streams(RNGK1_ALL_STREAMS)
st_local("rngk1_guard_begin_rc",strofreal(kssbc_rng__guard_begin()))
end
...
mata: st_local("rngk1_guard_restore_rc", strofreal(kssbc_rng__guard_restore()))

mata:
RNGK1_AFTER_GUARD = kssbc_rng__capture_full()
st_local("rngk1_all_stream_restore_rc",strofreal(rngk1__restore_streams(
    RNGK1_ALL_STREAMS,RNGK1_STREAMS_BEFORE)))
RNGK1_AFTER = kssbc_rng__capture_full()
