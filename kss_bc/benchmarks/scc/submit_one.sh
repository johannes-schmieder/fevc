...
case "$run_dir" in
  ...
  *) printf '%s\n' "RUN_DIR must be under /projectnb/welfgr/kss-bc/runs/" >&2; exit 198 ;;
esac
case "$source_dir" in
  "$run_dir"/source) ;;
...
test -f "$source_dir/kss_bc/kss_bc.ado"
...
case "$job" in
  portability)
    script="$source_dir/kss_bc/benchmarks/scc/run_portability.sge"
    job_id=$(qsub -terse -P welfgr -pe omp 4 -l h_rt=02:00:00 \
      -l mem_per_core=4G -j y -o "$run_dir/logs/portability.stdout.txt" \
      -v "$common_env" "$script")
    ;;
  oracle)
    script="$source_dir/kss_bc/benchmarks/scc/run_oracle.sge"
    job_id=$(qsub -terse -P welfgr -pe omp 4 -l h_rt=02:00:00 \
  ...
  smoke|medium|large)
    ...
    script="$source_dir/kss_bc/benchmarks/scc/run_scale.sge"
    case "$job" in
