...
allowlist="$source_root/kss_bc/benchmarks/prod_bundle_allowlist.txt"
"$source_root/.venv/bin/python" \
  "$source_root/kss_bc/benchmarks/validate_prod_scc.py" \
  --plan "$source_root/kss_bc/benchmarks/prod_experiments.tsv" --static
temporary=$(mktemp -d "${TMPDIR:-/tmp}/kss-prod-bundle.XXXXXX")
trap 'rm -rf "$temporary"' EXIT
bundle_sha=$(
  "$source_root/.venv/bin/python" \
    "$source_root/kss_bc/benchmarks/build_prod_bundle.py" \
