...
usage() {
  printf '%s\n' \
    ...
    "cold/warm: submit_matlab_scale.sh MODE RUN_DIR SOURCE_DIR SOURCE_COMMIT BUNDLE_ARCHIVE BUNDLE_SHA256 SOURCE_MANIFEST LABEL INPUT_CSV INPUT_SHA256 CASE_JSON CASE_SHA256 CONTRACT_SHA256 MATLAB_ROOT TIMEOUT_SECONDS TIMEOUT_BASIS" >&2
  exit 198
}
...
case "$source_dir" in
  "/projectnb/welfgr/kss-bc/bundles/$bundle_sha/source") ;;
  *) exit 198 ;;
esac
case "$bundle_archive" in
  "/projectnb/welfgr/kss-bc/bundles/$bundle_sha/$bundle_sha.tar.gz") ;;
  *) exit 198 ;;
esac
case "$source_manifest" in
  "/projectnb/welfgr/kss-bc/bundles/$bundle_sha/$bundle_sha.files.sha256") ;;
  *) exit 198 ;;
esac
...
if [[ "$stage" == prepare ]]; then
  ...
  mem_per_core_gib=4
  stata_processors=4
  script="$source_dir/kss_bc/benchmarks/matlab_scale/run_prepare_fixed_sample.sge"
  ...
  test -f "$run_dir/matlab_scale/$label/prepare/wrapper.json"
  grep -Fq '"status": "PASS"' \
  ...
  contract_path=\
"$source_dir/kss_bc/benchmarks/matlab_scale/source_contract.json"
...
raw_job_id=$(qsub -terse -P welfgr -pe omp "$requested_slots" \
  -l "mem_per_core=${mem_per_core_gib}G" \
  -l "h_rt=$scheduler_hard_wall_hms" -j y -m a \
  -N "kms_${stage}" -o "$run_dir/logs/$record.stdout.txt" \
