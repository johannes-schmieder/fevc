...
def verify_source_bundle(source_dir, source_commit, bundle_sha256):
    ...
    validator = source_dir / "kss_bc/benchmarks/scc/validate_kss_scale.py"
    driver = source_dir / "kss_bc/benchmarks/scc/kss_scale_driver.do"
    require(
...
def expected_admission(report):
    ...
    return {
        "receipt_version": "KSS-SCALE-ADMISSION-V1",
        ...
        "phase_peak_complete": str(int(phase_rss["phase_peak_complete"])),
        "next_scale_allowed": str(int(reconciliation["next_scale_allowed"])),
    }
...
def build(args):
    ...
    require(report["status"] == "KSS_SCALE_VALIDATION_PASS", "KSS run was not accepted")
