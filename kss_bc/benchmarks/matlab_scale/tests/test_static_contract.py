...
def test_fixed_sample_preparation_is_scalar_stata_and_separates_reservation():
    ...
    assert "kssbc_scale_fixture" in driver
...
def test_scale_deployer_syntax_checks_both_matlab_entry_points():
    deployer = (PACKAGE.parent / "scc" / "deploy_scale_bundle.sh").read_text(
        encoding="utf-8"
    )
    assert 'matlab_scale/run_prepare_fixed_sample.sge"' in deployer
    assert 'matlab_scale/submit_matlab_scale.sh"' in deployer
...
def test_reference_converter_is_bundled_and_derives_kss_kind():
    ...
    assert '"kind": "stata_kss_bc"' in converter
    ...
    assert "kss_bc/benchmarks/matlab_scale/build_reference_receipt.py\n" in allowlist
