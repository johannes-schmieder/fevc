...
struct kssbc_cmg__aggregation_result
{
    string scalar status
    string scalar message
...
struct kssbc_cmg__hierarchy
{
    string scalar status
    string scalar message
    struct kssbc_cmg__options scalar options
    real scalar n_level
    pointer(struct kssbc_cmg__level scalar) rowvector level
...
struct kssbc_cmg__apply_result
{
    string scalar status
    string scalar message
    real matrix value
...
struct kssbc_cmg__preflight_result
{
    string scalar status
    string scalar message
    string scalar route
...
struct kssbc_cmg__route_result
{
    string scalar status
    string scalar message
    string scalar route
...
struct kssbc_cmg__cells scalar kssbc_cmg__empty_cells()
{
    struct kssbc_cmg__cells scalar out
...
struct kssbc_cmg__graph scalar kssbc_cmg__empty_graph()
{
...
struct kssbc_cmg__options scalar kssbc_cmg__options_default()
{
    struct kssbc_cmg__options scalar out
...
struct kssbc_cmg__options scalar kssbc_cmg__options_resource(
    ...
    real scalar planned_rhs)
{
    ...
    out = kssbc_cmg__options_default()
...
struct kssbc_cmg__forest scalar kssbc_cmg__empty_forest()
{
...
struct kssbc_cmg__aggregation_result scalar kssbc_cmg__empty_aggregation()
{
...
struct kssbc_cmg__hierarchy scalar kssbc_cmg__empty_hierarchy()
{
    struct kssbc_cmg__hierarchy scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy"
    out.options = kssbc_cmg__options_default()
    out.n_level = 0
...
struct kssbc_cmg__level scalar kssbc_cmg__empty_level()
{
    struct kssbc_cmg__level scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy level"
    out.graph = kssbc_cmg__empty_graph()
    out.component = J(0,1,.)
    out.n_component = 0
...
pointer(struct kssbc_cmg__level scalar) scalar kssbc_cmg__store_level(
    struct kssbc_cmg__level scalar value)
{
    struct kssbc_cmg__level scalar stored

    stored = value
    return(&stored)
}

pointer(real colvector) scalar kssbc_cmg__store_colvector(
...
pointer(real matrix) scalar kssbc_cmg__store_matrix(real matrix value)
{
    real matrix stored
...
struct kssbc_cmg__apply_result scalar kssbc_cmg__empty_apply_result()
{
    struct kssbc_cmg__apply_result scalar out
...
struct kssbc_cmg__preflight_result scalar kssbc_cmg__empty_preflight()
{
    struct kssbc_cmg__preflight_result scalar out
...
struct kssbc_cmg__route_result scalar kssbc_cmg__empty_route()
{
    struct kssbc_cmg__route_result scalar out
...
struct kssbc_cmg__workspace scalar kssbc_cmg__empty_workspace()
{
    struct kssbc_cmg__workspace scalar out
...
struct kssbc_cmg__diag_result scalar kssbc_cmg__empty_diagnostics()
{
    struct kssbc_cmg__diag_result scalar out
...
real matrix kssbc_cmg__lex_panel(
    real matrix values,
...
real scalar kssbc_cmg__dense_ids(real colvector identifier)
{
...
real scalar kssbc_cmg__find_root(
...
real colvector kssbc_cmg__components(
    struct kssbc_cmg__graph scalar graph)
{
    ...
    for (vertex=1; vertex<=graph.n_vertex; vertex++) {
        ...
        root = kssbc_cmg__find_root(parent,u)
...
real matrix kssbc_cmg__graph_action(
    struct kssbc_cmg__graph scalar graph,
...
real scalar kssbc_cmg__pilot_cap(real scalar planned_rhs)
{
...
struct kssbc_cmg__preflight_result scalar kssbc_cmg__preflight(
    ...
    struct kssbc_cmg__options scalar options)
{
    ...
    out = kssbc_cmg__empty_preflight()
    ...
    if (cells.status != "CONVERGED" |
        ...
        !kssbc_cmg__options_valid(options)) return(out)
    predicted_edges = 0
    ...
    out.pilot_cap = kssbc_cmg__pilot_cap(max((planned_rhs,8)))
...
real scalar kssbc_cmg__park_miller_step(real scalar state)
{
...
real matrix kssbc_cmg__pilot_rhs(
    real colvector firm_key,
    real colvector component)
{
    ...
    for (position=1; position<=rows(firm_key); position++) {
        ...
        state[1] = kssbc_cmg__park_miller_step(state[1])
        state[2] = kssbc_cmg__park_miller_step(state[2])
...
struct kssbc_cmg__route_result scalar kssbc_cmg__route_decide(
    real scalar planned_rhs,
...
real colvector kssbc_cmg__cluster_members(
    real colvector head,
...
real colvector kssbc_cmg__mask_side(
    real scalar mask,
...
real scalar kssbc_cmg__mask_lex_less(
    ...
    real scalar cluster_size)
{
    ...
    candidate_side = kssbc_cmg__mask_side(candidate,cluster_size)
    incumbent_side = kssbc_cmg__mask_side(incumbent,cluster_size)
...
real scalar kssbc_cmg__cluster_bad_mask(
    struct kssbc_cmg__graph scalar graph,
    ...
    real scalar awd)
{
    ...
    for (candidate=2; candidate<=length(location); candidate++) {
        ...
        if (kssbc_cmg__mask_lex_less(candidate_mask,best_mask,size)) {
...
struct kssbc_cmg__aggregation_result scalar kssbc_cmg__aggregate_vertices(
    struct kssbc_cmg__graph scalar graph,
    real colvector component,
    ...
    struct kssbc_cmg__options scalar options)
{
    ...
    for (seed_position=1; seed_position<=graph.n_vertex; seed_position++) {
        ...
        while (cluster_size[cluster_id] < options.target_size) {
            best_edge = 0
            best_rank = .
            members = kssbc_cmg__cluster_members(
    ...
    while (screen_cursor <= cluster_count) {
        ...
        bad_mask = kssbc_cmg__cluster_bad_mask(
        ...
        size = rows(members)
        side = kssbc_cmg__mask_side(bad_mask,size)
        member_map[members] = (1::size)
...
struct kssbc_cmg__aggregation_result scalar kssbc_cmg__aggregate_fallback(
    struct kssbc_cmg__graph scalar graph,
    real colvector component,
    struct kssbc_cmg__options scalar options)
{
...
real scalar kssbc_cmg__component_reduce(
    real scalar fine_vertices,
    ...
    real scalar scratch_bytes)
{
    ...
    out.auxiliary_worker = J(0,1,.)
    out.status = "CONVERGED"
    out = kssbc_cmg__graph_finalize(out)
    ...
    struct kssbc_cmg__options scalar options)
{
    ...
    for (one_component=1; one_component<=level.n_component;
        ...
        level.coarse_keep[one_component] =
            kssbc_cmg__store_colvector(keep)
        level.coarse_scale[one_component] =
            kssbc_cmg__store_colvector(scale)
        level.coarse_factor[one_component] =
            kssbc_cmg__store_matrix(factor)
...
real matrix kssbc_cmg__component_project(
    struct kssbc_cmg__level scalar level,
    real matrix argument)
{
...
real matrix kssbc_cmg__coarse_apply(
    struct kssbc_cmg__level scalar level,
    real matrix right_hand_side)
{
    ...
    compatible = kssbc_cmg__component_project(level,right_hand_side)
    ...
    out = kssbc_cmg__component_project(level,out)
...
real scalar kssbc_cmg__options_valid(
    struct kssbc_cmg__options scalar options)
{
...
struct kssbc_cmg__hierarchy scalar kssbc_cmg__attempt_begin(
...
struct kssbc_cmg__hierarchy scalar kssbc_cmg__attempt_update(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    real scalar components,
    real scalar coarse_vertices,
    ...
    string scalar algorithm)
{
    ...
    while (1) {
        ...
        component_size = level.component_panel[.,2]-
            level.component_panel[.,1]:+1
        if (max(component_size) <= options.coarse_max) {
            level = kssbc_cmg__coarse_prepare(level,options)
            if (level.status != "CONVERGED") {
                out.status = level.status
                out.message = level.message
            ...
            out.level = out.level,kssbc_cmg__store_level(level)
        ...
        if (missing(reduction) | reduction < options.min_reduction) {
            aggregation_result = kssbc_cmg__aggregate_fallback(
                current,level.component,options)
            if (aggregation_result.status != "CONVERGED") {
                out.status = aggregation_result.status
                out.message = aggregation_result.message
    ...
    real scalar memory_cap_bytes)
{
    ...
    out = kssbc_cmg__empty_workspace()
    ...
    for (level_index=1; level_index<=hierarchy.n_level; level_index++) {
        level = *hierarchy.level[level_index]
        out.compatible[level_index] = kssbc_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.iterate[level_index] = kssbc_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.work[level_index] = kssbc_cmg__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
    }
    out.status = "CONVERGED"
...
struct kssbc_cmg__diag_result scalar kssbc_cmg__diagnostics(
    struct kssbc_cmg__hierarchy scalar hierarchy)
{
    struct kssbc_cmg__diag_result scalar out
    struct kssbc_cmg__level scalar level
    ...
    out = kssbc_cmg__empty_diagnostics()
...
real matrix kssbc_cmg__apply_level(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    real scalar level_index,
    real matrix right_hand_side)
{
    ...
    if (level_index == hierarchy.n_level) {
        return(kssbc_cmg__coarse_apply(level,compatible))
    }
    out = kssbc_cmg__smooth(level,compatible,hierarchy.options.omega)
    if (rows(out) != level.graph.n_vertex) return(J(0,0,.))
    action = kssbc_cmg__graph_action(
    ...
    correction = kssbc_cmg__apply_level(
...
struct kssbc_cmg__apply_result scalar kssbc_cmg__apply(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    real matrix right_hand_side)
{
    struct kssbc_cmg__apply_result scalar out
    ...
    out.value = kssbc_cmg__apply_level(hierarchy,1,right_hand_side)
...
real scalar kssbc_cmg__project_self(
    struct kssbc_cmg__level scalar level,
...
real scalar kssbc_cmg__smooth_into(
    struct kssbc_cmg__level scalar level,
    real matrix argument,
    real scalar column_count,
    real scalar omega,
...
real scalar kssbc_cmg__graph_into(
    struct kssbc_cmg__graph scalar graph,
    real matrix argument,
    real scalar column_count,
    real scalar scratch_bytes,
...
real scalar kssbc_cmg__coarse_into(
    struct kssbc_cmg__level scalar level,
    real matrix right_hand_side,
    real scalar column_count,
    real matrix out)
{
    ...
    return(kssbc_cmg__project_self(level,column_count,out))
}

real scalar kssbc_cmg__apply_ws_level(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    ...
    real matrix right_hand_side)
{
    ...
    if (level_index == hierarchy.n_level) {
        return(kssbc_cmg__coarse_into(
            level,*compatible,column_count,*iterate))
    }

    if (!kssbc_cmg__smooth_into(level,*compatible,column_count,
        hierarchy.options.omega,*iterate)) return(0)
    if (!kssbc_cmg__graph_into(level.graph,*iterate,column_count,
    ...
    if (!kssbc_cmg__graph_into(level.graph,*iterate,column_count,
        hierarchy.options.action_scratch_bytes,*work)) return(0)
    (*work)[.,used] = (*compatible)[.,used]-(*work)[.,used]
    if (!kssbc_cmg__smooth_into(level,*work,column_count,
        hierarchy.options.omega,*compatible)) return(0)
    (*iterate)[.,used] = (*iterate)[.,used]+(*compatible)[.,used]
    return(kssbc_cmg__project_self(level,column_count,*iterate))
}

struct kssbc_cmg__apply_result scalar kssbc_cmg__workspace_apply(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    struct kssbc_cmg__workspace scalar workspace,
    real matrix right_hand_side)
{
...
real matrix kssbc_cmg__project_labels(
...
struct kssbc_cmg__apply_result scalar kssbc_cmg__apply_kss(
    struct kssbc_cmg__hierarchy scalar hierarchy,
    real matrix firm_rhs)
{
    ...
    core = kssbc_cmg__apply(hierarchy,injected)
...
struct kssbc_cmg__apply_result scalar kssbc_cmg__apply_ppml(
    struct kssbc_cmg__hierarchy scalar hierarchy,
...
struct kssbc_cmg__cells scalar kssbc_cmg__cells_prepare(
    real colvector worker,
    real colvector firm,
    real colvector weight,
    real colvector worker_key,
    real colvector firm_key)
{
    ...
    if (!kssbc_cmg__dense_ids(worker) | !kssbc_cmg__dense_ids(firm)) {
    ...
    pair_panel = kssbc_cmg__lex_panel(pair_values,pair_order)
...
struct kssbc_cmg__graph scalar kssbc_cmg__hybrid_build(
    struct kssbc_cmg__cells scalar cells)
{
    struct kssbc_cmg__graph scalar out
...
real matrix kssbc_cmg__dense_schur(struct kssbc_cmg__cells scalar cells)
{
    real matrix out
...
real matrix kssbc_cmg__dense_laplacian(struct kssbc_cmg__graph scalar graph)
{
    real matrix out
...
real matrix kssbc_cmg__dense_hybrid_schur(
    struct kssbc_cmg__graph scalar graph)
{
    ...
    if (graph.status != "CONVERGED") return(J(0,0,.))
    lap = kssbc_cmg__dense_laplacian(graph)
