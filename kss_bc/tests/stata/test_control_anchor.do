...
too_many = kssbc__canonical_controls(I(33),J(33,1,1),1e-10)
...
foreach deletion_mode in observation match {
    ...
    foreach nuisance_mode in joint fixedoffset {
        foreach selected_algorithm in exact auto {
            capture noisily kss_bc y order_z1 order_z2, ///
            ...
            capture noisily kss_bc y order_u1 order_u2, ///
                worker(worker) firm(firm) ///
                `deletion_options' algorithm(`selected_algorithm') ///
                nuisance(`nuisance_mode') nodisplay
            assert _rc == 498
...
foreach deletion_mode in observation match {
    ...
    foreach nuisance_mode in joint fixedoffset {
        foreach selected_algorithm in exact auto {
            kss_bc y order_z1 order_z2, worker(worker) firm(firm) ///
                `deletion_options' algorithm(`selected_algorithm') ///
                nuisance(`nuisance_mode') nodisplay
            assert "`e(algorithm)'" == "exact"
            matrix order_reference = e(results)

            kss_bc y order_u1 order_u2, worker(worker) firm(firm) ///
                `deletion_options' algorithm(`selected_algorithm') ///
                nuisance(`nuisance_mode') nodisplay
            assert "`e(algorithm)'" == "exact"
            assert mreldif(order_reference,e(results)) < 2e-10

            kss_bc y order_u1 order_u2, worker(worker_relabel) ///
                firm(firm_relabel) `relabeled_options' ///
                algorithm(`selected_algorithm') ///
                nuisance(`nuisance_mode') nodisplay
            assert "`e(algorithm)'" == "exact"
...
frequency = J(9,1,1)
base = kssbc__canonical_controls(Q,frequency,1e-10)
changed = kssbc__canonical_controls(minus_Q,frequency,1e-10)
