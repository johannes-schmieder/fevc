...
void ksse_oracle__run()
{
    ...
    struct kssbc_scale_design scalar design
    struct kssbc_scale_engine_result scalar out, alternate, invalid
    struct kssbc_scale_engine_result scalar rng_out, rng_alternate
    struct kssbc_scale_engine_matrix_atoms scalar atom_source
    struct kssbc_scale_atom_provider scalar provider, rng_provider
    struct kssbc_scale_atom_provider scalar rng_provider_alternate
    struct kssbc_scale_atom_provider scalar bad_rng_provider
    struct kssbc_scale_rng_context scalar rng_context, rng_context_alternate
    struct kssbc_scale_rng_context scalar bad_rng_context
    ...
    direction = kssbc_scale_eng__center_target(
        first,J(design.coefficient_cells,1,1/6))
    score = kssbc_scale_eng__balance_score(
    ...
    score = kssbc_scale_eng__center_target(
        first,J(design.coefficient_cells,1,1/6))
    score = kssbc_scale_eng__balance_score(
        score,(sum(abs(first))+abs(sum(first))))
    assert(rows(score) == design.coefficient_cells)
    assert(kssbc__norm2(score) > 1e-15)
    assert(sum(score) == 0)
    ...
    assert(rng_context.leverage_initialized == 0)
    assert(rng_context.target_initialized == 0)
    rng_out = kssbc_scale_engine__run(
    ...
    assert(rng_context.leverage_initialized == 1)
    assert(rng_context.target_initialized == 1)
    assert(rng_out.rng_contract == kssbc_rng__production_contract())
    assert(ksse_oracle__reldif(
    ...
    alternate = kssbc_scale_eng__run_atoms(
    ...
    invalid = kssbc_scale_eng__run_atoms(
