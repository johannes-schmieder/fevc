...
mata: KSS_TEST_RNG_SUCCESS_BEFORE = kssbc_rng__capture_full()
mata: assert(KSS_TEST_RNG_SUCCESS_BEFORE.status == "OK")

quietly timer clear 70
quietly timer on 70
quietly kss_bc y [fw=frequency] if scope, worker(worker) firm(firm) ///
...
mata: assert(kssbc_scale_runtime__status() == "EMPTY")
...
mata:
KSS_TEST_RNG_SUCCESS_AFTER = kssbc_rng__capture_full()
...
// reinterpretations.
quietly kss_bc y control [fw=frequency] if scope,                  ///
...
mata: KSS_TEST_RNG_FAILURE_BEFORE = kssbc_rng__capture_full()
mata: assert(KSS_TEST_RNG_FAILURE_BEFORE.status == "OK")
capture noisily kss_bc y control [fw=frequency] if scope,          ///
    worker(worker) firm(firm) deletion(match) deletionid(match)    ///
    targetweight(target) probeorder(atom_key) algorithm(jla)       ///
...
mata: assert(kssbc_scale_runtime__status() == "EMPTY")
mata:
KSS_TEST_RNG_FAILURE_AFTER = kssbc_rng__capture_full()
...
capture noisily kss_bc y [fw=frequency] if scope,                 ///
