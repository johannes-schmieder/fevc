...
local oldpwd `"`c(pwd)'"'
capture confirm file "kss_bc/kss_bc_lifecycle.ado"
if _rc {
    capture confirm file "../../kss_bc_lifecycle.ado"
    if _rc {
...
adopath ++ `"`pkgroot'"'
quietly do `"`pkgroot'/kss_bc_lifecycle.ado"'
...
assert strpos(lower(fileread(`"`pkgroot'/kss_bc_lifecycle.ado"')), ///
    "shell") == 0
...
if strtrim(`"`configured_phase_file'"') == "" {
    _kss_bc_lifecycle_phase, phase(import_selection)
...
foreach registered_phase in import_selection compression_transition      ///
    ...
    _kss_bc_lifecycle_phase_write, phase(`registered_phase')       ///
...
capture noisily _kss_bc_lifecycle_phase_write, phase(target)      ///
    phasefile(`"`phase_marker'"')
assert _rc == 198
assert fileread(`"`phase_marker'"') == "restoration"

capture noisily _kss_bc_lifecycle_phase_write, phase(numerical)   ///
...
kss_bc_lifecycle, method(preserve) sample(sample)                 ///
...
capture noisily kss_bc_lifecycle, method(preserve) sample(sample) ///
...
kss_bc_lifecycle, method(tempfile) sample(sample)                 ///
...
capture noisily kss_bc_lifecycle, method(tempfile) sample(sample) ///
    callback(_kss_test_scale_lifecycle_work) callbackoptions(fail) ///
    certify
assert _rc == 459
quietly _datasignature
