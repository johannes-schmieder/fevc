...
void test_scale_resource()
{
    ...
    struct kssbc_resource_forecast scalar compressed, generic, advisory
    struct kssbc_resource_forecast scalar wall_miss, hard_fail, routed
    struct kssbc_resource_model scalar model
    ...
    assert(kssbc_resource__rng_call_upper() == 0)
    ...
    advisory = kssbc_resource__forecast(
    ...
    wall_miss = kssbc_resource__forecast(
    ...
    hard_fail = kssbc_resource__forecast(
    ...
    assert(kssbc_solver__resource_budget(1000) == 1000)
    assert(kssbc_solver__resource_config(
        "compressed",51,24,39,27,100,160,1000) == 0)
    assert(kssbc_solver__resource_budget(1000) == 109)
    ...
    assert(kssbc_solver__resource_status() == "RESOURCE_ADMISSION_FAILED")
    kssbc_solver__resource_clear()
    ...
    unsafe = kssbc_resource__reconcile(
    ...
    selected = kssbc_resource__select_auto(1,compressed,generic)
