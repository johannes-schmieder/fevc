...
void test_scale_compression()
{
    ...
    struct kssbc_scale_design scalar design, reordered, compact
    ...
    diagnostic = kssbc_scale__diagnose(
    ...
    assert(mreldif(kssbc_scale__stable_groupsum(
        design.unit_outcome_sum,design.unit_cell_order,
        design.unit_cell_panel),design.cell_outcome_sum) <= 2e-15)
    assert(mreldif(kssbc_scale__strata_to_cells(
        design.strata,design.strata.target_mass,
        design.coefficient_cells),design.cell_target_mass) <= 2e-15)
    assert(kssbc_scale__stable_colsum((1e16\1\-1e16))[1] == 1)
    assert(kssbc_scale__version() == "0.1.0-experimental")

    base = kssbc__fe_prepare(worker,firm,frequency,1e-10)
    assert(base.status == "CONVERGED")
    row_values = (outcome, sin(1::n_rows))
    cell_values = kssbc_scale__collapse_rows(design,row_values)
    base_rhs = kssbc__fe_transpose(base,row_values)
    compressed_rhs = kssbc_scale__fe_transpose(design,cell_values)
    assert(mreldif(base_rhs,compressed_rhs) <= 2e-15)

    gamma = ((.4,-.1)\(-.2,.3)\(0,0))
    base_schur = kssbc__fe_schur_action(base,gamma)
    compressed_schur = kssbc_scale__fe_schur_action(design,gamma)
    ...
    base_prediction = kssbc__fe_predict(base,coefficient)
    cell_prediction = kssbc_scale__fe_predict(design,coefficient)
    assert(mreldif(base_prediction,cell_prediction[design.row_to_cell,.]) <=
        2e-15)
    assert(mreldif(
        kssbc_scale__weighted_rss(design,cell_prediction),
        colsum(frequency:*(outcome:-base_prediction):^2)) <= 2e-15)

    residual = kssbc_scale__fe_full_residual(
        design,base_rhs,coefficient)
    full_rhs = kssbc_scale__fe_full_rhs(design,base_rhs)
    alpha = coefficient[1..design.worker_levels,.]
    gamma = kssbc_scale__full_firm(design,coefficient)
    ...
    assert(cols(kssbc_scale__fe_full_relres(
        design,base_rhs,coefficient)) == 2)

    coefficient = kssbc_scale__fe_from_firm(
    ...
    reordered = kssbc_scale__prepare(
    ...
    compact = kssbc_scale__compact(design)
    ...
    compressed_schur = kssbc_scale__fe_schur_action(design,gamma)
    compact_schur = kssbc_scale__fe_schur_action(compact,gamma)
