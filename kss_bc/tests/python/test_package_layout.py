...
def test_control_and_frequency_certificates_are_fail_closed() -> None:
    ...
    assert "kssbc__exact_physical_total" in ado
    ...
    assert "projection_error=kssbc__norm2(projector*projector-projector)" in mata
    assert "span_error=kssbc__norm2(out.controls-orthonormal*checked)" in mata
    assert "return(1e-8)" in mata
    assert mata.count("kssbc__propagate_error(") >= 6
