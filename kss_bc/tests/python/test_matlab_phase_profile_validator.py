...
VALIDATOR_PATH = ROOT / "kss_bc" / "benchmarks" / "validate_matlab_phase_profile.py"
...
def write_aggregate(output_dir: Path, record: dict[str, object]) -> None:
    ...
    (output_dir / "wrapper.pass").write_text(
        f"KSS_BC_MATLAB_PHASE_PROFILE_PASS {LABEL} {BUNDLE} {SOURCE} "
...
def evidence(tmp_path: Path) -> tuple[Path, dict[str, object]]:
    ...
    (job_dir / "application.txt").write_text(
        f"KSS_BC MATLAB PHASE PROFILE PASS: {LABEL}\n", encoding="utf-8"
    )
...
def test_matlab_profiler_has_bounded_top_level_real_time_contract() -> None:
    source = (ROOT / "kss_bc/benchmarks/separations_matlab_phase_profile.m").read_text(
        encoding="utf-8"
    )
...
def test_scc_wrapper_binds_and_rechecks_immutable_inputs() -> None:
    source = (ROOT / "kss_bc/benchmarks/scc/run_matlab_phase_profile.sge").read_text(
        encoding="utf-8"
    )
    ...
    submit = (ROOT / "kss_bc/benchmarks/scc/submit_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
    collect = (ROOT / "kss_bc/benchmarks/scc/collect_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
...
def test_new_shell_entrypoints_pass_bash_syntax() -> None:
    scripts = [
        ROOT / "kss_bc/benchmarks/scc/run_matlab_phase_profile.sge",
        ROOT / "kss_bc/benchmarks/scc/submit_matlab_phase_profile.sh",
        ROOT / "kss_bc/benchmarks/scc/collect_matlab_phase_profile.sh",
    ]
