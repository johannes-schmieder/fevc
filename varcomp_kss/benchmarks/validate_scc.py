...
def validate_portability(root: Path, commit: str) -> None:
    ...
    require("VARCOMP_KSS SCC PORTABILITY PASS" in wrapper_log, "portability wrapper marker absent")
    full_log = text(root / "portability" / "full_suite.application.txt")
    install_log = text(root / "portability" / "install.application.txt")
    require("VARCOMP_KSS TEST SUITE PASS: full" in full_log, "full Stata suite marker absent")
    require("VARCOMP_KSS INSTALL TEST PASS" in install_log, "install marker absent")
    marker = text(root / "portability" / "portability.pass")
    require(marker.strip() == f"VARCOMP_KSS_PORTABILITY_PASS {commit}", "bad portability marker")
...
def validate_oracle(root: Path, commit: str) -> None:
    ...
    require("VARCOMP_KSS SCC ORACLE PASS" in wrapper_log, "oracle wrapper marker absent")
    require("VARCOMP_KSS MATLAB ORACLE PASS" in matlab_log, "MATLAB oracle marker absent")
    require("VARCOMP_KSS STATA/MATLAB ORACLE PASS" in stata_log, "paired oracle marker absent")
    require(
        text(root / "oracle" / "oracle.wrapper.pass").strip()
        == f"VARCOMP_KSS_ORACLE_PASS {commit}",
...
def validate_scale(root: Path, commit: str, scenario: str) -> None:
    ...
    require(f"VARCOMP_KSS SCC SCALE PASS: {scenario}" in wrapper_log, f"{scenario} wrapper marker absent")
    require(f"VARCOMP_KSS BENCHMARK PASS: {scenario}" in app_log, f"{scenario} application marker absent")
    require(
        text(job_dir / f"{scenario}.wrapper.pass").strip()
        == f"VARCOMP_KSS_SCALE_PASS {scenario} {commit}",
...
def main() -> int:
    ...
    for job in args.jobs:
        ...
        print(f"PASS {job}")
    print("VARCOMP_KSS SCC EVIDENCE PASS")
    return 0
