#!/usr/bin/env python3
"""Run the PREP-RHS-1 local archive-isolated benchmark.

The numerical workload is the tracked Stata driver ``numopt2_local.do``.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-prep-rhs1-") as tmp:
        temporary = Path(tmp)
        roots = {role: temporary / role for role in commits}
        for role, commit in commits.items():
            roots[role].mkdir()
            _archive(repo, commit, roots[role])
        driver = roots["candidate"] / "varcomp_kss/benchmarks/numopt2_local.do"
        baseline_driver = roots["baseline"] / "varcomp_kss/benchmarks/numopt2_local.do"
    ...
    summary = {
        "schema": "varcomp-kss-prep-rhs1-local-v1",
        "status": "PASS",
