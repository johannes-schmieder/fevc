...
if "`route'" == "cmg" {
    ...
    quietly do "varcomp_kss/varcomp_kss_cmg.mata"
    quietly do "varcomp_kss/tests/support/vckss_cmg_adapter.mata"
    mata: mata drop vckss__stata_jla()
    quietly do "varcomp_kss/tests/support/vckss_cmg_bridge_override.mata"
...
timer on 81
if "`route'" == "exact" {
    capture noisily varcomp_kss y_minus_xb, worker(worker) firm(firm) ///
...
if `converged' {
    file write `marker' ///
        "VARCOMP_KSS_SEPARATIONS_CONVERGED `route' `label' `source_commit'" _n
}
else {
    file write `marker' ///
        "VARCOMP_KSS_SEPARATIONS_TYPED_FAILURE `route' `label' `route_status' `source_commit'" _n
}
file close `marker'
...
di as result "VARCOMP_KSS SEPARATIONS ESTIMATOR PASS: `route' `label'"
exit 0
