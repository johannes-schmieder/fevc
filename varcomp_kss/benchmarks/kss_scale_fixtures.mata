...
struct vckss_scale__diagnostic
{
...
real scalar vckss_scale_fixture__api_level()
{
    return(2)
}

string scalar vckss_scale_fixture__build_id()
{
    return("kss-scale-fixtures-api2-replicated-blocks")
}

real scalar vckss_scale__pair_count(
...
real matrix vckss_scale__copy_pairs(
    string scalar design,
    real scalar copies)
{
    real matrix pairs
    ...
    if (design == "replicated_blocks" | design == "well_connected") {
        pairs = J(vckss_scale__pair_count(design,copies),2,.)
        pair = 0
...
real matrix vckss_scale__meta_metrics(
    string scalar design,
    real scalar copies)
{
    ...
    pairs = vckss_scale__pair_count(design,copies)
...
real scalar vckss_scale__unique_pair_count(
...
struct vckss_scale__diagnostic scalar vckss_scale__diagnose(
    real colvector worker,
    real colvector firm,
    real colvector deletion_id,
    real colvector frequency)
{
    struct vckss_scale__diagnostic scalar out
    ...
    out.cells = vckss_scale__unique_pair_count(worker,firm)
...
void vckss_scale__stata_diagnose(
...
void vckss_scale__stata_connect(
    string scalar design,
    real scalar copies,
    ...
    string scalar metrics_name)
{
    ...
    pairs = vckss_scale__copy_pairs(design,copies)
    metrics = vckss_scale__meta_metrics(design,copies)
