...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-prep-bnd1-") as temporary_name:
        temporary = Path(temporary_name)
        roots = {role: temporary / role for role in ROLES}
        for role in ROLES:
            roots[role].mkdir()
            archive(repo, commits[role], roots[role])
        driver = repo / "varcomp_kss/benchmarks/prep_bnd1/local_driver.do"
    ...
    summary = {
        "schema": "varcomp-kss-prep-bnd1-local-v1",
        "status": "PASS",
