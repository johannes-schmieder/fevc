function varcomp_kss_dense_oracle(output_dir, source_commit)
...
if nargin ~= 2 || strlength(string(source_commit)) < 7
    error('varcomp_kss:invalidInput','output_dir and source_commit are required');
end
...
if max(abs(plugin-expected_plugin)) > 2e-10 || ...
    ...
    error('varcomp_kss:oracleMismatch','dense MATLAB oracle missed registered values');
end
fprintf('VARCOMP_KSS MATLAB ORACLE PASS: %s\n',source_commit);
end
