...
timer on 81
capture noisily varcomp_kss outcome, worker(worker) firm(firm) ///
...
if `converged' {
    file write `marker' ///
        "VARCOMP_KSS_NUMOPT_CONVERGED `route' `scenario' `source_commit'" _n
}
else {
    file write `marker' ///
        "VARCOMP_KSS_NUMOPT_TYPED_FAILURE `route' `scenario' `route_status' `source_commit'" _n
}
file close `marker'
...
di as result "VARCOMP_KSS NUMOPT BENCHMARK PASS: `route' `scenario'"
exit 0
