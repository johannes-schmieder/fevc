function prep_bnd1_matlab_run()
% Source-bound maintained MATLAB comparison on one fixed synthetic input.

output_dir = required_env('PBM_OUTPUT_DIR');
scratch_dir = required_env('PBM_SCRATCH_DIR');
input_file = required_env('PBM_INPUT_CSV');
input_sha = required_env('PBM_INPUT_SHA256');
matlab_root = required_env('PBM_MATLAB_ROOT');
task_sha = required_env('PBM_TASK_SHA256');
source_identity_path = required_env('PBM_SOURCE_IDENTITY');
process_identity_file = required_env('PBM_PROCESS_IDENTITY');
experiment = required_env('PBM_EXPERIMENT_ID');
source_commit = required_env('PBM_SOURCE_COMMIT');
bundle_sha = required_env('PBM_BUNDLE_SHA256');
expected_rows = required_integer('PBM_ROWS');
expected_workers = required_integer('PBM_WORKERS');
expected_firms = required_integer('PBM_FIRMS');
probes = required_integer('PBM_PROBES');
seed = required_integer('PBM_SEED');
mex_dir = fullfile(scratch_dir,'mex');
detail_dir = fullfile(scratch_dir,'details');

assert_pbm(expected_workers == 40*expected_firms && ...
    expected_rows == 3*expected_workers && ...
    ismember(expected_firms,[64 256 1024]),'Dimensions', ...
    'Synthetic task dimensions changed.');
assert_pbm(ismember(probes,[20 200]) && seed >= 1,'Randomness', ...
    'Probe or seed contract changed.');
assert_pbm(~isempty(regexp(experiment,'^[A-Za-z0-9._-]+$','once')) && ...
...
try
    ...
    assert_pbm(strcmp(source_identity.status,'PASS'),'SourceIdentity', ...
    ...
    assert_pbm(strcmp(canonical_path(resolved),canonical_path( ...
    ...
    assert_pbm(isequal(data.Properties.VariableNames,required), ...
    ...
    clear data
    assert_pbm(numel(outcome)==expected_rows && all(isfinite(outcome)) && ...
    ...
    assert_pbm(min(worker)==1 && max(worker)==expected_workers && ...
        min(firm)==1 && max(firm)==expected_firms && ...
        numel(unique(match))==expected_rows && ...
        numel(unique([worker firm],'rows'))==expected_rows, ...
        'InputDimensions','Input identifiers changed.');
    assert_pbm(isequal(observation_key,(1:expected_rows)') && ...
        all(period>=1 & period<=3),'InputOrder','Input order changed.');
    input_validation_seconds = toc(validation_started);
    clear period match observation_key

    existing_pool = gcp('nocreate');
    assert_pbm(isempty(existing_pool),'PreexistingPool', ...
    ...
    assert_pbm(pool.NumWorkers==4,'PoolSize', ...
    ...
    assert_pbm(isequal(worker_indices,1:4) && ...
    ...
    record = struct('schema','PREP-BND-1-MATLAB-AGGREGATE-V1', ...
        ...
        'matlab_version',version,'rows',expected_rows, ...
        'workers',expected_workers,'firms',expected_firms, ...
        'cells_per_worker',3,'probes',probes,'seed',seed, ...
        ...
        'same_literal_rows',true,'uniform_stored_row_targets',true, ...
    ...
    assert_pbm(marker>=0,'Marker','Could not write MATLAB pass marker.');
    fprintf(marker,'PREP_BND1_MATLAB_PASS %s %s\n',experiment,task_sha);
    fclose(marker);
    fprintf('PREP-BND-1 MATLAB PASS: %s\n',experiment);
catch exception
    ...
    failure = struct('schema','PREP-BND-1-MATLAB-FAILURE-V1', ...
...
assert_pbm(status==0,'DetailCount','Could not count detail rows.');
count = str2double(strtrim(text));
assert_pbm(isfinite(count) && count>=1 && count==floor(count), ...
...
value = getenv(name);
assert_pbm(~isempty(value),'Environment',['Missing ' name '.']);
end
...
assert_pbm(isfinite(value) && value>=1 && value==floor(value), ...
...
assert_pbm(handle>=0,'JSON','Could not open JSON output.');
fprintf(handle,'%s\n',jsonencode(value));
fclose(handle);
...
function assert_pbm(condition,code,message)
if ~condition
    error(['varcomp_kss:prepBnd1Matlab:' code],'%s',message);
end
end
