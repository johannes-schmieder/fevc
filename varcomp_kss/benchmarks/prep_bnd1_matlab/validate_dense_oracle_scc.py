...
def validate(
    ...
    for relative in (
        "SOURCE_COMMIT.txt",
        "varcomp_kss/benchmarks/oracle/stata_oracle.do",
        "varcomp_kss/benchmarks/oracle/varcomp_kss_dense_oracle.m",
    ...
    validation_log = (job_dir / "dense_oracle_validation.application.txt").read_text(
        encoding="utf-8", errors="replace")
    require(f"VARCOMP_KSS MATLAB ORACLE PASS: {expected_source_commit}" in matlab_log,
            "MATLAB application marker missing")
    require("VARCOMP_KSS STATA/MATLAB ORACLE PASS" in stata_log,
