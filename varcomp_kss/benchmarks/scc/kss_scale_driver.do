...
if inlist("`fixture'", "replicated_blocks", "ring") & `scale_factor' >= 2 {
    ...
    quietly do "`c(pwd)'/varcomp_kss/benchmarks/kss_scale_fixtures.mata"
    quietly do "`c(pwd)'/varcomp_kss/benchmarks/kss_scale_fixtures.do"
    ...
    quietly vckss_scale_fixture, `fixture_options'
    ...
    // separate implementations of several vckss_scale__* diagnostics.  The
    // constructed Stata data and copied scalar receipts survive a Mata clear;
    // remove the fixture runtime now so varcomp_kss can load its registered
    // production runtime without a namespace collision.
    capture program drop vckss_scale_fixture
    mata: mata clear
...
capture noisily varcomp_kss y_minus_xb `weight_spec', `command_options'
