...
timer on 81
capture noisily varcomp_kss y c1 c2 [fw=frequency], ///
...
if `command_rc' {
    ...
    file open `failure' using "`output_dir'/`scenario'.failure.txt", ///
        write text replace
    file write `failure' "VARCOMP_KSS_BENCHMARK_FAIL rc=`command_rc'" _n
    file close `failure'
    exit `command_rc'
}
if "`e(status)'" != "KSS_POINT_ESTIMATES_ONLY" {
    di as error "varcomp_kss did not return its success status"
    exit 498
}
...
file open `marker' using "`output_dir'/`scenario'.stata.pass", ///
    write text replace
file write `marker' "VARCOMP_KSS_BENCHMARK_PASS `scenario' `source_commit'" _n
file close `marker'

di as result "VARCOMP_KSS BENCHMARK PASS: `scenario'"
exit 0
