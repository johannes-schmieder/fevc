*! varcomp_kss production solver routing adapter
...
{
    return("varcomp-kss-solver-api26-gpl-mata-cmg")
}
...
struct vckss_solver_pilot_evidence
{
...
struct vckss_solver_resource_gate
{
    real scalar active
    real scalar applied
    string scalar route
...
struct vckss_solver_resource_gate scalar vckss_solver__resource_empty()
{
    struct vckss_solver_resource_gate scalar out
...
void vckss_solver__resource_clear()
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE

    VCKSS_SOLVER_RESOURCE_GATE = vckss_solver__resource_empty()
}

real scalar vckss_solver__resource_config(
    string scalar route,
    real scalar non_solver_numerical_bytes,
    real scalar selection_peak_bytes,
    real scalar transition_peak_bytes,
    ...
    real scalar hard_wall_seconds)
{
    ...
    VCKSS_SOLVER_RESOURCE_GATE.active = 1
    VCKSS_SOLVER_RESOURCE_GATE.route = route
    VCKSS_SOLVER_RESOURCE_GATE.status = "CONFIGURED"
    VCKSS_SOLVER_RESOURCE_GATE.message =
    ...
    VCKSS_SOLVER_RESOURCE_GATE.selection_peak_bytes =
        selection_peak_bytes
    VCKSS_SOLVER_RESOURCE_GATE.transition_peak_bytes =
        transition_peak_bytes
    VCKSS_SOLVER_RESOURCE_GATE.restoration_peak_bytes =
        restoration_peak_bytes
    VCKSS_SOLVER_RESOURCE_GATE.wall_forecast_upper_seconds =
        wall_forecast_upper_seconds
    VCKSS_SOLVER_RESOURCE_GATE.hard_memory_bytes = hard_memory_bytes
    VCKSS_SOLVER_RESOURCE_GATE.hard_wall_seconds = hard_wall_seconds
    return(0)
}

real scalar vckss_solver__resource_active()
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE

    return(VCKSS_SOLVER_RESOURCE_GATE.active)
}

real scalar vckss_solver__resource_budget(
...
real scalar vckss_solver__resource_apply(
    real scalar routed_solver_peak_bytes)
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE
    ...
    peaks = (
        ...
        VCKSS_SOLVER_RESOURCE_GATE.numerical_peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.restoration_peak_bytes)
    VCKSS_SOLVER_RESOURCE_GATE.peak_bytes = max(peaks)
    if (VCKSS_SOLVER_RESOURCE_GATE.peak_bytes == peaks[1]) {
        VCKSS_SOLVER_RESOURCE_GATE.peak_phase = "selection"
    }
    else if (VCKSS_SOLVER_RESOURCE_GATE.peak_bytes == peaks[2]) {
        VCKSS_SOLVER_RESOURCE_GATE.peak_phase = "transition"
    }
    else if (VCKSS_SOLVER_RESOURCE_GATE.peak_bytes == peaks[3]) {
        VCKSS_SOLVER_RESOURCE_GATE.peak_phase = "numerical"
    }
    else VCKSS_SOLVER_RESOURCE_GATE.peak_phase = "restoration"

    VCKSS_SOLVER_RESOURCE_GATE.memory_admission_bytes =
        ceil(1.30*VCKSS_SOLVER_RESOURCE_GATE.peak_bytes)
    if (missing(VCKSS_SOLVER_RESOURCE_GATE.wall_forecast_upper_seconds)) {
        VCKSS_SOLVER_RESOURCE_GATE.wall_admission_seconds = .
    }
    ...
    else VCKSS_SOLVER_RESOURCE_GATE.wall_admitted =
        VCKSS_SOLVER_RESOURCE_GATE.wall_admission_seconds <=
            VCKSS_SOLVER_RESOURCE_GATE.hard_wall_seconds
    VCKSS_SOLVER_RESOURCE_GATE.admitted =
        VCKSS_SOLVER_RESOURCE_GATE.memory_admitted
    VCKSS_SOLVER_RESOURCE_GATE.applied = 1
    if (VCKSS_SOLVER_RESOURCE_GATE.admitted) {
        VCKSS_SOLVER_RESOURCE_GATE.status = "ADMITTED"
        if (VCKSS_SOLVER_RESOURCE_GATE.memory_admission_bytes >
                VCKSS_SOLVER_RESOURCE_GATE.hard_memory_bytes |
            !VCKSS_SOLVER_RESOURCE_GATE.wall_admitted) {
            VCKSS_SOLVER_RESOURCE_GATE.message =
    ...
    if (VCKSS_SOLVER_RESOURCE_GATE.route == "compressed") {
        VCKSS_SOLVER_RESOURCE_GATE.status = "RESOURCE_ADMISSION_FAILED"
        VCKSS_SOLVER_RESOURCE_GATE.message =
...
string scalar vckss_solver__resource_status()
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE

    return(VCKSS_SOLVER_RESOURCE_GATE.status)
}

string scalar vckss_solver__resource_message()
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE

    return(VCKSS_SOLVER_RESOURCE_GATE.message)
}

real scalar vckss_solver__target_id_resid(real matrix values)
{
...
real scalar vckss_solver__rhs_resid_max(real matrix diagnostics)
{
...
real scalar vckss_solver__roundoff_gate()
{
    return(4096*2.2204460492503131e-16)
}

real rowvector vckss_solver__resource_vector()
{
    external struct vckss_solver_resource_gate scalar VCKSS_SOLVER_RESOURCE_GATE

    if (!VCKSS_SOLVER_RESOURCE_GATE.applied) return(J(1,15,.))
    return((
        VCKSS_SOLVER_RESOURCE_GATE.selection_peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.transition_peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.numerical_peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.restoration_peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.peak_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.memory_headroom_fraction,
        VCKSS_SOLVER_RESOURCE_GATE.memory_admission_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.wall_forecast_upper_seconds,
        VCKSS_SOLVER_RESOURCE_GATE.wall_headroom_fraction,
        VCKSS_SOLVER_RESOURCE_GATE.wall_admission_seconds,
        VCKSS_SOLVER_RESOURCE_GATE.hard_memory_bytes,
        VCKSS_SOLVER_RESOURCE_GATE.hard_wall_seconds,
        VCKSS_SOLVER_RESOURCE_GATE.memory_admitted,
...
void vckss_solver__stata_res_rcpt(
    ...
    string scalar applied_local)
{
    ...
    if (VCKSS_SOLVER_RESOURCE_GATE.active &
        ...
        if (!missing(forecast_row) & forecast_row == floor(forecast_row) &
            ...
            live_nonsolver = sum(
                ...
                (VCKSS_SOLVER_RESOURCE_GATE.route == "generic")*
                components[forecast_row,1]
            forecast_vector = vckss_solver__resource_vector()
    ...
    st_local(status_local,VCKSS_SOLVER_RESOURCE_GATE.status)
    st_local(message_local,VCKSS_SOLVER_RESOURCE_GATE.message)
    st_local(phase_local,VCKSS_SOLVER_RESOURCE_GATE.peak_phase)
    st_local(applied_local,applied)
    vckss_solver__resource_clear()
}

struct vckss_route_result scalar vckss_solver__empty_route()
{
...
real colvector vckss_solver__canonical_keys(
...
real scalar vckss_solver__planned_rhs(
    real scalar probes,
...
{
    return(vckss_cmg__api_level() == 7 &
        vckss_cmg__numeric_mode() == "off" &
        vckss_cmg__design_label() ==
        "gpl-cmg-mata-degree3-hybrid-v7-varcomp-kss-component")
}
...
real scalar vckss_solver__diagonal_work(
...
real scalar vckss_solver__cmg_work(
    real scalar planned_rhs,
    real scalar iterations,
    real scalar observations,
    real scalar workers,
...
real scalar vckss_solver__fallback_work_ok(real scalar work)
{
...
real scalar vckss_solver__cmg_work_wins(real scalar work_ratio)
{
...
struct vckss_solver_pilot_evidence scalar vckss_solver__pilot_empty(
    real scalar backend_code)
{
    struct vckss_solver_pilot_evidence scalar out
...
struct vckss_solver_pilot_evidence scalar vckss_solver__pilot_evidence(
    ...
    real scalar exact_inverse)
{
    ...
    out = vckss_solver__pilot_empty(backend_code)
    ...
    for (pilot=1; pilot<=4; pilot++) {
        ...
        if (backend_code == 1) {
            out.diagnostics[pilot,15] =
                vckss_solver__fallback_work_ok(work)
        }
...
struct vckss_solver_cmg_context
{
...
struct vckss_solver_cmg_context scalar vckss_solver__cmg_context(
    pointer(struct vckss_cmg__hierarchy scalar) scalar hierarchy,
    real scalar batch_capacity,
    real scalar memory_envelope_bytes,
    real scalar enable_workspace)
{
    struct vckss_solver_cmg_context scalar out
    real scalar workspace_cap

    out.hierarchy = hierarchy
    out.workspace = vckss_cmg__empty_workspace()
    out.use_workspace = 0
...
struct vckss_cmg__apply_result scalar vckss_solver__cmg_apply_ws(
    ...
    real matrix firm_rhs)
{
    ...
    core = vckss_cmg__workspace_apply(
...
struct vckss_preconditioner_result scalar vckss_solver__cmg_apply(
    ...
    real matrix residual)
{
    ...
    pointer(struct vckss_solver_cmg_context scalar) scalar cmg_context
    pointer(struct vckss_cmg__hierarchy scalar) scalar hierarchy
    ...
    if ((*cmg_context).use_workspace &
        ...
        applied = vckss_solver__cmg_apply_ws(
            *cmg_context,residual)
    }
    else applied = vckss_cmg__apply_kss(*hierarchy,residual)
...
struct vckss_solver_backend scalar vckss_solver__cmg_backend(
    pointer(struct vckss_solver_cmg_context scalar) scalar context)
{
    struct vckss_solver_backend scalar out

    out.route = "CMG"
    out.context = context
    out.apply = &vckss_solver__cmg_apply()
...
// observation arrays.
struct vckss_cmg__hierarchy scalar vckss_solver__hierarchy_cells(
    ...
    real scalar planned_rhs)
{
    ...
    return(vckss_solver__hierarchy_cells(
    ...
    real colvector firm_key)
{
    ...
    firm_rhs = vckss_cmg__pilot_rhs(firm_key,component)
    if (rows(firm_rhs) != design.firm_levels) return(J(0,0,.))
    return(J(design.worker_levels,4,0) \
        firm_rhs)
}

real scalar vckss_solver__pilots_valid(
    struct vckss_solve_result scalar solved,
    real scalar cap,
    real scalar tolerance)
{
...
struct vckss_route_result scalar vckss_solver__pilot_legacy(
    real colvector y,
    ...
    real scalar semantic_atom_mode)
{
    ...
    struct vckss_cmg__level scalar fine
    struct vckss_solver_cmg_context scalar cmg_context
    struct vckss_solver_backend scalar diagonal_backend, cmg_backend, backend
    struct vckss_solve_result scalar diagonal_pilots, cmg_pilots
    struct vckss_solver_pilot_evidence scalar diagonal_evidence, cmg_evidence
    ...
    if (requested_route == "DIAGONAL") {
        ...
        worker_key = vckss_solver__canonical_keys(
            base.worker,base.worker_levels)
        firm_key = vckss_solver__canonical_keys(
    ...
    if (requested_route == "DIAGONAL") {
        ...
        pilot_rhs = vckss_solver__pilot_rhs(base,firm_key)
        if (rows(pilot_rhs) == 0) {
            out.estimator = vckss__failure(
        ...
        diagonal_valid = vckss_solver__pilots_valid(
        ...
        if (requested_route == "AUTO" & diagonal_realistic &
            ...
            if (hierarchy.status != "CONVERGED") {
                if (requested_route == "AUTO" & diagonal_realistic) {
                    ...
                    else out.estimator = vckss__failure(
                ...
                use_cmg = cmg_valid &
                    (requested_route == "CMG" | !diagonal_realistic |
                     vckss_solver__cmg_work_wins(work_ratio))
    ...
    // Enforce the solver reservation before vckss__jla_backend initializes
    ...
    if (vckss_solver__resource_active() &
        !vckss_solver__resource_apply(forecast_peak)) {
        out.estimator = vckss__failure(
            vckss_solver__resource_status(),
            vckss_solver__resource_message())
        out.status = out.estimator.status
        out.message = out.estimator.message
        out.reason = out.estimator.message
        return(out)
