# CMG API 7 component contract

## Ownership and generated surfaces

CMG is an internal component of `varcomp_kss`. It has no public Stata command,
...
Generator API 4 creates exactly two namespace artifacts:

| Target | Namespace | `matalnum` | Location |
| --- | --- | --- | --- |
| package | `vckss_cmg` | `off` | `varcomp_kss/varcomp_kss_cmg.mata` |
| test | `cmgtest` | `on` | `varcomp_kss/cmg/generated/cmg_test.mata` |
...
API 7 changes ownership and callable surface only. Its numerical core is the
...
- `vckss_cmg__api_level() == 7`;
- `vckss_cmg__numeric_mode() == "off"`;
- design label `gpl-cmg-mata-degree3-hybrid-v7-varcomp-kss-component`.
...
certificate. `varcomp_kss` retains the exact original-system action,
