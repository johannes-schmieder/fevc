...
*! generator_api 4
*! namespace cmgtest
*! canonical_template_sha256 43b74a0f214213f930d63606363ce43dfd78dc69e0e41c55865db4237b047fdd
*! generated_section_sha256 a7d78580b99715f7ae1709777fb6a120d6e12048d5530dcaae2f9b4f1e076508
...
{
    return("gpl-cmg-mata-degree3-hybrid-v7-varcomp-kss-component")
}
...
struct cmgtest__aggregation_result
{
    string scalar status
    string scalar message
...
struct cmgtest__hierarchy
{
    string scalar status
    string scalar message
    struct cmgtest__options scalar options
    real scalar n_level
    pointer(struct cmgtest__level scalar) rowvector level
...
struct cmgtest__preflight_result
{
    string scalar status
    string scalar message
    string scalar route
...
{
    struct cmgtest__cells scalar out
...
struct cmgtest__graph scalar cmgtest__empty_graph()
{
    struct cmgtest__graph scalar out
...
struct cmgtest__options scalar cmgtest__options_default()
{
...
struct cmgtest__options scalar cmgtest__options_resource(
...
struct cmgtest__forest scalar cmgtest__empty_forest()
{
    struct cmgtest__forest scalar out
...
struct cmgtest__aggregation_result scalar cmgtest__empty_aggregation()
{
...
struct cmgtest__hierarchy scalar cmgtest__empty_hierarchy()
{
    struct cmgtest__hierarchy scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy"
    out.options = cmgtest__options_default()
    out.n_level = 0
...
struct cmgtest__level scalar cmgtest__empty_level()
{
    struct cmgtest__level scalar out

    out.status = "INVALID_INPUT"
    out.message = "invalid hierarchy level"
    out.graph = cmgtest__empty_graph()
    out.component = J(0,1,.)
    out.n_component = 0
...
pointer(struct cmgtest__level scalar) scalar cmgtest__store_level(
    struct cmgtest__level scalar value)
{
    struct cmgtest__level scalar stored

    stored = value
    return(&stored)
}

pointer(real colvector) scalar cmgtest__store_colvector(
...
pointer(real matrix) scalar cmgtest__store_matrix(real matrix value)
{
    real matrix stored
...
struct cmgtest__apply_result scalar cmgtest__empty_apply_result()
{
    struct cmgtest__apply_result scalar out
...
struct cmgtest__preflight_result scalar cmgtest__empty_preflight()
{
    struct cmgtest__preflight_result scalar out
...
struct cmgtest__route_result scalar cmgtest__empty_route()
{
    struct cmgtest__route_result scalar out
...
struct cmgtest__workspace scalar cmgtest__empty_workspace()
{
    struct cmgtest__workspace scalar out
...
struct cmgtest__diag_result scalar cmgtest__empty_diagnostics()
{
    struct cmgtest__diag_result scalar out
...
real matrix cmgtest__lex_panel(
    real matrix values,
...
real scalar cmgtest__dense_ids(real colvector identifier)
{
...
struct cmgtest__graph scalar cmgtest__graph_finalize(
    struct cmgtest__graph scalar graph)
{
...
real scalar cmgtest__find_root(
...
real colvector cmgtest__components(
    struct cmgtest__graph scalar graph)
{
    ...
    for (edge=1; edge<=graph.n_edge; edge++) {
        u = graph.u[edge]
        v = graph.v[edge]
        root_u = cmgtest__find_root(parent,u)
        root_v = cmgtest__find_root(parent,v)
    ...
    for (vertex=1; vertex<=graph.n_vertex; vertex++) {
        ...
        root = cmgtest__find_root(parent,u)
...
real matrix cmgtest__graph_action(
    struct cmgtest__graph scalar graph,
...
real scalar cmgtest__pilot_cap(real scalar planned_rhs)
{
...
struct cmgtest__preflight_result scalar cmgtest__preflight(
    ...
    struct cmgtest__options scalar options)
{
    ...
    if (cells.status != "CONVERGED" |
        ...
        !cmgtest__options_valid(options)) return(out)
    predicted_edges = 0
    ...
    out.pilot_cap = cmgtest__pilot_cap(max((planned_rhs,8)))
...
real scalar cmgtest__park_miller_step(real scalar state)
{
...
real matrix cmgtest__pilot_rhs(
    real colvector firm_key,
    real colvector component)
{
    ...
    for (position=1; position<=rows(firm_key); position++) {
        ...
        state[1] = cmgtest__park_miller_step(state[1])
        state[2] = cmgtest__park_miller_step(state[2])
...
struct cmgtest__route_result scalar cmgtest__route_decide(
    real scalar planned_rhs,
...
real colvector cmgtest__indexed_sum(
...
real colvector cmgtest__forest_depth(real colvector parent)
{
...
struct cmgtest__aggregation_result scalar cmgtest__aggregate_gpl(
    struct cmgtest__graph scalar graph,
    real colvector component,
    struct cmgtest__options scalar options)
{
    ...
    depth = cmgtest__forest_depth(parent)
    ...
    for (one_depth=max(depth); one_depth>=1; one_depth--) {
        ...
        if (rows(nodes) > 0) {
            contribution = cmgtest__indexed_sum(
    ...
    if (rows(active) > 0) {
        ...
        internal = cmgtest__indexed_sum(incidence_vertex,
...
struct cmgtest__forest scalar cmgtest__forest_select(
    struct cmgtest__graph scalar graph,
    real colvector component,
    struct cmgtest__options scalar options)
{
    ...
    if (rows(active) > 0) {
        forest_volume = cmgtest__indexed_sum(
...
real colvector cmgtest__cluster_members(
    real colvector head,
...
real colvector cmgtest__mask_side(
    real scalar mask,
...
real scalar cmgtest__mask_lex_less(
    ...
    real scalar cluster_size)
{
    ...
    candidate_side = cmgtest__mask_side(candidate,cluster_size)
    incumbent_side = cmgtest__mask_side(incumbent,cluster_size)
...
real scalar cmgtest__cluster_bad_mask(
    struct cmgtest__graph scalar graph,
    ...
    real scalar awd)
{
    ...
    for (candidate=2; candidate<=length(location); candidate++) {
        ...
        if (cmgtest__mask_lex_less(candidate_mask,best_mask,size)) {
...
struct cmgtest__aggregation_result scalar cmgtest__aggregate_vertices(
    struct cmgtest__graph scalar graph,
    real colvector component,
    struct cmgtest__forest scalar forest,
    struct cmgtest__options scalar options)
{
    struct cmgtest__aggregation_result scalar out
    ...
    for (seed_position=1; seed_position<=graph.n_vertex; seed_position++) {
        ...
        while (cluster_size[cluster_id] < options.target_size) {
            best_edge = 0
            best_rank = .
            members = cmgtest__cluster_members(
    ...
    while (screen_cursor <= cluster_count) {
        ...
        bad_mask = cmgtest__cluster_bad_mask(
        ...
        size = rows(members)
        side = cmgtest__mask_side(bad_mask,size)
        member_map[members] = (1::size)
    ...
    real scalar scratch_bytes)
{
    ...
    out.auxiliary_worker = J(0,1,.)
    out.status = "CONVERGED"
    out = cmgtest__graph_finalize(out)
...
struct cmgtest__level scalar cmgtest__level_prepare_cert(
    struct cmgtest__graph scalar graph,
    real colvector component)
{
    struct cmgtest__level scalar out
...
struct cmgtest__level scalar cmgtest__level_prepare(
    struct cmgtest__graph scalar graph)
{
    struct cmgtest__level scalar out
    real colvector component
    ...
    return(cmgtest__level_prepare_cert(graph,component))
}

struct cmgtest__level scalar cmgtest__coarse_prepare(
    struct cmgtest__level scalar level,
    struct cmgtest__options scalar options)
{
    ...
    for (one_component=1; one_component<=level.n_component;
        ...
        level.coarse_keep[one_component] =
            cmgtest__store_colvector(keep)
        level.coarse_scale[one_component] =
            cmgtest__store_colvector(scale)
        level.coarse_factor[one_component] =
            cmgtest__store_matrix(factor)
...
real matrix cmgtest__component_project(
    struct cmgtest__level scalar level,
    real matrix argument)
{
...
real matrix cmgtest__coarse_apply(
    struct cmgtest__level scalar level,
    real matrix right_hand_side)
{
    ...
    compatible = cmgtest__component_project(level,right_hand_side)
    ...
    out = cmgtest__component_project(level,out)
...
real scalar cmgtest__options_valid(
    struct cmgtest__options scalar options)
{
...
struct cmgtest__hierarchy scalar cmgtest__hierarchy_mode(
    struct cmgtest__graph scalar graph,
    struct cmgtest__options scalar options,
    string scalar algorithm)
{
    ...
    struct cmgtest__level scalar level
    struct cmgtest__forest scalar forest
    struct cmgtest__aggregation_result scalar aggregation_result
    ...
    out.options = options
    if (graph.status != "CONVERGED" | !cmgtest__options_valid(options) |
    ...
    while (1) {
        ...
        if (out.n_level == 0) {
            ...
            level = cmgtest__level_prepare_cert(
        ...
        component_size = level.component_panel[.,2]-
            level.component_panel[.,1]:+1
        if (max(component_size) <= options.coarse_max) {
            level = cmgtest__coarse_prepare(level,options)
            if (level.status != "CONVERGED") {
                out.status = level.status
                out.message = level.message
        ...
        if (algorithm == "STEINER_MATA" &
            ...
            aggregation_result = cmgtest__aggregate_gpl(
...
struct cmgtest__hierarchy scalar cmgtest__hierarchy_build(
    ...
    struct cmgtest__options scalar options)
{
    return(cmgtest__hierarchy_mode(
        graph,options,"STEINER_MATA"))
}

struct cmgtest__hierarchy scalar cmgtest__hierarchy_v5(
    struct cmgtest__graph scalar graph,
    struct cmgtest__options scalar options)
{
    return(cmgtest__hierarchy_mode(
...
struct cmgtest__workspace scalar cmgtest__workspace_init(
    struct cmgtest__hierarchy scalar hierarchy,
    real scalar batch_capacity,
    real scalar memory_cap_bytes)
{
    ...
    for (level_index=1; level_index<=hierarchy.n_level; level_index++) {
        level = *hierarchy.level[level_index]
        out.compatible[level_index] = cmgtest__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.iterate[level_index] = cmgtest__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
        out.work[level_index] = cmgtest__store_matrix(
            J(level.graph.n_vertex,batch_capacity,0))
    }
    out.status = "CONVERGED"
...
struct cmgtest__diag_result scalar cmgtest__diagnostics(
    struct cmgtest__hierarchy scalar hierarchy)
{
    struct cmgtest__diag_result scalar out
    struct cmgtest__level scalar level
...
real matrix cmgtest__apply_level(
    struct cmgtest__hierarchy scalar hierarchy,
    real scalar level_index,
    real matrix right_hand_side)
{
    ...
    if (level_index == hierarchy.n_level) {
        return(cmgtest__coarse_apply(level,compatible))
    }
    out = cmgtest__smooth(level,compatible,hierarchy.options.omega)
    if (rows(out) != level.graph.n_vertex) return(J(0,0,.))
    action = cmgtest__graph_action(
    ...
    correction = cmgtest__apply_level(
...
struct cmgtest__apply_result scalar cmgtest__apply(
    struct cmgtest__hierarchy scalar hierarchy,
    real matrix right_hand_side)
{
    struct cmgtest__apply_result scalar out
    struct cmgtest__level scalar fine
    real colvector component_size
    real scalar one_component, vertex

    out = cmgtest__empty_apply_result()
    if (hierarchy.status != "CONVERGED" | hierarchy.n_level < 1 |
    ...
    out.value = cmgtest__apply_level(hierarchy,1,right_hand_side)
...
real scalar cmgtest__project_self(
    struct cmgtest__level scalar level,
...
real scalar cmgtest__smooth_into(
    struct cmgtest__level scalar level,
    real matrix argument,
    real scalar column_count,
    real scalar omega,
...
real scalar cmgtest__graph_into(
    struct cmgtest__graph scalar graph,
    real matrix argument,
    real scalar column_count,
    real scalar scratch_bytes,
...
real scalar cmgtest__coarse_into(
    struct cmgtest__level scalar level,
    real matrix right_hand_side,
    real scalar column_count,
    real matrix out)
{
    ...
    return(cmgtest__project_self(level,column_count,out))
}

real scalar cmgtest__apply_ws_level(
    struct cmgtest__hierarchy scalar hierarchy,
    ...
    real matrix right_hand_side)
{
    ...
    if (level_index == hierarchy.n_level) {
        return(cmgtest__coarse_into(
            level,*compatible,column_count,*iterate))
    }

    if (!cmgtest__smooth_into(level,*compatible,column_count,
        hierarchy.options.omega,*iterate)) return(0)
    if (!cmgtest__graph_into(level.graph,*iterate,column_count,
    ...
    if (!cmgtest__graph_into(level.graph,*iterate,column_count,
        hierarchy.options.action_scratch_bytes,*work)) return(0)
    (*work)[.,used] = (*compatible)[.,used]-(*work)[.,used]
    if (!cmgtest__smooth_into(level,*work,column_count,
        hierarchy.options.omega,*compatible)) return(0)
    (*iterate)[.,used] = (*iterate)[.,used]+(*compatible)[.,used]
    return(cmgtest__project_self(level,column_count,*iterate))
}

struct cmgtest__apply_result scalar cmgtest__workspace_apply(
    struct cmgtest__hierarchy scalar hierarchy,
    struct cmgtest__workspace scalar workspace,
    real matrix right_hand_side)
{
...
real matrix cmgtest__project_labels(
...
struct cmgtest__apply_result scalar cmgtest__apply_kss(
    struct cmgtest__hierarchy scalar hierarchy,
    real matrix firm_rhs)
{
    struct cmgtest__apply_result scalar out, core
    struct cmgtest__level scalar fine
    ...
    compatible = cmgtest__project_labels(firm_component,firm_rhs)
    ...
    core = cmgtest__apply(hierarchy,injected)
    if (core.status != "CONVERGED") return(core)
    out = core
    out.value = cmgtest__project_labels(
...
struct cmgtest__cells scalar cmgtest__cells_prepare(
    real colvector worker,
    real colvector firm,
    real colvector weight,
    real colvector worker_key,
    real colvector firm_key)
{
    ...
    if (!cmgtest__dense_ids(worker) | !cmgtest__dense_ids(firm)) {
    ...
    pair_panel = cmgtest__lex_panel(pair_values,pair_order)
...
struct cmgtest__graph scalar cmgtest__hybrid_build(
    struct cmgtest__cells scalar cells)
{
    struct cmgtest__graph scalar out
...
real matrix cmgtest__dense_schur(struct cmgtest__cells scalar cells)
{
    real matrix out
...
real matrix cmgtest__dense_laplacian(struct cmgtest__graph scalar graph)
{
    real matrix out
...
real matrix cmgtest__dense_hybrid_schur(
    struct cmgtest__graph scalar graph)
{
