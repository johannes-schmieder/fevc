...
foreach nuisance in joint fixedoffset {
    quietly varcomp_kss y control, worker(worker) firm(firm)          ///
...
quietly varcomp_kss y i.category, worker(worker) firm(firm)           ///
...
foreach nuisance in joint fixedoffset {
    quietly varcomp_kss y control [fw=frequency], worker(worker) firm(firm) ///
...
quietly varcomp_kss y if eligible in 2/7, worker(worker) firm(firm)   ///
...
capture quietly varcomp_kss y, worker(worker) firm(firm)             ///
    deletion(match) deletionid(deletion_id) algorithm(exact)         ///
...
capture quietly varcomp_kss y control, worker(worker) firm(firm)     ///
...
capture quietly varcomp_kss y zero_control, worker(worker) firm(firm) ///
...
capture quietly varcomp_kss y spike_control, worker(worker) firm(firm) ///
    deletion(observation) algorithm(exact) backend(rust) nodisplay
assert _rc != 0
...
di as result "VARCOMP_KSS RUST PUBLIC EXACT PASS"
exit 0
