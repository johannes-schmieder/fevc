...
local oldpwd `"`c(pwd)'"'
capture confirm file "varcomp_kss/varcomp_kss_rng.mata"
if _rc {
    capture confirm file "../../varcomp_kss_rng.mata"
    if _rc exit 601
    quietly cd "../.."
    local pkgroot `"`c(pwd)'"'
}
...
assert(vckss_rng__build_id() ==
    "varcomp-kss-rng-numeric-ranks-v4")
assert(vckss_rng__invariant_version() == "KSS-RNG-K1-INVARIANT-V1")
assert(vckss_rng__k1_recommendation() == "per_domain_stream_cursor")
assert(vckss_rng__max_binomial_trials() == 100000000000)
assert(vckss_rng__maximum_exact_integer() == 2^53-1)
assert(vckss_rng__domain_stream("leverage") == 1)
assert(vckss_rng__domain_stream("target") == 2)
...
else assert(vckss_rng__production_contract() == "")
assert(vckss_rng__trials_ok((2^53-1)) == 1)
assert(vckss_rng__trials_ok((2^53-1 \ 1)) == 0)
end
...
trials = (7 \ 1 \ 19 \ 2)
permutation = (3 \ 1 \ 4 \ 2)
streams_before = vckss_rng__capture_streams((1 \ 2))
assert(streams_before.status == "OK")

cursor_all = vckss_rng__open_cursor(8675309,"leverage",ranks,trials)
cursor_split = vckss_rng__open_cursor(8675309,"leverage",ranks,trials)
cursor_shuffled = vckss_rng__open_cursor(
...
all_atoms = vckss_rng__cursor_next(&cursor_all,3)
first_atoms = vckss_rng__cursor_next(&cursor_split,1)
later_atoms = vckss_rng__cursor_next(&cursor_split,2)
shuffled_atoms = vckss_rng__cursor_next(&cursor_shuffled,3)
...
target_cursor = vckss_rng__open_cursor(8675309,"target",ranks,trials)
target_atoms = vckss_rng__cursor_next(&target_cursor,3)
...
range_cursor = vckss_rng__open_cursor(8675309,"leverage",ranks,trials)
range_cursor.next_probe = 16384
range_atom = vckss_rng__cursor_next(&range_cursor,1)
assert(range_atom.status == "OK")
assert(range_atom.probe_start == 16384)

streams_after = vckss_rng__capture_streams((1 \ 2))
assert(streams_after.status == "OK")
assert(streams_after.state == streams_before.state)

shape_before = vckss_rng__capture_streams(J(1,1,30001))
shape = vckss_rng__compare_call_shapes(
    8675309,30001,(1 \ 2 \ 7 \ 19 \ 100))
shape_after = vckss_rng__capture_streams(J(1,1,30001))
assert(shape.status == "OK")
assert(shape.atoms_equal == 1 & shape.states_equal == 1)
assert(shape_after.state == shape_before.state)

chunk_before = vckss_rng__capture_streams(J(1,1,30002))
assert(vckss_rng__set_stream_seed(30002,8675309) == 0)
chunked = vckss_rng__binomial_atom_limit(17,10)
assert(chunked[2] == 2)
assert(abs(chunked[1]) <= 17 & mod(chunked[1]+17,2) == 0)
assert(vckss_rng__restore_streams(chunk_before) == 0)

bad = vckss_rng__open_cursor(
    8675309,"leverage",(7 \ 7),(1 \ 1))
assert(bad.status == "RNG_SEMANTIC_KEY_INVALID")
too_large = vckss_rng__open_cursor(
