...
capture confirm file "varcomp_kss/varcomp_kss_cmg.mata"
if _rc {
    di as error "run the forced-CMG test from the repository root"
    exit 601
}
capture mata: vckss__api_level()
if _rc quietly do "varcomp_kss/varcomp_kss.mata"
capture mata: vckss_cmg__api_level()
if _rc quietly do "varcomp_kss/varcomp_kss_cmg.mata"
quietly do "varcomp_kss/tests/support/vckss_cmg_adapter.mata"
mata: assert(vckss_cmg__api_level() == 7)
mata: assert(vckss_cmg__design_label() == "gpl-cmg-mata-degree3-hybrid-v7-varcomp-kss-component")
...
diagonal = vckss__fe_solve_matrix(design,rhs,1e-8,20000)
