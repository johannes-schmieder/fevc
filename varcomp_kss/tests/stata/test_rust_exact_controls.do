...
varcomp_kss_rust requestcapability, algorithm(exact)                 ///
...
varcomp_kss_rust requestcapability, algorithm(exact) deletion(match) ///
...
foreach nuisance in joint fixedoffset {
    varcomp_kss_rust prepare worker firm deletion_id outcome frequency  ///
    ...
    varcomp_kss_rust solve `handle', algorithm(exact) deletion(match)   ///
...
assert r(deletion_mode_code) == 2
varcomp_kss_rust solve `handle', algorithm(exact) deletion(observation) ///
...
di as result "VARCOMP_KSS RUST EXACT CONTROLS PASS"
exit 0
