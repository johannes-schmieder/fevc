...
mata: mata clear
capture noisily do "varcomp_kss/benchmarks/kss_scale_fixtures.mata"
if _rc quietly do "../../benchmarks/kss_scale_fixtures.mata"
capture noisily do "varcomp_kss/benchmarks/kss_scale_fixtures.do"
...
vckss_scale_fixture, design(well-connected) copies(4) ///
...
sort shuffle
drop shuffle
vckss_scale_fixture, design(well_connected) copies(4) ///
    worker(worker) firm(firm) deletionid(deletion_unit) outcome(y) ///
    frequency(frequency) target(target) copyvar(copy_id) ///
    connectorvar(connector) rowkey(observation_key)
cf _all using `well'
...
use `base', clear
vckss_scale_fixture, design(ring) copies(4) ///
    worker(worker) firm(firm) deletionid(deletion_unit) outcome(y) ///
    frequency(frequency) target(target) copyvar(copy_id) ///
    connectorvar(connector) rowkey(observation_key)
...
vckss_scale_fixture, design(well_connected) copies(2) ///
    worker(worker) firm(firm) deletionid(deletion_unit) outcome(y) ///
...
capture noisily vckss_scale_fixture, design(ring) copies(2) ///
    worker(worker) firm(firm) deletionid(deletion_unit) outcome(y)
assert _rc == 459
