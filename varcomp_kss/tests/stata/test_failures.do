...
capture noisily varcomp_kss y [fw=frequency], worker(worker) firm(firm) ///
...
capture noisily varcomp_kss y constant, worker(worker) firm(firm) ///
...
foreach deletion_mode in observation match {
    ...
    foreach nuisance_mode in joint fixedoffset {
        capture noisily varcomp_kss y explicit_zero, worker(worker) firm(firm) ///
...
capture noisily varcomp_kss y negative_zero, worker(worker) firm(firm) ///
...
capture noisily varcomp_kss y explicit_zero [fw=frequency_two], ///
...
capture noisily varcomp_kss y basis_one transformed_zero, worker(worker) ///
...
// Every JLA route that materializes literal-copy probe state has an explicit,
// typed allocation boundary before leverage or target probes can allocate.
capture noisily varcomp_kss y, worker(worker) firm(firm) ///
...
// Reviewer N's two-K(2,2) component witness has equal exact component masses
// but order-dependent rounded sums above 2^53.  API 13 rejects the literal
// total before graph ranking in both identifier labelings.
preserve
clear
set obs 8
generate long worker = cond(_n<=4, 1+floor((_n-1)/2), ///
    3+floor((_n-5)/2))
generate long firm = cond(_n<=4, 1+mod(_n-1,2), ///
    3+mod(_n-5,2))
generate double y = _n
generate double frequency = 2^52-20
replace frequency = 2^52-19 in 3/6
generate long worker_relabel = worker
replace worker_relabel = 3-worker in 1/4
replace worker_relabel = 7-worker in 5/8

capture noisily varcomp_kss y [fw=frequency], worker(worker) firm(firm) ///
    deletion(match) algorithm(exact) nodisplay
assert _rc == 498
assert "`e(withholding_status)'" == "PHYSICAL_TOTAL_LIMIT"
capture noisily varcomp_kss y [fw=frequency], worker(worker_relabel) ///
    firm(firm) deletion(match) algorithm(exact) nodisplay
assert _rc == 498
assert "`e(withholding_status)'" == "PHYSICAL_TOTAL_LIMIT"
restore
...
quietly varcomp_kss y, worker(worker) firm(firm) ///
...
generate double y = z
capture noisily varcomp_kss y z, worker(worker) firm(firm) ///
...
capture noisily varcomp_kss y block_only_control, worker(worker) firm(firm) ///
...
capture noisily varcomp_kss y z1 z2, worker(worker) firm(firm) ///
    deletion(match) deletionid(match) algorithm(exact) nodisplay
assert _rc == 498
assert inlist("`e(withholding_status)'", "NONESTIMABLE_DELETION", ///
