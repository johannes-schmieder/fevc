...
if _rc {
    capture confirm file "../../varcomp_kss.mata"
    if _rc exit 601
    quietly do "../../varcomp_kss.mata"
}
else quietly do "varcomp_kss/varcomp_kss.mata"
...
mata:
struct vckss_preconditioner_result scalar test__exact_apply(
    ...
    real matrix residual)
{
    ...
    laplacian = vckss__fe_schur_action(design,I(design.firm_levels))
...
struct vckss_solver_backend scalar test__exact_backend(
    string scalar route,
...
struct vckss_preconditioner_result scalar test__false_exact_apply(
    pointer scalar context,
    struct vckss_fe_design scalar design,
    real matrix residual)
{
...
direct = vckss__fe_solve_matrix_backend(design,rhs,1e-10,20000,backend)
ordinary = vckss__fe_solve_matrix(design,rhs,1e-10,20000)
...
assert(mreldif(direct.prediction,
    vckss__fe_predict(design,direct.coefficient)) < 1e-14)

backend = test__exact_backend("TEST_FALSE_EXACT",&test__false_exact_apply())
rejected = vckss__fe_solve_matrix_backend(
