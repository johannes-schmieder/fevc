...
quietly do "varcomp_kss/varcomp_kss_rng.mata"
quietly do "varcomp_kss/varcomp_kss_solver.mata"
...
void test_structural_routes()
{
    ...
    struct vckss_route_result scalar automatic, diagonal, cmg
    ...
    automatic = vckss_solver__jla_routed(
    ...
    diagonal = vckss_solver__jla_routed(
        y,worker,firm,J(9,0,.),frequency,target,deletion_id,
        "match","joint",4,2,8675309,1e-8,1000,1e-10,1e-10,10000,
        "DIAGONAL",1024^3)
...
assert(vckss_rng__guard_begin() == 0)
test_structural_routes()
assert(vckss_rng__guard_restore() == 0)
end
