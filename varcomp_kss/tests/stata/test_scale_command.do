...
quietly timer on 70
quietly varcomp_kss y [fw=frequency] if scope, worker(worker) firm(firm) ///
...
// reinterpretations.
quietly varcomp_kss y control [fw=frequency] if scope,                  ///
...
capture noisily varcomp_kss y control [fw=frequency] if scope,          ///
    worker(worker) firm(firm) deletion(match) deletionid(match)    ///
    targetweight(target) probeorder(atom_key) algorithm(jla)       ///
...
capture noisily varcomp_kss y [fw=frequency] if scope,                 ///
