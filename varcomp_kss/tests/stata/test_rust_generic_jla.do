...
varcomp_kss_rust requestcapability, algorithm(jla) deletion(match)   ///
...
foreach deletion in match observation {
    ...
    foreach nuisance in joint fixedoffset {
        ...
        forvalues q = 0/1 {
            ...
            quietly varcomp_kss_rust requestcapability, algorithm(jla) ///
            ...
            quietly varcomp_kss_rust prepare worker firm deletion_id outcome ///
            ...
            quietly varcomp_kss_rust solve `handle', algorithm(jla)          ///
                deletion(`deletion') nuisance(`nuisance') route(diagonal)    ///
...
local handle = r(handle)
capture noisily varcomp_kss_rust solve `handle', algorithm(jla) deletion(match) ///
...
quietly varcomp_kss_rust snapshot
assert r(state) == 4
capture noisily varcomp_kss_rust result `handle'
assert _rc == 198
quietly varcomp_kss_rust release `handle'
quietly varcomp_kss_rust release `handle'
quietly varcomp_kss_rust snapshot
...
foreach corruption in bad_schema0_rows bad_v1_copy bad_v2_copy {
    ...
    quietly varcomp_kss_rust prepare worker firm deletion_id outcome frequency ///
        target_weight, cleanup memorygib(1) deletion(match)
    local handle = r(handle)
    quietly varcomp_kss_rust solve `handle', algorithm(exact) deletion(match) ///
...
di as result "VARCOMP_KSS RUST GENERIC JLA PASS"
exit 0
