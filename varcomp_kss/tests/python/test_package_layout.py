...
def test_package_manifest_is_complete() -> None:
    manifest = (ROOT / "varcomp_kss.pkg").read_text(encoding="utf-8").splitlines()
    shipped = {
    ...
    assert shipped == {
        "varcomp_kss.ado",
        "varcomp_kss.mata",
        "varcomp_kss_graph.mata",
        "varcomp_kss_cmg.mata",
        "varcomp_kss_solver.mata",
        "varcomp_kss_rng.mata",
        "varcomp_kss_scale.mata",
        "varcomp_kss_resource.mata",
...
def test_version_identifiers_agree() -> None:
    assert VERSION in (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8")
    assert VERSION in (ROOT / "varcomp_kss.mata").read_text(encoding="utf-8")
    assert VERSION in (ROOT / "varcomp_kss.sthlp").read_text(encoding="utf-8")
    toc = (ROOT / "stata.toc").read_text(encoding="utf-8").splitlines()
    assert toc[0] == f"v {VERSION}"


def test_mata_api_guard_agrees() -> None:
    ado = (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8")
    ...
    assert "varcomp-kss-solver-api26-gpl-mata-cmg" in solver
    ...
    cmg = (ROOT / "varcomp_kss_cmg.mata").read_text(encoding="utf-8")
    assert "vckss_cmg__api_level()" in cmg
    assert "return(7)" in cmg
...
def test_package_records_internal_license_boundary() -> None:
    manifest = (ROOT / "varcomp_kss.pkg").read_text(encoding="utf-8").lower()
    readme = " ".join(
...
def test_public_command_is_a_hard_cut_without_predecessor_alias() -> None:
    ado = (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8")
    assert "program define varcomp_kss, eclass" in ado
    assert 'ereturn local cmd "varcomp_kss"' in ado
    assert 'ereturn local cmdline `"varcomp_kss `0\'"\'' in ado
    assert not (ROOT / "kss_bc.ado").exists()


def test_mata_uses_valid_noncolliding_profile_timers() -> None:
    runtime = (ROOT / "varcomp_kss.mata").read_text(encoding="utf-8")
    timer_ids = {
        int(value)
...
def test_only_factor_metadata_can_remove_an_omitted_control() -> None:
    runtime = (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8")
...
def test_cmg_route_is_installed_behind_package_solver_contract() -> None:
    ado = (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8").lower()
    mata = (ROOT / "varcomp_kss.mata").read_text(encoding="utf-8")
    solver = (ROOT / "varcomp_kss_solver.mata").read_text(encoding="utf-8")
    ...
    manifest = (ROOT / "varcomp_kss.pkg").read_text(encoding="utf-8")
    assert "preconditioner(string)" in ado
    assert "varcomp_kss/cmg" not in manifest
    assert "f varcomp_kss_cmg.mata" in manifest
...
def test_separations_harness_is_read_only_and_aggregate_collectable() -> None:
    ...
    assert "VARCOMP_KSS SEPARATIONS MATLAB-ONLY EVIDENCE PASS" in validator
    ...
    assert 'case "$KSS_MATLAB_RUN" in /projectnb/welfgr/varcomp-kss/runs/*)' in sample_wrapper
