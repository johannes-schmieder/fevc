...
MODULE = ROOT / "varcomp_kss_rng.mata"
...
def test_production_contract_is_runtime_versioned_and_fail_closed() -> None:
    ...
    production = source.split(
        "string scalar vckss_rng__production_contract()", 1
    )[1].split("real scalar vckss_rng__max_binomial_trials", 1)[0]
...
def test_production_cursor_has_no_legacy_probe_registry_cap() -> None:
    source = MODULE.read_text(encoding="utf-8")
    cursor = source.split("vckss_rng__open_cursor", 1)[1]
    assert "vckss_rng__maximum_probes()" not in cursor
    assert "vckss_rng__maximum_exact_integer()" in cursor
    legacy = source.split("vckss_rng__generate(", 1)[1].split(
        "vckss_rng__open_cursor", 1
    )[0]
    assert "vckss_rng__maximum_probes()" in legacy


def test_contract_keys_exclude_execution_path_choices() -> None:
    source = MODULE.read_text(encoding="utf-8")
    struct = source.split("struct vckss_rng__result", 1)[1].split("}", 1)[0]
    for required in (
        "contract_version",
        "candidate",
        "domain",
...
def test_module_registers_large_count_and_call_shape_gates() -> None:
    ...
    assert "vckss_rng__compare_call_shapes" in source
    assert "vckss_rng__binomial_atom_limit" in source
...
def test_candidate_recommendation_is_computed_from_paired_timing() -> None:
    source = MODULE.read_text(encoding="utf-8")
    assert "vckss_rng__benchmark" in source
...
def test_fixed_domain_candidate_has_nonreplaying_cursor_api() -> None:
    source = MODULE.read_text(encoding="utf-8")
    assert "struct vckss_rng__cursor" in source
    assert "vckss_rng__open_cursor" in source
    assert "vckss_rng__cursor_next" in source
    cursor = source.split("vckss_rng__cursor_next", 1)[1]
    ...
    assert "for (probe=1; probe<=finish; probe++)" not in cursor.split(
        "vckss_rng__compare_call_shapes", 1
    )[0]
...
def test_caller_rng_restoration_is_an_explicit_success_and_failure_gate() -> None:
    ...
    assert "struct vckss_rng__snapshot" in source
    assert "struct vckss_rng__stream_snapshot" in source
    assert "vckss_rng__capture_streams" in source
    assert "vckss_rng__restore_streams" in source
...
def test_semantic_order_is_sorted_and_dense_ids_are_not_an_input() -> None:
    ...
    canonical = source.split(
        "real colvector vckss_rng__canonical_order", 1
    )[1].split("real scalar vckss_rng__trials_ok", 1)[0]
...
def test_stata_gate_covers_partition_and_row_order_invariance() -> None:
    ...
    assert "vckss_rng__benchmark(" not in test
