...
SOURCE = (ROOT / "varcomp_kss_scale.mata").read_text(encoding="utf-8")
RUNTIME_SOURCE = (ROOT / "varcomp_kss_scale_runtime.mata").read_text(
    encoding="utf-8"
)
ADO_SOURCE = (ROOT / "varcomp_kss.ado").read_text(encoding="utf-8")
...
def test_mata_source_exposes_standalone_compressed_api() -> None:
    required = (
        ...
        "vckss_scale__diagnose(",
        "vckss_scale__prepare(",
        "vckss_scale__compact(",
        "vckss_scale__fe_transpose(",
        "vckss_scale__fe_schur_action(",
        "vckss_scale__fe_worker(",
        "vckss_scale__fe_predict(",
        "vckss_scale__fe_full_residual(",
        "vckss_scale__weighted_rss(",
        "vckss_scale__strata_prepare(",
        "vckss_scale__strata_to_cells(",
        "unit_cell_order",
        "unit_cell_panel",
    )
    ...
    stable_helper = SOURCE[
        SOURCE.index("vckss_scale__stable_groupsum(") :
        SOURCE.index("vckss_scale__integer_group_sum(")
    ]
    ...
    target_helper = SOURCE[
        SOURCE.index("vckss_scale__strata_prepare(") :
        SOURCE.index("vckss_scale__diagnose(")
    ]
    assert "tolerance" not in target_helper


def test_command_constructs_and_cleans_one_cached_compressed_state() -> None:
    assert ADO_SOURCE.count("vckss_srt__prepare(") == 1
    assert "vckss_scale__stata_diag(" not in ADO_SOURCE
    assert "vckss_scale__diagnose(" not in RUNTIME_SOURCE
    assert "vckss_scale__prepare(" not in RUNTIME_SOURCE
    assert "VCKSS_SCALE_RUNTIME = vckss_srt__empty()" not in RUNTIME_SOURCE

    prepare = SOURCE[
        SOURCE.index("void vckss_srt__prepare(") :
        SOURCE.index("void vckss_scale__stata_diag(")
    ]
    assert prepare.count("vckss_scale__prepare(") == 1
    assert "vckss_scale__diagnose(" not in prepare
    assert "diagnostic = design.diagnostic" in prepare

    wrapper = ADO_SOURCE[: ADO_SOURCE.index("program define _vckss_impl")]
    assert wrapper.count("vckss_scale_runtime__reset()") == 2
    command_prepare = ADO_SOURCE.index("capture noisily mata: vckss_srt__prepare(")
