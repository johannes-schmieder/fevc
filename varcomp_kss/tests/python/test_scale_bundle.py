...
KSS_ROOT = REPO_ROOT / "varcomp_kss"
...
def test_allowlist_closes_installed_runtime_and_streamlined_harness() -> None:
    ...
    assert builder.REQUIRED_MATLAB_HARNESS <= selected
    ...
    assert not any(path.suffix in {".dta", ".log", ".csv", ".tsv", ".mat"}
                   for path in rows)
    assert all(
            str(path) == "CODE_LICENSE.md" or
            str(path).startswith(("LICENSES/", "varcomp_kss/", "varcomp_kss/cmg/"))
        for path in rows
    )
    assert PurePosixPath(
        "varcomp_kss/benchmarks/prod_bundle_allowlist.txt"
    ) not in selected
    assert PurePosixPath(
        "varcomp_kss/benchmarks/scc/submit_rng_k1.sh"
    ) not in selected
    matlab_support = {
        path for path in rows
        if "benchmarks/matlab_scale/" in str(path)
    }
    assert matlab_support == builder.REQUIRED_MATLAB_HARNESS
...
def test_scale_bundle_is_deterministic_regular_and_manifest_complete() -> None:
    ...
    with tarfile.open(fileobj=io.BytesIO(tar_payload), mode="r:") as archive:
        ...
        assert member_by_name[
            "varcomp_kss/benchmarks/scc/deploy_scale_bundle.sh"
        ].mode == 0o755
...
def test_deployer_preserves_single_job_and_prod_boundaries() -> None:
    ...
    assert "/projectnb/welfgr/varcomp-kss/bundles/$bundle_sha" in script
