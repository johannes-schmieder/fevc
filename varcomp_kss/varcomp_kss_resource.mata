*! varcomp_kss KSS-STREAMLINE-1 resource planning
...
real scalar vckss_resource__hard_mem_bytes()
{
    ...
    return(56*vckss_resource__gib())
}

real scalar vckss_resource__hard_wall_secs()
{
    // Compatibility planning target, not an execution gate.
    return(12*60*60)
}

real scalar vckss_resource__wall_margin()
{
    return(0.50)
}

real scalar vckss_resource__runtime_rss()
{
    /*
...
real scalar vckss_resource__rng_call_upper()
{
...
struct vckss_resource_components
{
...
struct vckss_resource_forecast
{
    ...
    struct vckss_resource_components scalar components
...
struct vckss_resource_selection
{
    string scalar status
    string scalar message
    string scalar route
...
struct vckss_resource_reconciliation
{
    string scalar status
    string scalar message
...
struct vckss_resource_model
{
    ...
    struct vckss_resource_components scalar compressed_components
    struct vckss_resource_components scalar generic_components
    struct vckss_resource_forecast scalar compressed
    struct vckss_resource_forecast scalar generic
}

struct vckss_resource_components scalar vckss_resource__empty_components()
{
    struct vckss_resource_components scalar out
...
real rowvector vckss_resource__component_vector(
    struct vckss_resource_components scalar components)
{
    return((
...
string rowvector vckss_resource__component_names()
{
    return((
        "raw_stata_bytes",
        "cell_bytes",
...
real scalar vckss_resource__valid_components(
    struct vckss_resource_components scalar components)
{
    real rowvector values

    values = vckss_resource__component_vector(components)
    return(!missing(values) & min(values) >= 0)
}

struct vckss_resource_components scalar vckss_resource__components_from(
    real rowvector values)
{
    struct vckss_resource_components scalar out

    out = vckss_resource__empty_components()
...
real scalar vckss_resource__nonsolver_peak(
    string scalar route,
    struct vckss_resource_components scalar components)
{
    ...
    if (!(route == "compressed" | route == "generic") |
        !vckss_resource__valid_components(components)) return(.)
...
real scalar vckss_resource__alloc_nonsolver(
    string scalar route,
    struct vckss_resource_components scalar components,
    real scalar transition_peak_bytes)
{
    ...
    live_peak = vckss_resource__nonsolver_peak(route,components)
...
struct vckss_resource_forecast scalar vckss_resource__empty_forecast(
    string scalar route)
{
    struct vckss_resource_forecast scalar out
    ...
    out.admitted = 0
    out.before_rng = 1
    out.components = vckss_resource__empty_components()
    ...
    out.wall_headroom_fraction = vckss_resource__wall_margin()
    out.wall_admission_seconds = .
    out.hard_memory_bytes = vckss_resource__hard_mem_bytes()
    out.hard_wall_seconds = vckss_resource__hard_wall_secs()
...
struct vckss_resource_forecast scalar vckss_resource__forecast(
    string scalar route,
    struct vckss_resource_components scalar components,
    real scalar wall_forecast_upper_seconds,
    real scalar memory_headroom_fraction,
    ...
    real scalar hard_wall_seconds)
{
    ...
    out = vckss_resource__empty_forecast(route)
    out.components = components
    out.component_bytes = vckss_resource__component_vector(components)
    ...
    if (!(route == "compressed" | route == "generic") |
        !vckss_resource__valid_components(components) |
    ...
    out.transition_peak_bytes =
        ...
        components.runtime_resident_bytes
    out.non_solver_numerical_bytes =
        vckss_resource__alloc_nonsolver(
...
struct vckss_resource_forecast scalar vckss_resource__route_reconcile(
    struct vckss_resource_forecast scalar forecast,
    real scalar routed_solver_peak_bytes)
{
    struct vckss_resource_forecast scalar out
    real rowvector peaks
...
vckss_resource__forecast().
*/
struct vckss_resource_model scalar vckss_resource__model(
    real scalar n_rows,
    real scalar n_physical,
    ...
    real scalar hard_wall_seconds)
{
    struct vckss_resource_model scalar out
    ...
    out.compressed_components = vckss_resource__empty_components()
    out.generic_components = vckss_resource__empty_components()
    out.compressed = vckss_resource__empty_forecast("compressed")
    out.generic = vckss_resource__empty_forecast("generic")
    ...
    */
    ...
    out.generic = vckss_resource__forecast(
...
real rowvector vckss_resource__forecast_vector(
    struct vckss_resource_forecast scalar forecast)
{
    return((
...
struct vckss_resource_forecast scalar vckss_resource__forecast_default(
    string scalar route,
    struct vckss_resource_components scalar components,
    real scalar wall_forecast_upper_seconds)
{
    return(vckss_resource__forecast(
        route,components,wall_forecast_upper_seconds,0.30))
}

struct vckss_resource_selection scalar vckss_resource__empty_selection()
{
    struct vckss_resource_selection scalar out
...
struct vckss_resource_selection scalar vckss_resource__select_auto(
    real scalar compressed_eligible,
    struct vckss_resource_forecast scalar compressed,
    struct vckss_resource_forecast scalar generic)
{
    ...
    out = vckss_resource__empty_selection()
...
struct vckss_resource_reconciliation scalar vckss_resource__empty_recon()
{
    struct vckss_resource_reconciliation scalar out
...
struct vckss_resource_reconciliation scalar vckss_resource__reconcile(
    struct vckss_resource_forecast scalar forecast,
    real rowvector actual_phase_bytes,
    real scalar process_peak_bytes,
    real scalar qacct_maxvmem_bytes,
    real scalar actual_wall_seconds)
{
    ...
    out = vckss_resource__empty_recon()
...
void vckss_resource__stata_model(
    real scalar n_rows,
    real scalar n_physical,
    real scalar coefficient_cells,
    real scalar deletion_units,
    ...
    string scalar generic_phase_local)
{
    ...
    st_matrix(components_name,
        vckss_resource__component_vector(out.compressed_components) \
        vckss_resource__component_vector(out.generic_components))
    st_matrix(forecasts_name,
        vckss_resource__forecast_vector(out.compressed) \
        vckss_resource__forecast_vector(out.generic))
...
void vckss_resource__stata_route(
    string scalar route,
    string scalar components_name,
    ...
    string scalar phase_local)
{
    ...
    if (!valid_component_row) {
        components = vckss_resource__components_from(J(1,11,.))
    }
    else components = vckss_resource__components_from(
        registered_components[component_row,.])
    forecast = vckss_resource__forecast(
        route,components,wall_forecast_upper_seconds,0.30,
        hard_memory_bytes,hard_wall_seconds)
    forecast = vckss_resource__route_reconcile(
    ...
    st_matrix(forecast_name,vckss_resource__forecast_vector(forecast))
