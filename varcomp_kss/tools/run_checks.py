#!/usr/bin/env python3
"""Run the integrated local qualification gates for ``varcomp_kss``."""
...
def stata_executable() -> str | None:
    configured = os.environ.get("VARCOMP_KSS_STATA")
    if configured:
        return configured
...
def main() -> int:
    ...
    if stata is None:
        if os.environ.get("VARCOMP_KSS_REQUIRE_STATA") == "1":
            raise RuntimeError(
                "VARCOMP_KSS_REQUIRE_STATA=1 but no Stata/MP executable was found."
            )
        print(
            "\n== KSS Stata gates ==\n"
            "SKIP: no Stata/MP launcher found; set VARCOMP_KSS_STATA or "
            "VARCOMP_KSS_REQUIRE_STATA=1.",
            flush=True,
        )
        return 0
    ...
    run(
        ...
        [str(PYTHON), "varcomp_kss/cmg/tools/run_checks.py"],
        env=component_environment,
    )

    suite = ROOT / "varcomp_kss/tests/stata/run_all.do"
    run_stata(
        "KSS Stata quick suite",
        stata,
        suite,
        ["quick"],
        "VARCOMP_KSS TEST SUITE PASS: quick",
    )
    run_stata(
        ...
        suite,
        ["full"],
        "VARCOMP_KSS TEST SUITE PASS: full",
    )
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-install-") as temporary:
        install_root = Path(temporary) / "plus"
        install_root.mkdir()
        run_stata(
            "KSS clean-install smoke test",
            stata,
            ROOT / "varcomp_kss/tests/stata/test_install.do",
            [str(ROOT / "varcomp_kss"), str(install_root)],
            "VARCOMP_KSS INSTALL TEST PASS",
            Path(temporary),
        )
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-benchmark-") as temporary:
        output = Path(temporary)
        run_stata(
            "KSS synthetic benchmark harness smoke test",
            stata,
            ROOT / "varcomp_kss/benchmarks/synthetic_benchmark.do",
            [
                "local-gate",
                "smoke",
                "100",
            ...
            "VARCOMP_KSS BENCHMARK PASS: smoke",
        )
        validate_benchmark(output)
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-numopt-") as temporary:
        output = Path(temporary)
        commit = "0" * 40
        for route in ("b1", "cmg"):
            ...
            run_stata(
                ...
                stata,
                ROOT / "varcomp_kss/benchmarks/estimator_cmg_benchmark.do",
                [
                    "local-gate",
                    route,
                    "moderate",
                ...
                f"VARCOMP_KSS NUMOPT BENCHMARK PASS: {route} moderate",
            )
        run(
            "KSS paired B1/forced-CMG benchmark validation",
            [
                str(PYTHON),
                "varcomp_kss/benchmarks/validate_numopt.py",
                "--run-dir",
                str(output),
                "--expected-commit",
                commit,
    ...
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-separations-") as temporary:
        output = Path(temporary)
        run_stata(
            "KSS Separations preparation duplicate-key smoke test",
            stata,
            ROOT / "varcomp_kss/tests/stata/test_separations_prepare.do",
            [str(ROOT / "varcomp_kss"), str(output)],
            "VARCOMP_KSS SEPARATIONS PREPARE PASS: fixture",
        )
        validate_separations_preparation(output)
    with tempfile.TemporaryDirectory(
        prefix="varcomp-kss-matlab-sample-"
    ) as temporary:
        run_stata(
            "KSS MATLAB-retained sample and bridge-audit smoke test",
            stata,
            ROOT / "varcomp_kss/tests/stata/test_separations_matlab_sample.do",
            [str(ROOT / "varcomp_kss"), temporary],
            "PASS test_separations_matlab_sample.do",
        )
    print("\nVARCOMP_KSS LOCAL QUALIFICATION PASS", flush=True)
    return 0
