...
program define _vckss_impl, eclass sortpreserve
    ...
    if "`stayers'" == "both" & "`backend_selected'" != "mata" {
        quietly _vckss_post_failure "STAYER_HYBRID_BACKEND_UNSUPPORTED" ///
            "The mixed-deletion stayer hybrid is implemented only by the Mata backend."
        di as error "stayers(both) requires backend(mata)"
        exit 498
    }
    ...
    if `rust_public' & "`algorithm'" == "exact" {
        ...
        capture quietly varcomp_kss_rust requestcapability,      ///
    ...
    local expected_mata_build "varcomp-kss-api21-stayer-hybrid"
    ...
    if _rc {
        if `mata_runtime_loaded' {
            ...
            di as error "a different varcomp_kss Mata runtime is already loaded; restart Stata or run discard before retrying"
            exit 498
        }
        ...
        if _rc {
            ...
            di as error "the loaded varcomp_kss Mata runtime does not match this command build"
            exit 498
        }
    }
    ...
    if _rc {
        ...
        if _rc {
            ...
            di as error "varcomp_kss_graph.mata was not found on the Stata adopath"
            exit 601
        }
        quietly do `"`r(fn)'"'
        capture mata: assert(vckss_graph__api_level() == 21 &      ///
    ...
    if "`selected_algorithm'" == "jla" {
        ...
        if _rc {
            ...
            if _rc {
                ...
                di as error "varcomp_kss_lifecycle.ado was not found on the Stata adopath"
                exit 601
            }
            quietly do `"`r(fn)'"'
        }
        ...
        if _rc {
            ...
            if _rc {
                ...
                di as error "varcomp_kss_scale.mata was not found on the Stata adopath"
                exit 601
            }
            quietly do `"`r(fn)'"'
            capture mata: assert(vckss_scale__api_level() == 6 &   ///
        ...
        if _rc {
            ...
            if _rc {
                ...
                di as error "varcomp_kss_resource.mata was not found on the Stata adopath"
                exit 601
            }
            quietly do `"`r(fn)'"'
            capture mata: assert(vckss_resource__api_level() == 10 & ///
    ...
    if "`selected_algorithm'" == "exact" {
        ...
        capture mata: assert(vckss_cmg__api_level() == 7 &        ///
        ...
        if _rc {
            ...
            if _rc {
                ...
                di as error "varcomp_kss_cmg.mata was not found on the Stata adopath"
                exit 601
            }
            quietly do `"`r(fn)'"'
            capture mata: assert(vckss_cmg__api_level() == 7 &    ///
        ...
        if _rc {
            ...
            if _rc {
                ...
                di as error "varcomp_kss_solver.mata was not found on the Stata adopath"
                exit 601
            }
            quietly do `"`r(fn)'"'
            capture mata: assert(vckss_solver__api_level() == 26 & ///
        ...
        if "`engine_selected'" == "compressed" {
            ...
            if _rc {
                ...
                if _rc {
                    ...
                    di as error "varcomp_kss_scale_engine.mata was not found on the Stata adopath"
                    exit 601
                }
                quietly do `"`r(fn)'"'
                capture mata: assert(                             ///
            ...
            if _rc {
                ...
                if _rc {
                    ...
                    di as error "varcomp_kss_scale_runtime.mata was not found on the Stata adopath"
                    exit 601
                }
                quietly do `"`r(fn)'"'
                capture mata: assert(                             ///
    ...
    ereturn scalar numerical_mcse_available = ///
        ("`selected_algorithm'" == "jla")
    ereturn local cmd "varcomp_kss"
    ereturn local cmdline `"varcomp_kss `0'"'
    ereturn local version "0.3.0-dev"
    ereturn local model "linear"
    ereturn local correction_method "kss"
    ereturn local backend_requested "`backend_requested'"
...
program define _vckss_failure_guidance, rclass
    ...
    if inlist("`failure_status'", "INVALID_DEPVAR",              ///
        ...
        local suggestion "Use backend(mata) algorithm(exact) deletion(match), or use stayers(movers) for the ordinary mover headline."
    }
