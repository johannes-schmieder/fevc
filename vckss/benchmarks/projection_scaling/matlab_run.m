...
record = struct('schema','VCKSS-PROJECTION-SCALING-MATLAB-V1', ...
...
write_marker(fullfile(output_dir,'application.pass'), ...
    sprintf('VCKSS PROJECTION SCALING MATLAB PASS rows=%d',rows_expected));
fprintf('VCKSS PROJECTION SCALING MATLAB PASS rows=%d\n',rows_expected);
end
...
if ~condition
    error(['vckss:projectionScaling:' code],'%s',message);
end
end
