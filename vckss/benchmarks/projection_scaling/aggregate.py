...
def base_cell(
    ...
    return {
        ...
        "stata_baseline_rss_kib": (
            baseline if role == "vckss" and baseline is not None else ""
        ),
        "incremental_rss_kib": "",
...
def failed_cells(
    ...
    role_map = {"rust": "vckss", "matlab": "matlab"}
    ...
    if tree is not None and int(tree.get("whole_peak_rss_kib", 0)) > 0:
        ...
        if first_role == "vckss" and baseline is not None:
...
def successful_summaries(
    ...
    for rows in ROWS:
        vckss = find(rows, "vckss")
        matlab = find(rows, "matlab")
        if vckss is None or matlab is None:
            continue
        summaries.append(
            {
                "rows": rows,
...
def main() -> None:
    ...
    v24 = next(
        (
            item
            for item in summaries
            if item["rows"] == 24000 and item["role"] == "vckss"
        ),
        None,
    )
    v96 = next(
        (
            item
            for item in summaries
            if item["rows"] == 96000 and item["role"] == "vckss"
        ),
        None,
    )
    rss_growth = None
    ...
    result = {
        "schema": "VCKSS-PROJECTION-SCALING-SUMMARY-V2",
        "status": status,
        "source_commit": source_commit,
