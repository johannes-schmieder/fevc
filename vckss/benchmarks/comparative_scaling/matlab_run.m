...
weak_dimensions = strcmp(structure,'weak_d3') && ...
    mod(expected_workers,5)==0 && expected_firms == expected_workers/5+1601;
...
assert_vcs(ismember(active_cores,[1 2 4 8 16]) && probes==200 && seed>=1, ...
...
try
    ...
    identity = struct('schema','VCKSS-COMPARATIVE-SCALING-MATLAB-PROCESS-V1', ...
    ...
    record = struct('schema','VCKSS-COMPARATIVE-SCALING-MATLAB-V1', ...
    ...
    write_marker(fullfile(output_dir,'application.pass'), ...
        ['VCKSS_COMPARATIVE_SCALING_MATLAB_PASS ' experiment]);
    fprintf('VCKSS COMPARATIVE SCALING MATLAB PASS: %s\n',experiment);
catch exception
    ...
    failure = struct('schema','VCKSS-COMPARATIVE-SCALING-MATLAB-FAILURE-V1', ...
...
if ~condition
    error(['vckss:comparativeScaling:' code],'%s',message);
end
end
