...
% Compile the maintained MATLAB CMG MEX set exactly once for one SCC run.
if nargin~=2 || ~isfolder(matlab_root) || isfolder(output_dir) || isfile(output_dir)
    error('vckss:comparativeScaling:Preparation','Invalid MEX preparation arguments.');
end
mkdir(output_dir);
...
for index = 1:numel(names)
    mex('-silent','-largeArrayDims','-outdir',output_dir, ...
...
mex('-silent','-largeArrayDims',['-I' include_dir],'-outdir',output_dir, ...
    '-output','mx_d_preconditioner',sources{:});
fprintf('VCKSS COMPARATIVE SCALING MATLAB MEX PREPARE PASS\n');
end
