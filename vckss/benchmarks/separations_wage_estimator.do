...
timer on 81
if "`route'" == "exact" {
    capture noisily vckss y_minus_xb, worker(worker) firm(firm) ///
...
if `converged' {
    file write `marker' ///
        "VCKSS_SEPARATIONS_CONVERGED `route' `label' `source_commit'" _n
}
else {
    file write `marker' ///
        "VCKSS_SEPARATIONS_TYPED_FAILURE `route' `label' `route_status' `source_commit'" _n
}
file close `marker'
...
di as result "VCKSS SEPARATIONS ESTIMATOR PASS: `route' `label'"
exit 0
