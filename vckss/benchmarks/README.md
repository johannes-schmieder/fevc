...
The original KB6 drivers qualify the internal `vckss` point-estimation
...
```text
/projectnb/welfgr/vckss/runs/<run-id>/
```
...
```bash
bash vckss/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/vckss/runs/<run-id> "$PWD" <commit> portability

bash vckss/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/vckss/runs/<run-id> "$PWD" <commit> oracle

bash vckss/benchmarks/scc/submit_one.sh \
  /projectnb/welfgr/vckss/runs/<run-id> "$PWD" <commit> \
...
```bash
bash vckss/benchmarks/scc/collect_qacct.sh \
  /projectnb/welfgr/vckss/runs/<run-id> smoke
```
...
```bash
./.venv/bin/python vckss/benchmarks/validate_scc.py \
...
`/projectnb/welfgr/vckss/bundles/<sha256>/`. Every job in a run verifies that
...
```bash
./.venv/bin/python vckss/benchmarks/build_prod_bundle.py \
  --root "$PWD" \
  --allowlist vckss/benchmarks/prod_bundle_allowlist.txt \
  --source-commit "$(git rev-parse HEAD)" \
  --check-only
./.venv/bin/python vckss/benchmarks/validate_prod_scc.py --static
bash -n vckss/benchmarks/scc/*.sh vckss/benchmarks/scc/*.sge
```
...
```bash
cp vckss/benchmarks/prod_data_manifest.example.tsv \
...
rsync -av /private/tmp/kss-prod-data-manifest.tsv \
  scc:/projectnb/welfgr/vckss/manifests/<run-id>.tsv
bash vckss/benchmarks/scc/deploy_prod_bundle.sh "$PWD" <run-id>

ssh scc bash /projectnb/welfgr/vckss/bundles/<bundle-sha>/source/vckss/benchmarks/scc/submit_prod_dag.sh \
  /projectnb/welfgr/vckss/runs/<run-id> \
  /projectnb/welfgr/vckss/bundles/<bundle-sha> <bundle-sha> \
  /projectnb/welfgr/vckss/manifests/<run-id>.tsv preflight
```
...
python3 <bundle>/source/vckss/benchmarks/validate_prod_scc.py \
  --run-dir /projectnb/welfgr/vckss/runs/<run-id> \
  --bundle-sha <bundle-sha256> --source-commit <commit> \
  --data-manifest /projectnb/welfgr/vckss/runs/<run-id>/input/data_manifest.tsv \
...
```bash
bash <bundle>/source/vckss/benchmarks/scc/submit_matlab_phase_profile.sh \
...
bash <bundle>/source/vckss/benchmarks/scc/collect_matlab_phase_profile.sh \
  <run-dir> <label>

python3 <bundle>/source/vckss/benchmarks/validate_matlab_phase_profile.py \
