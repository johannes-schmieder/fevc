...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="vckss-fe-buf1-") as tmp:
        temporary = Path(tmp)
        roots = {role: temporary / role for role in commits}
        for role, commit in commits.items():
            roots[role].mkdir()
            _archive(repo, commit, roots[role])
        driver = roots["candidate"] / "vckss/benchmarks/fe_buf1/local_driver.do"
        baseline_driver = roots["baseline"] / "vckss/benchmarks/fe_buf1/local_driver.do"
