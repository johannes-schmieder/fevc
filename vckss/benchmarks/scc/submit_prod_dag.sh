...
phase=$5
shift 5
case "$run_dir" in /projectnb/welfgr/vckss/runs/*) ;; *) usage ;; esac
case "$bundle_dir" in /projectnb/welfgr/vckss/bundles/*) ;; *) usage ;; esac
...
plan="$source_dir/vckss/benchmarks/prod_experiments.tsv"
runner="$source_dir/vckss/benchmarks/scc/run_prod_stage.sge"
test -s "$plan"
test -s "$runner"
...
python3 "$source_dir/vckss/benchmarks/validate_prod_scc.py" \
  --plan "$plan" --static
...
if [[ "$phase" == stress ]]; then
  ...
  python3 "$source_dir/vckss/benchmarks/scc/validate_stress_projection.py" \
