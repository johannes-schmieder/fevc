...
if inlist("`fixture'", "replicated_blocks", "ring") & `scale_factor' >= 2 {
    ...
    quietly do "`c(pwd)'/vckss/benchmarks/kss_scale_fixtures.mata"
    quietly do "`c(pwd)'/vckss/benchmarks/kss_scale_fixtures.do"
    ...
    // remove the fixture runtime now so vckss can load its registered
...
capture noisily vckss y_minus_xb `weight_spec', `command_options'
...
generate str64 route_api = "`route_api'"
