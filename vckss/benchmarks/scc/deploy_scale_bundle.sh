...
allowlist="$source_root/vckss/benchmarks/scale_bundle_allowlist.txt"
builder="$source_root/vckss/benchmarks/build_scale_bundle.py"
deployer="$source_root/vckss/benchmarks/scc/deploy_scale_bundle.sh"
scale_shell_sources=(
  "$deployer"
  "$source_root/vckss/benchmarks/scc/run_kss_scale.sge"
  "$source_root/vckss/benchmarks/scc/submit_kss_scale.sh"
)
test -f "$allowlist"
test -f "$builder"
...
[[ "$upload_dir" == /projectnb/welfgr/vckss/uploads/streamline-* ]]
[[ "$bundle_dir" == /projectnb/welfgr/vckss/bundles/$bundle_sha ]]
[[ "$run_dir" == /projectnb/welfgr/vckss/runs/$run_id ]]
[[ "$bundle_sha" =~ ^[0-9a-f]{64}$ ]]
[[ "$source_commit" =~ ^[0-9a-f]{40}$ ]]
[[ "$run_id" =~ ^[0-9]{8}T[0-9]{6}Z-[A-Za-z0-9._-]+$ ]]
...
verify_bundle() {
  ...
  test "$(tr -d '[:space:]' < "$source/SOURCE_COMMIT.txt")" = \
    "$source_commit"
  test -x "$source/vckss/benchmarks/scc/deploy_scale_bundle.sh"
}
