...
if inlist("`stage'", "install_auto", "install_cmg") {
    ...
    net install vckss, from(`"`c(pwd)'/vckss"') replace
    discard
    mata: mata clear
    capture findfile vckss.ado
...
if "`temperature'" == "warm" {
    capture noisily vckss y_minus_xb, worker(worker) firm(firm) `command_options'
    if _rc {
...
forvalues repetition = 1/`repetitions' {
    // Mata's profiling service owns Stata's timer registry while vckss is
