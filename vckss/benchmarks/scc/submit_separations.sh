...
case "$job" in
  matlab-sample)
    ...
    detail_sha=$5
    wage_sha=$6
    case "$matlab_run" in /projectnb/welfgr/vckss/runs/*) ;; *) exit 198 ;; esac
    ...
    script="$source_dir/vckss/benchmarks/scc/run_separations_matlab_sample.sge"
    runtime=00:30:00
  ...
  prepare)
    ...
    script="$source_dir/vckss/benchmarks/scc/run_separations_prepare.sge"
    runtime=00:30:00
    memory=8G
    ;;
  exact|b1|cmg)
    ...
    script="$source_dir/vckss/benchmarks/scc/run_separations_estimator.sge"
    runtime=01:45:00
    memory=16G
    ;;
  matlab)
    ...
    script="$source_dir/vckss/benchmarks/scc/run_separations_matlab.sge"
    runtime=01:45:00
    memory=16G
    ;;
  compare)
    (( $# == 0 )) || exit 198
    environment="$common"
    script="$source_dir/vckss/benchmarks/scc/run_separations_compare.sge"
    runtime=00:20:00
    memory=4G
    ;;
  *) exit 198 ;;
