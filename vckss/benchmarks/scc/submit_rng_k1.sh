...
[[ "$run_dir" =~ ^/projectnb/welfgr/vckss/runs/[0-9]{8}T[0-9]{6}Z-[A-Za-z0-9._-]+$ ]] || {
...
test -f "$source_dir/vckss/benchmarks/scc/rng_k1_driver.do"
test -f "$source_dir/vckss/benchmarks/scc/run_rng_k1.sge"
test -f "$source_dir/vckss/benchmarks/scc/validate_rng_k1.py"
...
raw_job_id=$(qsub -terse -P welfgr -pe omp 14 \
  ...
  "$source_dir/vckss/benchmarks/scc/run_rng_k1.sge")
[[ "$raw_job_id" =~ ^[0-9]+$ ]] || {
