...
It compares one source-bound `vckss` candidate with the checksum-bound
...
differ from `vckss`. Therefore corrected estimates have no equality
...
oracle in `../oracle/vckss_dense_oracle.m` and Stata exact driver in
...
./.venv/bin/python -m pytest -q \
  vckss/benchmarks/prep_bnd1_matlab/tests
./.venv/bin/python -m ruff check \
  vckss/benchmarks/prep_bnd1_matlab
```
...
/Applications/MATLAB_R2024b.app/bin/matlab -batch \
  "addpath('vckss/benchmarks/oracle'); vckss_dense_oracle('$oracle_dir','$candidate')"
/Applications/Stata/StataMP.app/Contents/MacOS/stata-mp -q do \
  vckss/benchmarks/oracle/stata_oracle.do "$oracle_dir" "$candidate"
./.venv/bin/python \
...
bundle=$(./.venv/bin/python \
  vckss/benchmarks/prep_bnd1_matlab/build_bundle.py \
...
./.venv/bin/python \
  vckss/benchmarks/prep_bnd1_matlab/build_manifest.py \
...
```text
/projectnb/welfgr/vckss/bundles/<bundle>.tar.gz
/projectnb/welfgr/vckss/bundles/<bundle>.files.sha256
/projectnb/welfgr/vckss/bundles/<bundle>/source/
/projectnb/welfgr/vckss/prep-bnd1-matlab/runs/<run-id>/input/tasks/
```
...
PBM_SOURCE_DIR=/projectnb/welfgr/vckss/bundles/<bundle>/source
PBM_SOURCE_COMMIT=<candidate-full-sha>
PBM_BUNDLE_SHA256=<bundle>
PBM_BUNDLE_MANIFEST=/projectnb/welfgr/vckss/bundles/<bundle>.files.sha256
...
```bash
./.venv/bin/python \
  vckss/benchmarks/prep_bnd1_matlab/validate_scc_job.py \
...
```bash
./.venv/bin/python vckss/benchmarks/prep_bnd1_matlab/analyze.py \
...
pbm_root=/projectnb/welfgr/vckss/prep-bnd1-matlab
source_dir=/projectnb/welfgr/vckss/bundles/$bundle/source
bundle_manifest=/projectnb/welfgr/vckss/bundles/$bundle.files.sha256
run_root=$pbm_root/runs/$run_id

test -d "$source_dir"
test "$(tr -d '[:space:]' < "$source_dir/SOURCE_COMMIT.txt")" = "$candidate"
test "$(sha256sum "/projectnb/welfgr/vckss/bundles/$bundle.tar.gz" | \
  awk '{print $1}')" = "$bundle"
...
job_id=$(qsub -terse -P welfgr -pe omp 4 \
  ...
  "$source_dir/vckss/benchmarks/prep_bnd1_matlab/run_dense_oracle.sge")
...
python3 \
  "$source_dir/vckss/benchmarks/prep_bnd1_matlab/validate_dense_oracle_scc.py" \
