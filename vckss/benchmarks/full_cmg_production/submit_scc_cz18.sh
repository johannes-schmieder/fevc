...
temporary=$(mktemp -d /private/tmp/vckss-full-cmg-production-scc.XXXXXX)
...
job_id=$(ssh scc qsub -terse -P welfgr -pe omp 14 \
  ...
  -N vckss-pcz -j y -o "$remote/logs/job.txt" -m a \
  ...
  "$remote/source/vckss/benchmarks/full_cmg_production/run_scc_cz18.sge")
