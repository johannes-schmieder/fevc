...
harness=$source_dir/vckss/benchmarks/projection_akm_scaling
...
scratch=$TMPDIR/vckss-projection-akm-$JOB_ID-${SGE_TASK_ID:-gate}-$rows-$cores-$replicate
...
grep -Fq "VCKSS PROJECTION AKM STATA BASELINE PASS cores=$cores processors=$stata_processors" "$output/stata_baseline.application.txt"
...
printf 'VCKSS PROJECTION AKM PAIR CAPTURED rows=%s cores=%s replicate=%s order=%s\n' \
