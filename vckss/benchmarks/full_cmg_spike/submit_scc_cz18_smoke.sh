#!/usr/bin/env bash
# Deploy and submit one exact-source fixed-CZ18 P20 A/C/MATLAB smoke.

set -euo pipefail

run_id=${1:?usage: submit_scc_cz18_smoke.sh RUN_ID}
[[ "$run_id" =~ ^[A-Za-z0-9._-]+$ ]]
...
job_id=$(ssh scc qsub -terse -P welfgr -pe omp 14 \
  ...
  -N vckss-cz20 -j y -o "$remote/logs/job.txt" -m a \
  -v "FCMG_CZ_RUN_DIR=$remote,FCMG_CZ_SOURCE_COMMIT=$source_commit,FCMG_CZ_BASELINE_COMMIT=$baseline_commit,FCMG_CZ_CMG_COMMIT=$cmg_commit,FCMG_CZ_INPUT_DTA=$input_dta,FCMG_CZ_INPUT_SHA256=$input_sha,FCMG_CZ_TASK_SHA256=$task_sha,FCMG_CZ_MATLAB_ROOT=$matlab_root" \
...
printf 'VCKSS_FULL_CMG_CZ18_SCC_SUBMITTED job_id=%s run_dir=%s source_commit=%s task_sha256=%s\n' \
