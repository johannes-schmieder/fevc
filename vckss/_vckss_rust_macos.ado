if strpos("`c(machine_type)'", "Apple Silicon") {
    capture program _vckss_rust_macos, plugin using("vckss_rust_macos_arm64.plugin")
}
else {
    capture program _vckss_rust_macos, plugin using("vckss_rust_macos_x86_64.plugin")
}
