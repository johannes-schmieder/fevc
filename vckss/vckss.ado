...
program define _vckss_impl, eclass sortpreserve
    ...
    if `rust_public' & "`algorithm'" == "exact" {
        ...
        capture quietly vckss_rust requestcapability,      ///
    ...
    if _rc {
        if `mata_runtime_loaded' {
            ...
            di as error "a different vckss Mata runtime is already loaded; restart Stata or run discard before retrying"
            exit 498
        }
        ...
        if _rc {
            ...
            di as error "the loaded vckss Mata runtime does not match this command build"
            exit 498
        }
    }
...
program define _vckss_rust_abort, eclass
    ...
    capture quietly vckss_rust lasterror
    if _rc {
        local native_code = .
...
program define _vckss_rust_finally
    ...
    if `active_handle' > 0 {
        capture quietly vckss_rust release `active_handle'
    }
    capture quietly vckss_rust clear
    capture quietly vckss_rust snapshot
    if _rc exit _rc
...
program define _vckss_failure_guidance, rclass
    ...
    if inlist("`failure_status'", "INVALID_DEPVAR",              ///
        ...
        local suggestion "Run discard or restart Stata, then reinstall one complete vckss build and confirm that all package files resolve from the same adopath location."
    }
...
program define _vckss_display_failure
    version 18.0
    di as error _newline "vckss could not compute the requested decomposition."
    ...
    di as text "See "                                           ///
        `"{help vckss##troubleshooting:help vckss, troubleshooting}."'
end
