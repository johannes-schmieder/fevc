...
REQUIRED_IDENTICAL = (ROOT / "LICENSE", ROOT / "vckss" / "LICENSE")
REQUIRED_TEXT = {
    ...
    ROOT / "vckss" / "THIRD_PARTY_NOTICES.txt": (
        "GPL-3.0-only",
        "rust/vendor/cmg/",
    ),
...
def main() -> int:
    ...
    package = (ROOT / "vckss" / "vckss.pkg").read_text(encoding="utf-8")
    for entry in ("f LICENSE", "f THIRD_PARTY_NOTICES.txt"):
        if entry not in package.splitlines():
            errors.append(f"vckss.pkg: missing package entry {entry!r}")
