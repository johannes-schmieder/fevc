#!/usr/bin/env python3
"""Qualify the varcomp_kss 0.3.0-dev -> vckss 0.4.0-dev hard-cut rename.
...
BASELINE_COMMIT = "7fcf1b20105a546e993e0b3186c842f05f1f79a8"
RECEIPT_SCHEMA = "vckss-rename-equivalence-qualification-v2"
EVIDENCE_DIR = Path("vckss/qualification/vckss_rename_equivalence")
RECEIPT_PATH = EVIDENCE_DIR / "receipt.json"
ORCHESTRATOR_PATH = Path("vckss/tools/run_vckss_rename_equivalence.py")
DRIVER_PATH = Path("vckss/tests/equivalence/vckss_equivalence_driver.do")
...
CMG_API_TRANSITIONS = {
    "cmg_api_level": ("+1.c000000000000X+002", "+1.0000000000000X+003"),
    "cmg_design_label": (
...
INVALID_MEMORY_HELP_TRANSITION = {
    "baseline": (
        "Check the option spelling and documented range in help varcomp_kss; "
...
def _stage_source_bound_plugin(
    ...
    manifest = evidence / "source-manifest.sha256"
    command = "varcomp_kss" if role == "baseline" else "vckss"
    plugin_name = f"{command}_rust_macos_arm64.plugin"
    plugin = evidence / "candidates" / plugin_name
    for path, label in ((manifest, "source manifest"), (plugin, "arm64 plugin")):
        if path.is_symlink() or not path.is_file():
            raise QualificationError(f"{role} plugin {label} is unavailable: {path}")

    manifest_bytes = manifest.read_bytes()
    try:
        lines = manifest_bytes.decode("utf-8").splitlines()
    except UnicodeDecodeError as exc:
        raise QualificationError(f"{role} plugin source manifest is not UTF-8") from exc
    observed: set[str] = set()
    for line_number, line in enumerate(lines, start=1):
        match = re.fullmatch(r"([0-9a-f]{64})  (.+)", line)
        if match is None:
            raise QualificationError(
                f"malformed {role} plugin source manifest line {line_number}"
            )
        ...
        if pure.is_absolute() or not pure.parts or any(
            ...
            raise QualificationError(f"unsafe {role} plugin source path: {relative!r}")
        if relative in observed:
            raise QualificationError(f"duplicate {role} plugin source path: {relative}")
        observed.add(relative)
        source = archive_root.joinpath(*pure.parts)
        if source.is_symlink() or not source.is_file() or _sha256_file(source) != digest:
            raise QualificationError(
                f"{role} plugin source binding failed for archived {relative}"
            )
    required = {
        "rust/Cargo.lock",
        ...
        f"{command}/{command}.ado",
        f"{command}/{command}_rust.ado",
    }
    if not required.issubset(observed):
        raise QualificationError(
            f"{role} plugin source manifest omitted: {sorted(required - observed)}"
        )
...
def _preflight_committed_candidate(repo: Path, commit: str) -> None:
    required = [
        *(f"vckss/{name}" for name in _runtime_filenames("vckss")),
        "vckss/vckss.sthlp",
        "vckss/vckss.pkg",
        "vckss/stata.toc",
        "vckss/cmg/src/cmg_core.mata.in",
        "vckss/cmg/tools/assemble.py",
        "vckss/cmg/generated/manifest.json",
        "CODE_LICENSE.md",
        "pyproject.toml",
...
def _validate_role_records(role: str, records: Sequence[Record]) -> None:
    ...
    expected_command = "varcomp_kss" if role == "baseline" else "vckss"
    expected_prefix = "vckss" if role == "baseline" else "vckss"
    expected_version = "0.3.0-dev" if role == "baseline" else "0.4.0-dev"
...
def _write_role_artifacts(
    ...
    payload = {
        "schema": "vckss-rename-equivalence-raw-v2",
        "role": role,
        "revision": revision,
...
def _validate_artifact_json(
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "vckss-rename-equivalence-raw-v2":
    ...
    *,
    ...
    for role in ("baseline", "candidate"):
        ...
        if (
            ...
            or item.get("plugin_filename")
            != f"{command}_rust_macos_arm64.plugin"
...
def _self_test() -> None:
    ...
    def role_fixture(role: str) -> list[Record]:
        command = "varcomp_kss" if role == "baseline" else "vckss"
        prefix = "vckss" if role == "baseline" else "vckss"
        version = "0.3.0-dev" if role == "baseline" else "0.4.0-dev"
    ...
    )] = Record("exact_match_joint", "metadata", "version", 0, 0, "0.3.0-dev")
    try:
    ...
    baseline = [
        ...
        Record("exact_match_joint", "metadata", "cmdline", 0, 0, "varcomp_kss y, nodisplay"),
    ...
    candidate = [
        ...
        Record(
            "exact_match_joint", "metadata", "cmdline", 0, 0, "vckss y, nodisplay"
        ),
    ...
    payload = {
        ...
        "rust_artifacts": {
            role: {
                "architecture": "arm64",
                "plugin_filename": (
                    "varcomp_kss_rust_macos_arm64.plugin"
                    if role == "baseline"
                    else "vckss_rust_macos_arm64.plugin"
                ),
                "plugin_sha256": "8" * 64,
                "source_manifest_path": (
                    EVIDENCE_DIR / f"{role}.plugin-source-manifest.sha256"
    ...
    print("PASS run_vckss_rename_equivalence.py --self-test")
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--baseline-plugin-evidence", type=Path)
    parser.add_argument("--candidate-plugin-evidence", type=Path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        or args.baseline_plugin_evidence is None
        or args.candidate_plugin_evidence is None
    ):
        raise QualificationError(
            "--candidate, --output-dir, --baseline-plugin-evidence, and "
            "--candidate-plugin-evidence are required for qualification"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="vckss-rename-eq-v2-") as temporary:
        ...
        baseline_package = baseline_archive / "varcomp_kss"
        candidate_package = candidate_archive / "vckss"
        ...
        _preflight_package(baseline_package, "varcomp_kss")
        _preflight_package(candidate_package, "vckss")
        rust_artifacts: dict[str, dict[str, object]] = {}
        rust_manifest_bytes: dict[str, bytes] = {}
        for role, evidence_dir, archive_root, package_root in (
            (
                "baseline",
                args.baseline_plugin_evidence,
                baseline_archive,
                baseline_package,
            ),
            (
                "candidate",
                args.candidate_plugin_evidence,
                candidate_archive,
                candidate_package,
            ),
        ):
            rust_artifacts[role], rust_manifest_bytes[role] = (
                _stage_source_bound_plugin(
                    evidence_dir, archive_root, package_root, role
                )
            )
        harness = {
            "orchestrator": {
        ...
        roles = {
            "baseline": (baseline_package, "varcomp_kss", "vckss"),
            "candidate": (candidate_package, "vckss", "vckss"),
        }
