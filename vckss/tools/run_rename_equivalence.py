#!/usr/bin/env python3
"""Qualify the kss_bc -> varcomp_kss namespace-only rename.
...
BASELINE_COMMIT = "59aeb133470c0a27a5f543ca16fb8cdad5cc1a89"
RECEIPT_SCHEMA = "varcomp-kss-rename-equivalence-qualification-v1"
EVIDENCE_DIR = Path("varcomp_kss/qualification/rename_equivalence")
RECEIPT_PATH = EVIDENCE_DIR / "receipt.json"
ORCHESTRATOR_PATH = Path("varcomp_kss/tools/run_rename_equivalence.py")
DRIVER_PATH = Path("varcomp_kss/tests/equivalence/equivalence_driver.do")
...
CMG_API_TRANSITIONS = {
    "cmg_api_level": ("+1.8000000000000X+002", "+1.c000000000000X+002"),
    "cmg_design_label": (
...
def _runtime_filenames(command: str) -> tuple[str, ...]:
    return (
        f"{command}.ado",
        f"{command}_lifecycle.ado",
        f"{command}.mata",
        f"{command}_rng.mata",
        f"{command}_graph.mata",
        f"{command}_solver.mata",
        f"{command}_resource.mata",
        f"{command}_cmg.mata",
        f"{command}_scale.mata",
        f"{command}_scale_engine.mata",
        f"{command}_scale_runtime.mata",
    )
...
def _preflight_committed_candidate(repo: Path, commit: str) -> None:
    required = [
        *(f"varcomp_kss/{name}" for name in _runtime_filenames("varcomp_kss")),
        "varcomp_kss/varcomp_kss.sthlp",
        "varcomp_kss/varcomp_kss.pkg",
        "varcomp_kss/stata.toc",
        "varcomp_kss/cmg/src/cmg_core.mata.in",
        "varcomp_kss/cmg/tools/assemble.py",
        "varcomp_kss/cmg/generated/manifest.json",
        "CODE_LICENSE.md",
        "pyproject.toml",
...
def _find_stata(explicit: str | None) -> tuple[Path, str]:
    ...
    environment = os.environ.get("VARCOMP_KSS_STATA")
    if environment is not None:
        resolved = _resolve_executable(environment)
        if resolved is None:
            raise QualificationError(
                f"VARCOMP_KSS_STATA is not executable: {environment}"
            )
    ...
    raise QualificationError("Stata was not found; pass --stata or set VARCOMP_KSS_STATA")
...
def _run_stata_case(
    *,
    ...
    marker = f"VARCOMP_KSS_RENAME_EQ_PASS::{nonce}::{role}::{case_id}"
...
def _validate_role_records(role: str, records: Sequence[Record]) -> None:
    ...
    expected_command = "kss_bc" if role == "baseline" else "varcomp_kss"
    expected_prefix = "kssbc" if role == "baseline" else "vckss"
    expected_version = "0.2.0-dev" if role == "baseline" else "0.3.0-dev"
    ...
    for case_id in REQUIRED_CASE_IDS:
        ...
        if case_id in EXPECTED_FAILURES:
            ...
            cmdline = _record_value(records, case_id, "metadata", "cmdline")
            token, separator, _ = cmdline.partition(" ")
            if token != expected_command or not separator:
                raise QualificationError(f"wrong observed e(cmdline) token for {role}/{case_id}")
...
def _validate_timing_record(record: Record) -> None:
    ...
    if record.value == "." or record.value.lower() == _STATA_SYSTEM_MISSING_HEX.lower():
        if not scalar_timing or record.name != "rng_seconds":
...
def _compare_records(
    ...
    for case_id in REQUIRED_CASE_IDS:
        ...
        results[case_id] = {
            ...
            "timing_invariant": (
                "same shape; values are canonical, nonnegative, and finite except "
                "registered rng_seconds not-applicable missing; raw values retained"
            ),
...
def _write_role_artifacts(
    ...
    payload = {
        "schema": "varcomp-kss-rename-equivalence-raw-v1",
        "role": role,
        "revision": revision,
...
def _validate_artifact_json(
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "varcomp-kss-rename-equivalence-raw-v1":
...
def _comparison_descriptor() -> dict[str, object]:
    return {
        ...
        "timing_invariant": (
            "same return shape; canonical nonnegative values, with only "
            "rng_seconds allowed to be system missing when not applicable"
        ),
...
def _publish_evidence_bundle(
    repo: Path, raw_bytes: dict[str, dict[str, bytes]], receipt: dict[str, object]
) -> Path:
...
def _self_test() -> None:
    ...
    def role_fixture(role: str) -> list[Record]:
        command = "kss_bc" if role == "baseline" else "varcomp_kss"
        prefix = "kssbc" if role == "baseline" else "vckss"
        version = "0.2.0-dev" if role == "baseline" else "0.3.0-dev"
    ...
    )] = Record("exact_match_joint", "metadata", "version", 0, 0, "0.2.0-dev")
    try:
    ...
    baseline = [
        Record("exact_match_joint", "metadata", "command", 0, 0, "kss_bc"),
        Record("exact_match_joint", "metadata", "cmdline", 0, 0, "kss_bc y, nodisplay"),
        Record("exact_match_joint", "timing", "fit_seconds", 0, 0, one),
        Record("exact_match_joint", "matrix", "kss", 1, 1, one),
    ]
    candidate = [
        Record("exact_match_joint", "metadata", "command", 0, 0, "varcomp_kss"),
        Record(
            "exact_match_joint", "metadata", "cmdline", 0, 0, "varcomp_kss y, nodisplay"
        ),
    ...
    bad_cmdline[1] = Record(
        "exact_match_joint", "metadata", "cmdline", 0, 0, "varcomp_kss z, nodisplay"
    )
    try:
    ...
    print("PASS run_rename_equivalence.py --self-test")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_receipt is not None:
        ...
        return 0
    if not args.candidate or args.output_dir is None:
        raise QualificationError("--candidate and --output-dir are required for qualification")
    ...
    with tempfile.TemporaryDirectory(prefix="varcomp-kss-rename-eq-") as temporary:
        run_root = Path(temporary)
        baseline_archive = run_root / "archive-baseline"
        candidate_archive = run_root / "archive-candidate"
        _archive_revision(repo, BASELINE_COMMIT, "kss_bc", baseline_archive)
        _archive_revision(repo, candidate_commit, "varcomp_kss", candidate_archive)
        baseline_package = baseline_archive / "kss_bc"
        candidate_package = candidate_archive / "varcomp_kss"
        ...
        _preflight_package(baseline_package, "kss_bc")
        _preflight_package(candidate_package, "varcomp_kss")
        harness = {
            "orchestrator": {
        ...
        roles = {
            "baseline": (baseline_package, "kss_bc", "kssbc"),
            "candidate": (candidate_package, "varcomp_kss", "vckss"),
        }
        ...
        if args.write_receipt:
            ...
            receipt_path = _publish_evidence_bundle(repo, raw_bytes, receipt)
