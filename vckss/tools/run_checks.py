#!/usr/bin/env python3
"""Run the integrated local qualification gates for ``vckss``."""
...
def stata_executable() -> str | None:
    configured = os.environ.get("VCKSS_STATA")
    if configured:
        return configured
...
def main() -> int:
    ...
    if stata is None:
        if os.environ.get("VCKSS_REQUIRE_STATA") == "1":
            raise RuntimeError(
                "VCKSS_REQUIRE_STATA=1 but no Stata/MP executable was found."
            )
        print(
            "\n== KSS Stata gates ==\n"
            "SKIP: no Stata/MP launcher found; set VCKSS_STATA or "
            "VCKSS_REQUIRE_STATA=1.",
            flush=True,
        )
        return 0
    ...
    run(
        ...
        [str(PYTHON), "vckss/cmg/tools/run_checks.py"],
        env=component_environment,
    )

    suite = ROOT / "vckss/tests/stata/run_all.do"
    run_stata(
        "KSS Stata quick suite",
        stata,
        suite,
        ["quick"],
        "VCKSS TEST SUITE PASS: quick",
    )
    run_stata(
    ...
    with tempfile.TemporaryDirectory(prefix="vckss-install-") as temporary:
        install_root = Path(temporary) / "plus"
        install_root.mkdir()
        run_stata(
            "KSS clean-install smoke test",
            stata,
            ROOT / "vckss/tests/stata/test_install.do",
            [str(ROOT / "vckss"), str(install_root)],
            "VCKSS INSTALL TEST PASS",
            Path(temporary),
        )
    with tempfile.TemporaryDirectory(prefix="vckss-benchmark-") as temporary:
        output = Path(temporary)
        run_stata(
            "KSS synthetic benchmark harness smoke test",
            stata,
            ROOT / "vckss/benchmarks/synthetic_benchmark.do",
            [
                "local-gate",
                "smoke",
                "100",
            ...
            "VCKSS BENCHMARK PASS: smoke",
        )
        validate_benchmark(output)
    with tempfile.TemporaryDirectory(prefix="vckss-numopt-") as temporary:
        output = Path(temporary)
        commit = "0" * 40
        for route in ("b1", "cmg"):
            ...
            run_stata(
                ...
                stata,
                ROOT / "vckss/benchmarks/estimator_cmg_benchmark.do",
                [
                    "local-gate",
                    route,
                    "moderate",
                ...
                f"VCKSS NUMOPT BENCHMARK PASS: {route} moderate",
            )
        run(
            "KSS paired B1/forced-CMG benchmark validation",
            [
                str(PYTHON),
                "vckss/benchmarks/validate_numopt.py",
                "--run-dir",
                str(output),
                "--expected-commit",
                commit,
    ...
    with tempfile.TemporaryDirectory(prefix="vckss-separations-") as temporary:
        output = Path(temporary)
        run_stata(
            "KSS Separations preparation duplicate-key smoke test",
            stata,
            ROOT / "vckss/tests/stata/test_separations_prepare.do",
            [str(ROOT / "vckss"), str(output)],
            "VCKSS SEPARATIONS PREPARE PASS: fixture",
        )
        validate_separations_preparation(output)
    with tempfile.TemporaryDirectory(
        prefix="vckss-matlab-sample-"
    ) as temporary:
        run_stata(
            "KSS MATLAB-retained sample and bridge-audit smoke test",
            stata,
            ROOT / "vckss/tests/stata/test_separations_matlab_sample.do",
            [str(ROOT / "vckss"), temporary],
            "PASS test_separations_matlab_sample.do",
        )
    print("\nVCKSS LOCAL QUALIFICATION PASS", flush=True)
    return 0
