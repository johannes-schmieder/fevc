...
from vckss.benchmarks.scc.select_prod_calibration import (
...
from vckss.benchmarks.scc.validate_stress_projection import (
    main as stress_projection_main,
)
from vckss.benchmarks.scc.validate_stress_projection import (
    stress_projection_from_calibrations,
)
from vckss.benchmarks.validate_prod_scc import (
    expected_timeout,
    identity,
    load_plan,
...
def test_selector_and_stress_admission_pin_supported_scc_python() -> None:
    ...
    assert 'python3 "$source_dir/vckss/benchmarks/scc/' in runner
...
def test_submitter_rejects_comma_before_composing_qsub_environment() -> None:
    ...
    result = subprocess.run(
        ["bash", str(submitter),
         "/projectnb/welfgr/vckss/runs/bad,run",
         "/projectnb/welfgr/vckss/bundles/test", "a" * 64,
