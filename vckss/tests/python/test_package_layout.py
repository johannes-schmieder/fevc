...
def test_package_manifest_is_complete() -> None:
    manifest = (ROOT / "vckss.pkg").read_text(encoding="utf-8").splitlines()
    shipped = {
...
def test_version_identifiers_agree() -> None:
    ...
    assert VERSION in (ROOT / "vckss.sthlp").read_text(encoding="utf-8")
...
def test_help_examples_are_installed_and_uniquely_marked() -> None:
    ...
    assert "program define vckss_run" in runner
...
def test_package_records_internal_license_boundary() -> None:
    manifest = (ROOT / "vckss.pkg").read_text(encoding="utf-8").lower()
    readme = " ".join(
...
def test_public_command_is_a_hard_cut_without_predecessor_alias() -> None:
    ado = (ROOT / "vckss.ado").read_text(encoding="utf-8")
    assert "program define vckss, eclass" in ado
    assert 'ereturn local cmd "vckss"' in ado
    assert 'ereturn local cmdline `"vckss `0\'"\'' in ado
...
def test_only_factor_metadata_can_remove_an_omitted_control() -> None:
    runtime = (ROOT / "vckss.ado").read_text(encoding="utf-8")
...
def test_cmg_route_is_installed_behind_package_solver_contract() -> None:
    ado = (ROOT / "vckss.ado").read_text(encoding="utf-8").lower()
    mata = (ROOT / "vckss.mata").read_text(encoding="utf-8")
    solver = (ROOT / "vckss_solver.mata").read_text(encoding="utf-8")
    ...
    manifest = (ROOT / "vckss.pkg").read_text(encoding="utf-8")
    assert "preconditioner(string)" in ado
    assert "vckss/cmg" not in manifest
...
def test_separations_harness_is_read_only_and_aggregate_collectable() -> None:
    ...
    assert "VCKSS SEPARATIONS MATLAB-ONLY EVIDENCE PASS" in validator
    ...
    assert 'case "$KSS_MATLAB_RUN" in /projectnb/welfgr/vckss/runs/*)' in sample_wrapper
