...
VALIDATOR_PATH = ROOT / "vckss" / "benchmarks" / "validate_matlab_phase_profile.py"
...
def write_aggregate(output_dir: Path, record: dict[str, object]) -> None:
    ...
    (output_dir / "wrapper.pass").write_text(
        f"VCKSS_MATLAB_PHASE_PROFILE_PASS {LABEL} {BUNDLE} {SOURCE} "
...
def evidence(tmp_path: Path) -> tuple[Path, dict[str, object]]:
    ...
    (job_dir / "application.txt").write_text(
        f"VCKSS MATLAB PHASE PROFILE PASS: {LABEL}\n", encoding="utf-8"
    )
...
def test_matlab_profiler_has_bounded_top_level_real_time_contract() -> None:
    source = (ROOT / "vckss/benchmarks/separations_matlab_phase_profile.m").read_text(
        encoding="utf-8"
    )
...
def test_scc_wrapper_binds_and_rechecks_immutable_inputs() -> None:
    source = (ROOT / "vckss/benchmarks/scc/run_matlab_phase_profile.sge").read_text(
        encoding="utf-8"
    )
    ...
    submit = (ROOT / "vckss/benchmarks/scc/submit_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
    collect = (ROOT / "vckss/benchmarks/scc/collect_matlab_phase_profile.sh").read_text(
        encoding="utf-8"
    )
...
def test_new_shell_entrypoints_pass_bash_syntax() -> None:
    scripts = [
        ROOT / "vckss/benchmarks/scc/run_matlab_phase_profile.sge",
        ROOT / "vckss/benchmarks/scc/submit_matlab_phase_profile.sh",
        ROOT / "vckss/benchmarks/scc/collect_matlab_phase_profile.sh",
    ]
