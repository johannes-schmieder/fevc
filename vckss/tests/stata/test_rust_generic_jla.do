...
foreach deletion in match observation {
    ...
    foreach nuisance in joint fixedoffset {
        ...
        forvalues q = 0/1 {
            ...
            quietly vckss_rust requestcapability, algorithm(jla) ///
            ...
            quietly vckss_rust prepare worker firm deletion_id outcome ///
            ...
            quietly vckss_rust solve `handle', algorithm(jla)          ///
                deletion(`deletion') nuisance(`nuisance') route(diagonal)    ///
...
local handle = r(handle)
capture noisily vckss_rust solve `handle', algorithm(jla) deletion(match) ///
...
quietly vckss_rust snapshot
assert r(state) == 4
capture noisily vckss_rust result `handle'
assert _rc == 198
quietly vckss_rust release `handle'
quietly vckss_rust release `handle'
quietly vckss_rust snapshot
...
foreach corruption in bad_count bad_schema bad_phase bad_probe bad_side      ///
    ...
    quietly vckss_rust prepare worker firm deletion_id outcome frequency ///
        target_weight, cleanup memorygib(1) deletion(match)
    local handle = r(handle)
    quietly vckss_rust solve `handle', algorithm(jla) deletion(match) ///
...
foreach corruption in bad_schema0_rows bad_v1_copy bad_v2_copy {
    ...
    quietly vckss_rust prepare worker firm deletion_id outcome frequency ///
        target_weight, cleanup memorygib(1) deletion(match)
    local handle = r(handle)
    quietly vckss_rust solve `handle', algorithm(exact) deletion(match) ///
...
di as result "VCKSS RUST GENERIC JLA PASS"
exit 0
