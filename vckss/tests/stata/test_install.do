...
net install vckss, from(`"`package_source'"') replace
discard
mata: mata clear
...
assert _rc == 0
capture findfile vckss_run.ado
assert _rc == 0
capture findfile vckss.sthlp
assert _rc == 0
...
vckss_run exact_controls using vckss.sthlp
...
di as result "VCKSS INSTALL TEST PASS"
exit 0
