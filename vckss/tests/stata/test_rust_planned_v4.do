...
local probes = 7
capture quietly vckss_rust solve `handle', algorithm(jla) deletion(match) ///
...
quietly vckss_rust solve `cmg_handle', algorithm(jla) deletion(match) ///
...
quietly vckss_rust release `cmg_handle'
quietly vckss_rust snapshot
...
quietly vckss_rust requestcapability, algorithm(auto) deletion(match) ///
    nuisance(joint) route(auto) rngcontract(counter_v1) controls(0)        ///
...
quietly vckss_rust solve `exact_auto_handle', algorithm(auto)         ///
...
quietly vckss_rust release `exact_auto_handle'
quietly vckss_rust snapshot
assert r(state) == 0 & r(handle) == 0

di as result "VCKSS RUST PLANNED V4 PASS"
exit 0
