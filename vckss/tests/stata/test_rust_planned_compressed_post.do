...
capture noisily _vckss_rust_generic_planned outcome worker firm deletion_id ///
    ...
    "vckss outcome [fw=frequency], backend(rust) engine(auto)" ///
    diagonal auto 0 0
assert _rc == 0
...
capture noisily _vckss_rust_generic_planned outcome worker firm deletion_id ///
    ...
    "vckss outcome [fw=frequency], backend(rust) algorithm(auto) engine(auto)" ///
    auto auto 0 0
assert _rc == 0
assert `"`e(algorithm)'"' == "jla"
assert `"`e(engine_requested)'"' == "auto"
...
di as result "VCKSS RUST COMPRESSED PUBLIC ROUTES PASS"
...
quietly vckss_rust solve `xhandle', algorithm(auto) deletion(match) ///
...
quietly vckss_rust result `xhandle'
