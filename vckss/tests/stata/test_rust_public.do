...
quietly vckss y if eligible in 1/48 [fw=frequency],       ///
...
quietly vckss_rust prepare `dense_worker' `dense_firm'    ///
...
quietly vckss_rust solve `handle', seed(91827) probes(6) ///
...
assert r(N) == 0
quietly vckss_rust release `handle'
...
quietly vckss y if eligible in 3/46 [fw=frequency],       ///
...
capture quietly vckss y [fw=frequency],                   ///
    worker(worker) firm(firm) deletion(match) deletionid(match) ///
    targetweight(target) algorithm(jla) engine(compressed)      ///
...
capture quietly vckss y [fw=`huge_frequency'],            ///
    worker(worker) firm(firm) deletion(match) targetweight(target) ///
    algorithm(jla) engine(compressed) preconditioner(diagonal)   ///
    batch(2) probes(4) seed(91827) tolerance(1e-10)              ///
    maxiter(10000) backend(rust) rng(counter_v1) nodisplay
...
capture noisily vckss_rust prepare `dense_worker' `dense_firm' ///
    `dense_match' y `bad_frequency' target, cleanup                ///
    generate(__rust_error_keep) memorygib(4)
assert _rc == 198
quietly vckss_rust lasterror
...
foreach fault in prepare_break solve_break result_break release_break {
    ...
    capture noisily vckss y [fw=frequency], `strict_options'
...
capture quietly vckss y [fw=frequency], `strict_options'
assert _rc == 111
assert `"`e(cmd)'"' == ""
quietly vckss_rust snapshot
assert r(state) == 0 & r(handle) == 0
