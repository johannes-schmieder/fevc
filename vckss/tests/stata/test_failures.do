...
capture noisily vckss y constant, worker(worker) firm(firm) ///
...
foreach deletion_mode in observation match {
    ...
    foreach nuisance_mode in joint fixedoffset {
        capture noisily vckss y explicit_zero, worker(worker) firm(firm) ///
...
capture noisily vckss y negative_zero, worker(worker) firm(firm) ///
...
capture noisily vckss y explicit_zero [fw=frequency_two], ///
...
capture noisily vckss y basis_one transformed_zero, worker(worker) ///
...
capture noisily vckss y [fw=frequency], worker(worker_relabel) ///
...
generate double y = z
capture noisily vckss y z, worker(worker) firm(firm) ///
...
capture noisily vckss y block_only_control, worker(worker) firm(firm) ///
...
capture noisily vckss y z1 z2, worker(worker) firm(firm) ///
    deletion(match) deletionid(match) algorithm(exact) nodisplay
assert _rc == 498
