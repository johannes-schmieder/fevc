...
vckss y c1 c2, worker(worker_s) firm(firm_s) deletion(match) ///
...
vckss y c1 c2, worker(worker_r) firm(firm_r) deletion(match) ///
...
vckss y z1 z2, worker(worker) firm(firm) deletion(match) ///
    deletionid(match) algorithm(jla) probes(400) batch(11) ///
    seed(20261001) tolerance(1e-12) nodisplay
...
vckss y, worker(worker_r) firm(firm_r) deletion(match) ///
...
sort obsid
vckss y i.time, worker(worker) firm(firm) deletion(match) ///
...
vckss y time1 time2 time3, worker(worker) firm(firm) deletion(match) ///
    deletionid(match) algorithm(exact) nodisplay
...
vckss y if obsid != 2, worker(worker) firm(firm) ///
...
vckss y_missing, worker(worker) firm(firm) ///
    deletion(observation) algorithm(exact) nodisplay
