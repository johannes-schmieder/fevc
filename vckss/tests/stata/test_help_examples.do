...
foreach example in exact_controls jla_controls weights_targets {
    vckss_run `example' using vckss.sthlp
...
capture noisily vckss_run missing_example using vckss.sthlp
assert _rc == 111
quietly _datasignature
assert `"`r(datasignature)'"' == `"`caller_signature'"'

capture noisily vckss_run exact_controls using missing_varcomp_help.sthlp
assert _rc == 601
quietly _datasignature
assert `"`r(datasignature)'"' == `"`caller_signature'"'
