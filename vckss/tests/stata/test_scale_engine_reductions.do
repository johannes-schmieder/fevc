...
local oldpwd `"`c(pwd)'"'
capture confirm file "vckss/vckss_scale_engine.mata"
if _rc {
    capture confirm file "../../vckss_scale_engine.mata"
    if _rc {
        di as error "run from the repository root or vckss/tests/stata"
        exit 601
    }
    quietly cd "../.."
    local pkgroot `"`c(pwd)'"'
    ...
    real matrix prediction)
{
    ...
    for (batch=1; batch<=batches; batch++) {
        ...
        total = worker+firm
        out[batch,1] = vckss_scale_engine__dot(
            cell_weight,worker:^2)
        out[batch,2] = vckss_scale_engine__dot(
            cell_weight,firm:^2)
        out[batch,4] = vckss_scale_engine__dot(
            cell_weight,total:^2)
...
void kssered__run()
{
    struct vckss_maker_result scalar generic_maker
    struct vckss_result scalar ordinary
    ...
    assert(vckss_scale_eng__moment_width() == 8)
    assert(vckss_scale_eng__target_rows() == 4096)
    assert(vckss_scale_eng__atom_rows() == 4096)
    assert(vckss_scale_eng__roundoff_gate() ==
    ...
    target_new = vckss_scale_eng__target_contract(
        cell_weight,prediction)
    assert(rows(target_new) == batches & cols(target_new) == 4)
    assert(kssered__reldif(target_new,target_legacy) <=
        8*vckss_scale_eng__roundoff_gate())
    assert(vckss_scale_eng__identity_resid(target_new) <=
    ...
    assert(vckss_scale_eng__valid_atoms(atoms,trials))
    ...
    assert(vckss_scale_eng__aggregate_ok(
    ...
    for (boundary_case=1; boundary_case<=cols(residual_targets);
        ...
        generic_maker = vckss__low_rank_maker(
    ...
    timed = vckss_scale_eng__run_atoms(
    ...
    ordinary = vckss_scale_eng__as_result(timed,.25)
    ...
    for (repetition=1; repetition<=3; repetition++) {
        ...
        timer_clear(82)
        timer_on(82)
        moments = vckss_scale_eng__moment_reduce(projection,residual)
        timer_off(82)
    ...
    st_matrix("scale_reduction_shape",(
        groups,probes,vckss_scale_eng__moment_width(),rows(moments) \
        cells,batches,vckss_scale_eng__target_rows(),rows(target_new) \
        groups,probes,vckss_scale_eng__atom_rows(),1))
}
