...
// fresh Stata process for every predecessor/candidate case pair.
...
if "`role'" == "baseline" {
    if "`command'" != "kss_bc" | "`private_prefix'" != "kssbc" exit 198
}
else {
    if "`command'" != "varcomp_kss" | "`private_prefix'" != "vckss" exit 198
}
...
quietly do `"`package_root'/`command'_rng.mata"'
...
program define _vkeq_emit_matrix
    ...
    forvalues row = 1/`rows' {
        forvalues col = 1/`cols' {
            ...
            if `is_timing' {
                if missing(scalar(__vkeq_value)) | scalar(__vkeq_value) < 0 {
...
program define _vkeq_rng_before
    ...
    if "$VKEQ_ROLE" == "baseline"                              ///
        mata: VKEQ_RNG_PRIME = kssbc_rng__capture_full()
    else mata: VKEQ_RNG_PRIME = vckss_rng__capture_full()
    if "$VKEQ_ROLE" == "baseline"                              ///
        mata: VKEQ_RNG_BEFORE = kssbc_rng__capture_full()
...
program define _vkeq_rng_after
    ...
    args case
    if "$VKEQ_ROLE" == "baseline"                              ///
        mata: VKEQ_RNG_AFTER = kssbc_rng__capture_full()
...
program define _vkeq_emit_econtents
    ...
    if "$VKEQ_ROLE" == "baseline" {
        capture mata: st_numscalar("__vkeq_cmg_api", kssbc_cmg__api_level())
...
local terminal_marker ///
    "VARCOMP_KSS_RENAME_EQ_PASS::`nonce'::`role'::`case_id'"
