...
*! generated_section_sha256 ea43a8400769614c7849bd9550ea0ac4c2333aeebc12546563e90db6d77ad390
...
real colvector ppmltalo_cmg__indexed_sum(
...
real colvector ppmltalo_cmg__forest_depth(real colvector parent)
{
...
struct ppmltalo_cmg__aggregation_result scalar ppmltalo_cmg__aggregate_gpl(
    struct ppmltalo_cmg__graph scalar graph,
    real colvector component,
    struct ppmltalo_cmg__options scalar options)
{
    ...
    depth = ppmltalo_cmg__forest_depth(parent)
    ...
    for (one_depth=max(depth); one_depth>=1; one_depth--) {
        ...
        if (rows(nodes) > 0) {
            contribution = ppmltalo_cmg__indexed_sum(
    ...
    if (rows(active) > 0) {
        ...
        internal = ppmltalo_cmg__indexed_sum(incidence_vertex,
    ...
    struct ppmltalo_cmg__options scalar options)
{
    ...
    if (rows(active) > 0) {
        forest_volume = ppmltalo_cmg__indexed_sum(
...
struct ppmltalo_cmg__aggregation_result scalar ppmltalo_cmg__aggregate_fallback(
    struct ppmltalo_cmg__graph scalar graph,
    real colvector component,
    struct ppmltalo_cmg__options scalar options)
{
...
real scalar ppmltalo_cmg__component_reduce(
    real scalar fine_vertices,
...
struct ppmltalo_cmg__level scalar ppmltalo_cmg__level_prepare_cert(
    struct ppmltalo_cmg__graph scalar graph,
    real colvector component)
{
    struct ppmltalo_cmg__level scalar out
    ...
    struct ppmltalo_cmg__graph scalar graph)
{
    ...
    component = ppmltalo_cmg__components(graph)
    ...
    return(ppmltalo_cmg__level_prepare_cert(graph,component))
}
...
struct ppmltalo_cmg__hierarchy scalar ppmltalo_cmg__attempt_begin(
...
struct ppmltalo_cmg__hierarchy scalar ppmltalo_cmg__attempt_update(
    struct ppmltalo_cmg__hierarchy scalar hierarchy,
    real scalar components,
    real scalar coarse_vertices,
...
struct ppmltalo_cmg__hierarchy scalar ppmltalo_cmg__hierarchy_mode(
    struct ppmltalo_cmg__graph scalar graph,
    struct ppmltalo_cmg__options scalar options,
    string scalar algorithm)
{
    ...
    while (1) {
        ...
        out = ppmltalo_cmg__attempt_begin(out,current,
        ...
        if (missing(predicted_level_bytes) |
            ...
            out = ppmltalo_cmg__attempt_update(out,.,.,.,0,"NONE",
        ...
        if (out.n_level == 0) {
            ...
            level = ppmltalo_cmg__level_prepare_cert(
        ...
        if (out.n_level == 0) initial_components = level.n_component
            ...
            out = ppmltalo_cmg__attempt_update(out,level.n_component,.,.,0,
        ...
        if (max(component_size) <= options.coarse_max) {
            ...
            if (level.status != "CONVERGED") {
                out.status = level.status
                out.message = level.message
                out = ppmltalo_cmg__attempt_update(out,level.n_component,
        ...
        if (algorithm == "STEINER_MATA" &
            ...
            aggregation_result = ppmltalo_cmg__aggregate_gpl(
            ...
            forest = ppmltalo_cmg__forest_select(
                current,level.component,options)
        ...
        reduction = ppmltalo_cmg__component_reduce(current.n_vertex,
            aggregation_result.n_coarse,level.n_component)
        if (missing(reduction) | reduction < options.min_reduction) {
            aggregation_result = ppmltalo_cmg__aggregate_fallback(
                current,level.component,options)
            if (aggregation_result.status != "CONVERGED") {
                out.status = aggregation_result.status
                out.message = aggregation_result.message
...
struct ppmltalo_cmg__hierarchy scalar ppmltalo_cmg__hierarchy_build(
    ...
    struct ppmltalo_cmg__options scalar options)
{
    return(ppmltalo_cmg__hierarchy_mode(
        graph,options,"STEINER_MATA"))
}

struct ppmltalo_cmg__hierarchy scalar ppmltalo_cmg__hierarchy_v5(
    struct ppmltalo_cmg__graph scalar graph,
    struct ppmltalo_cmg__options scalar options)
{
    return(ppmltalo_cmg__hierarchy_mode(
